...
The recommended way to get started using the Node.js Core driver is by using the `NPM` (Node Package Manager) to install the dependency in your project.
...
This will download the MongoDB Core driver and add a dependency entry in your `package.json` file.
...
```
npm --loglevel verbose install mongodb-core
```
