...
function connectionFailureHandler(self, event) {
  return function(err) {
    ...
    // console.log(self.allConnections().map(function(x) { return x.id}))
    ...
    // console.log("=== connectionFailureHandler :: " + event + " :: 3 :: " + self.socketCount())
    // console.log(this.id)
    // console.log(self.allConnections().map(function(x) { return x.id}))

    // No more socket available propegate the event
    if(self.socketCount() == 0) {
      // console.log("=== connectionFailureHandler :: " + event + " :: 4")
      if(self.state != DESTROYED && self.state != DESTROYING) {
    ...
    authenticateStragglers(self, connection, function(err) {
      ...
      if(!workItem.immediateRelease) {
        ...
        || message.documents[0]['errmsg'] || message.documents[0]['code'])) {
          // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! messageHandler :: " + connection.id + " :: 0 " + connection.port)
        ...
        // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! messageHandler :: " + connection.id + " :: 1 " + connection.port)
        //self.inUseConnections, self.availableConnections
        // console.dir(self.inUseConnections.map(function(x) { return x.id}))
        // console.dir(self.availableConnections.map(function(x) { return x.id}))
        // console.log(workItem.cb.toString())
...
Pool.prototype.destroy = function() {
  // console.log("   ^^ POOL::destroy 0 :: " + this.id)
  var self = this;
  // Do not try again if the pool is already dead
  if(this.state == DESTROYED || self.state == DESTROYING) return;
  // console.log("   ^^ POOL::destroy 1 :: " + this.id)
  // Set state to destroyed
  stateTransition(this, DESTROYING);
  // console.log("   ^^ POOL::destroy 2 :: " + this.id)

  // console.log("destroy =========================== 1")
  // Wait for the operations to drain before we close the pool
  function checkStatus() {
    // console.log("========= destroy :: checkStatus 0 :: " + self.id + " :: " + self.queue.length)
    if(self.queue.length == 0) {
      ...
      // console.dir(connections.map(function(x) { return x.id }))
    ...
    waitForAuth(function() {
      ...
      while(true) {
        ...
        if(connection.isConnected()) {
          ...
          // console.log(workItem.cb.toString())
          // console.log(workItem.immediateRelease)
