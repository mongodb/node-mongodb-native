...
ReplSet.prototype.getConnection = function(options) {
  ...
  var server = pickServer(this.s, options.readPreference);
...
ReplSet.prototype.getServer = function(options) {
  ...
  options = options || {};
  // Pick the right server based on readPreference
  return pickServer(this.s, options.readPreference);
}
...
var executeWriteOperation = function(self, op, ns, ops, options, callback) {
  ...
  // Get a primary    
  try {
    server = pickServer(self.s, ReadPreference.primary);
...
ReplSet.prototype.command = function(ns, cmd, options, callback) {
  ...
  // Pick the right server based on readPreference
  try {
    server = pickServer(self.s, options.writeConcern ? ReadPreference.primary : options.readPreference);
  ...
  while(servers.length > 0) {
    ...
    var finalArguments = argsWithoutCallback.concat([function(err, r) {
      ...
      if(count == 0) {
        ...
        if(authErr == null) addCredentials(this.s, db, argsWithoutCallback);
...
var pickServer = function(s, readPreference) {
