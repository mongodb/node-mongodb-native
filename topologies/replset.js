...
var State = function() {
  ...
  this.contains = function(address) {
    ...
    // for(var i = 0; i < secondaries.length; i++) {
    //   console.log("secondary :: " + secondaries[i].name)
    // }
...
var ReplSet = function(seedlist, options) {
  ...
  var errorHandler = function(err, server) {
    ...
    if(emitError && self.listeners('error').length > 0) self.emit('error', err, server);
  }
  ...
  var replicasetInquirer = function(norepeat) {
    ...
    // console.log("========================================== inspectServer 0")
          ...
          //   console.log("================================ FOUND NON VALID SERVER")
          ...
          if(ismaster.ismaster && setName == ismaster.setName
            ...
            self.left('left', 'secondary', server);
          }
  ...
  var errorHandlerTemp = function(event) {
    return function(err, server) {
      // console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ errorHandlerTemp :: " + server.name + " :: " + event + " :: " + state)
      // console.dir(err)
      // Log the information
      ...
      // console.log("===================== initialConnectionServers :: " + initialConnectionServers.length)

      // if(initialConnectionServers.length == 0 
      //   && !replState.isSecondaryConnected() && !replState.isPrimaryConnected()) {
      ...
      // console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ errorHandlerTemp :: " + server.name + " :: 1")

      // End connection operation if we have no legal replicaset state
      if(initialConnectionServers == 0 && state == CONNECTING) {
      ...
      if(disconnectedServers.length == seedlist.length && state == CONNECTING) {
      ...
      // console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ errorHandlerTemp :: " + server.name + " :: 2")
    }
  }
  ...
  var connectToServer = function(host, port) {
    ...
    opts.emitError = true;
    // console.log("############################ ADDED SERVER WITH 2")
    // console.dir(opts.port)
  ...
  var inInitialConnectingServers = function(address) {
    // console.log("=============================== inInitialConnectingServers :: " + initialConnectionServers.length)

    for(var i = 0; i < initialConnectionServers.length; i++) {
      // console.log(initialConnectionServers[i].name + " = " + address)
  ...
  var processHosts = function(hosts) {
    // console.log("################################ START processHosts " + state)
    if(state == DESTROYED) return;
    if(Array.isArray(hosts)) {
      ...
      for(var i = 0; i < hosts.length; i++) {
        ...
        if(!replState.contains(hosts[i])) {
          // console.log("########################### processHosts :: " + id)
          // console.log(Object.keys(connectingServers))
          // console.log(hosts[i])
  ...
  var connectHandler = function(server) {
    ...
    // console.log("################ CONNECTED :: " + server.name + " :: " + setName)
    if(state == DESTROYED) return;
    ...
    var processNewServer = function() {
      ...
      if(ismaster.setName && setName != ismaster.setName) {
        ...
        self.emit('error', new MongoError("provided setName for Replicaset Connection does not match setName found in server seedlist"));
        return self.destroy();
      }
      ...
      // // Still no connection candidates
      // if(((ismaster.ismaster == false && (ismaster.primary == null || ismaster.primary == '') && !secondaryOnlyConnectionAllowed)
      //   || (initialConnectionServers.length == 0 && Object.keys(connectingServers).length == 0 && ismaster.ismaster == false && !replState.isPrimaryConnected() && !secondaryOnlyConnectionAllowed))
      //   && state == CONNECTING && ismaster.setName) {
      //   // console.log("--------------------------------------- yo 1 :: " + state)
      //   // console.log(ismaster)
      //   // console.log(initialConnectionServers.length)
  ...
  this.connect = function(_options) {
    ...
    seedlist.forEach(function(e) {
      ...
      opts.authProviders = authProviders;
      // Create a new Server
      // console.log("############################ ADDED SERVER WITH 0 :: " + id)
      // console.dir(opts.port)
      var server = new Server(opts);
    ...
    // console.log("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ connect :: " + initialConnectionServers.length + " :: " + id)
    ...
    while(disconnectedServers.length > 0) {
      ...
      // console.log("############################ ADDED SERVER WITH 1")
      // console.dir(opts.port)
  ...
  var pickServer = function(readPreference) {
    ...
    // console.log("================================================== :: " + readPreference.preference)
    ...
    if(readPreferenceStrategies != null && readPreferenceStrategies[readPreference.preference] != null) {
      // console.log("================================================== :: " + readPreference.preference)
      // console.dir(Object.keys(readPreferenceStrategies))
      // console.dir(readPreferenceStrategies[readPreference.preference])
      // console.log("------------------------------------- 0")
