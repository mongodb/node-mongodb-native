...
export interface CreateIndexesOptions extends Omit<CommandOperationOptions, 'writeConcern'> {
   ...
   * them in the `indexOptions` parameter of the three parameter `createIndex` overload, which is
   * typed {@link IndexOptions}.
   */
  background?: boolean;
  /**
  ...
  /**
   * Specifies how many data-bearing members of a replica set, including the primary, must complete
   * the index builds successfully before the primary marks the indexes as ready. This option
   * accepts the same values for the "w" field in a write concern plus "votingMembers", which
   * indicates all voting data-bearing nodes.
   */
  ...
  weights?: Document;
  /**
   * Renamed to `defaultLanguage` on {@link IndexOptions}.
   *
   * @deprecated Index options will be removed from this type in a future major release. Pass
   * them in the `indexOptions` parameter of the three parameter `createIndex` overload, which is
   * typed {@link IndexOptions}.
  ...
  /**
   * Renamed to `languageOverride` on {@link IndexOptions}.
   *
   * @deprecated Index options will be removed from this type in a future major release. Pass
   * them in the `indexOptions` parameter of the three parameter `createIndex` overload, which is
   * typed {@link IndexOptions}.
