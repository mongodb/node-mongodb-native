...
exports['insert with w=1 db level'] = function(test) {
...
exports['insert with w=1 collection level'] = function(test) {
  var db = new Db(MONGODB, 
    new Server("127.0.0.1", 27017, {auto_reconnect: true, poolSize: 4, ssl:useSSL})
...
exports['insert with w=1 operation level'] = function(test) {
  var db = new Db(MONGODB, 
    new Server("127.0.0.1", 27017, {auto_reconnect: true, poolSize: 4, ssl:useSSL})
    , {w:0, native_parser: (process.env['TEST_NATIVE'] != null)});
  db.open(function(err, db) {
...
exports['insert with journal db level'] = function(test) {
  var db = new Db(MONGODB, 
    new Server("127.0.0.1", 27017, {auto_reconnect: true, poolSize: 4, ssl:useSSL})
    , {journal:true, native_parser: (process.env['TEST_NATIVE'] != null)});
  db.open(function(err, db) {
    db.collection('insert_with_w_1').update({a:1}, {a:1}, {upsert:true}, function(err, result) {
      test.equal(null, err);
      test.equal(1, result);
...
exports['insert with journal and w == 1 at db level'] = function(test) {
  var db = new Db(MONGODB, 
    new Server("127.0.0.1", 27017, {auto_reconnect: true, poolSize: 4, ssl:useSSL})
    , {w:1, journal:true, wtimeout:1000, native_parser: (process.env['TEST_NATIVE'] != null)});
  db.open(function(err, db) {
    db.collection('insert_with_w_1').update({a:1}, {a:1}, {upsert:true}, function(err, result) {
      test.equal(null, err);
      test.equal(1, result);
