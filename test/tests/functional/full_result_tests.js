 ...
 * @ignore
 */
exports['Should correctly execute save using fullResult'] = {
  // Add a tag that our runner can trigger on
  // in this case we are setting that node needs to be higher than 0.10.X to run
  requires: {mongodb: ">2.5.5"},
  ...
  test: function(configuration, test) {
    ...
    db.open(function(err, db) {
      ...
      // Create a collection to hold our documents
      db.createCollection('test_full_result_0', function(err, collection) {

        // Insert a test document
        collection.save({a:1}, {w:1, fullResult:true}, function(err, result) {
          test.equal(null, err);
          test.equal(true, result.ok);
