...
exports['Should correctly perform a simple server connection using mock'] = {
  ...
  test: function(configuration, test) {
    ...
    co(function*() {
      // Create a server
      mockServer = yield mockupdb.createServer(27037, 'localhost');

      // Wait for ismaster
      var request = yield mockServer.receive();
      test.equal('op_query', request.type);
      test.deepEqual({ismaster:true}, request.document);
      ...
      // Wait for ismaster
      request = yield mockServer.receive();
      test.equal('op_query', request.type);
      test.deepEqual({ismaster:true}, request.document);
      request.reply({ "ismaster" : true,
      	"maxBsonObjectSize" : 16777216, "maxMessageSizeBytes" : 48000000,
