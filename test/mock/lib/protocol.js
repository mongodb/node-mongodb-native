...
var Query = function(bson, data) {
  ...
  var index = 0;
  // Message size
  this.size = data[index] | data[index + 1] << 8 | data[index + 2] << 16 | data[index + 3] << 24;
  index = index + 4;
  // requestId
  ...
  index = index + 4;
  // opCode
  this.opCode = data[index] | data[index + 1] << 8 | data[index + 2] << 16 | data[index + 3] << 24;
  index = index + 4;

  // flags
  this.flags = data[index] | data[index + 1] << 8 | data[index + 2] << 16 | data[index + 3] << 24;
  index = index + 4;
  ...
  // numberToSkip
  this.numberToSkip = data[index] | data[index + 1] << 8 | data[index + 2] << 16 | data[index + 3] << 24;
  index = index + 4;

  // numberToReturn
  this.numberToReturn = data[index] | data[index + 1] << 8 | data[index + 2] << 16 | data[index + 3] << 24;
  index = index + 4;

  // Read the document size
  var docSize = numberToReturn = data[index] | data[index + 1] << 8 | data[index + 2] << 16 | data[index + 3] << 24;
