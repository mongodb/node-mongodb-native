 ...
 * @ignore
 */
exports['Should correctly connect to the mongoses using the connection string'] = function(test) {
  Db.connect("mongodb://localhost:50000,localhost:50001", function(err, db) {
    test.equal(null, err);
    test.equal(2, db.serverConfig.allServerInstances().length);
