...
// Cases 5-6 preview: substringPreview removed in server 9.0.0 (SERVER-129158).
// `substringPreview` is queried with the `String` algorithm, which requires libmongocrypt 1.19.1+
// (MONGOCRYPT-937), matching the prefixPreview/suffixPreview floor.
...
describe('27. String Explicit Encryption', function () {
  ...
  beforeEach(async function () {
    ...
    //   encryptedFields-substring-ci-di.json. This step requires server 9.0.0+.
    ...
    // Create a MongoClient with auto-encryption enabled (without `bypassQueryAnalysis`) named
    // `autoEncryptedClient` with these `AutoEncryptionOpts`:
    // class AutoEncryptionOpts {
    //    keyVaultNamespace: "keyvault.datakeys",
    //    kmsProviders: { "local": { "key": <base64 decoding of LOCAL_MASTERKEY> } },
    // }
  ...
  // This is a regression test for DRIVERS-3470. Requires server 9.0.0+ and libmongocrypt 1.19.0+.
  it(
  ...
  it(
    ...
    async function () {
      ...
      // caseSensitive: false, diacriticSensitive: false, queryType: "prefix", contentionFactor: 0.
      ...
      // Assert the following document is returned: { "encryptedText": "cafébarbäz" }
      const {
        _id: _id1,
        __safeContent__: _sc1,
        ...prefixResult
      ...
      // Use `clientEncryption.encrypt()` to encrypt the string `"baz"` with the following `EncryptOpts`:
      // caseSensitive: false, diacriticSensitive: false, queryType: "suffix", contentionFactor: 0.
      const encryptedBaz = await clientEncryption.encrypt('baz', {
        keyId: keyId1,
        algorithm: 'String',
        queryType: 'suffix',
  ...
  // This is a regression test for DRIVERS-3470. Requires server 9.0.0+ and libmongocrypt 1.20.0+.
  it(
  ...
  it(
    ...
    async function () {
      ...
      // Use `clientEncryption.encrypt()` to encrypt the string `"cafe"` with the following `EncryptOpts`:
      // caseSensitive: false, diacriticSensitive: false, queryType: "substring", contentionFactor: 0.
      const encryptedCafe = await clientEncryption.encrypt('cafe', {
        keyId: keyId1,
        algorithm: 'String',
        queryType: 'substring',
