...
/**
 * The two parameter form filters index options against an allowlist before sending them
 * to the server, so options the server supports but the driver has not learned about yet are
 * silently dropped. The three parameter form —
 * `createIndex(keys, indexOptions, commandOptions)` — turns the filter off: supplying command
 * options separately is what opts a caller into passthrough.
 *
...
describe('createIndex option validation', function () {
  ...
  describe('when only index options are given (two parameter form)', function () {
  ...
  describe('when command options are given (three parameter form)', function () {
    ...
    describe('and command options are passed in the third parameter', function () {
      ...
      it('sends a session on the command and not in the index description', async function () {
        ...
        try {
          await collection.createIndex({ i: 1 }, { unique: true }, { session });
      ...
      it('sends a writeConcern on the command and not in the index description', async function () {
        await collection.createIndex({ j: 1 }, { unique: true }, { writeConcern: { w: 1 } });
      ...
      it('sends index options and command options together', async function () {
        await collection.createIndex(
          ...
          { maxTimeMS: 1000, writeConcern: { w: 1 } }
        );
