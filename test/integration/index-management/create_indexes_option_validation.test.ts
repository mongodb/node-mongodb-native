...
describe('createIndex option validation', function () {
  ...
  describe('when validateOptions is false', function () {
    it('does not send driver options the user never supplied', async function () {
      // the options bag has been through `resolveOptions`, which injects the BSON options,
      // `readPreference` and `timeoutMS` whether or not the user asked for them
    ...
    // TODO(NODE-6893): the options bag is both index options and command options, so turning the
    // allowlist off currently lets user-supplied command options through to the index description.
    // These capture the intended behaviour and fail today.
    it('keeps a user-supplied comment out of the index description', async function () {
      await collection.createIndex(
        { g: 1 },
        { unique: true, comment: 'a comment' },
        { validateOptions: false }
      );

      expect(sentIndexes()).to.deep.equal([{ unique: true, name: 'g_1', key: { g: 1 } }]);
      expect(sentCommand()).to.have.property('comment', 'a comment');
    });

    it('keeps a user-supplied maxTimeMS out of the index description', async function () {
      await collection.createIndex(
        { h: 1 },
        { unique: true, maxTimeMS: 1000 },
        { validateOptions: false }
      );
    ...
    it('keeps a user-supplied writeConcern out of the index description', async function () {
      await collection.createIndex(
        { j: 1 },
