...
async function getRTTPingers(client: MongoClient) {
  type RTTPingerConnection = Omit<RTTPinger, 'connection'> & {
...
describe('class RTTPinger', () => {
  ...
  describe('when serverApi is enabled', () => {
  ...
  describe('when serverApi is disabled', () => {
    ...
    describe('connected to a pre-hello server', () => {
    ...
    describe('connected to a helloOk server', () => {
      it('measures rtt with a hello command', async function () {
        await client.connect();
        const rttPingers = await getRTTPingers(client);
        const spies = rttPingers.map(rtt => sinon.spy(rtt.connection, 'command'));
  ...
  describe(`when the RTTPinger's hello command receives any error`, () => {
    let client: MongoClient;
