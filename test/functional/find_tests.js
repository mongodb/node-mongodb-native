  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('test_where', function(err, collection) {
        collection.insert([{ a: 1 }, { a: 2 }, { a: 3 }], configuration.writeConcernMax(), function(
          err,
          ids
        ) {
          collection.count(function(err, count) {
...
exports.shouldCorrectlyRetrieveSingleRecord = {
  ...
  // The actual test we wish to run
  test: function(configuration, test) {
    var p_client = configuration.newDbInstance(configuration.writeConcernMax(), {
      poolSize: 1,
        ...
        collection.insert({ a: 1, b: 2 }, configuration.writeConcernMax(), function(err, doc) {
          ...
          collection.findAndModify(
            ...
            function(err, updated_doc) {
              ...
              collection.insert({ a: 2, b: 2 }, configuration.writeConcernMax(), function(
                err,
                doc
              ) {
                // Let's modify the document in place
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('skipAndLimitOnFindWithToArray', function(err, collection) {
        collection.insert([{ a: 1 }, { b: 2 }, { c: 3 }], configuration.writeConcernMax(), function(
          err,
          result
        ) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('shouldCorrectlyFindAndModifyDocumentThatFailsInFirstStep', function(
        ...
        collection.ensureIndex([['failIndex', 1]], { unique: true, w: 1 }, function(err, index) {
          ...
          collection.insert({ a: 2, b: 2, failIndex: 2 }, configuration.writeConcernMax(), function(
            err,
            doc
          ) {
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('should_correctly_return_record_with_64bit_id', function(
        ...
        collection.insert([lowerDoc, higherDoc], configuration.writeConcernMax(), function(
          err,
          result
        ) {
          test.ok(err == null);
...
exports.shouldCorrectlyHandlerErrorForFindAndModifyWhenNoRecordExists = {
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      var db = client.db(configuration.database);
      db.createCollection('shouldCorrectlyHandlerErrorForFindAndModifyWhenNoRecordExists', function(
        err,
        collection
      ) {
    ...
    p_client.connect(function(err, client) {
      ...
      db.collection('collection1', function(err, collection) {
        ...
        setTimeout(function() {
          ...
          collection.insert({ _id: id, a: 1 }, configuration.writeConcernMax(), function(
            err,
            result
          ) {
            test.equal(null, err);
        ...
        collection.insert(
          ...
          function(err, r) {
            ...
            collection.ensureIndex('login', { unique: true, w: 1 }, function(err, result) {
              ...
              collection.findAndModify({ _id: id }, [], { $set: { login: 'user1' } }, {}, function(
                err,
                user
              ) {
                test.ok(err != null);
    ...
    client.connect(function(err, client) {
      ...
      col.insert(
        ...
        function(err, result) {
          ...
          col
            .find({}, { skip: 1, limit: 1, fields: { _id: 1, b: 0 } })
            .toArray(function(err, docs) {
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      var collection = db.collection('test_find_simple_batchsize_0', function(err, collection) {
        ...
        // Insert some test documents
        collection.insert([{ a: 2 }, { b: 3 }, { b: 4 }], configuration.writeConcernMax(), function(
          err,
          r
        ) {
          // Ensure correct insertion testing via the cursor and the count function
