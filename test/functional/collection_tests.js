...
exports.shouldCorrectExecuteBasicCollectionMethods = {
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      var collection = db.createCollection('test_collection_methods', function(err, collection) {
      ...
      db.createCollection('test.spiderman', function(r) {
        db.createCollection('test.mario', function(r) {
          ...
          db.collection('test.spiderman', function(err, spiderman_collection) {
            spiderman_collection.insert({ foo: 5 }, configuration.writeConcernMax(), function(
              err,
              r
            ) {
              db.collection('test.mario', function(err, mario_collection) {
                mario_collection.insert({ bar: 0 }, configuration.writeConcernMax(), function(
                  err,
                  r
                ) {
                  // Assert collections
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('test_collection_names', function(err, r) {
        ...
        db.listCollections().toArray(function(err, documents) {
          ...
          documents.forEach(function(document) {
            if (
              document.name == configuration.database + '.test_collection_names' ||
          ...
          collection.insert({ a: 1 }, configuration.writeConcernMax(), function(err, r) {
            ...
            db.listCollections().toArray(function(err, documents) {
              documents.forEach(function(document) {
                if (
                  document.name == configuration.database + '.test_collection_names2' ||
    ...
    client.connect(function(err, client) {
      ...
      db.collection('does-not-exist', { strict: true }, function(err, collection) {
        ...
        db.createCollection('test_strict_access_collection', function(err, collection) {
          db.collection('test_strict_access_collection', configuration.writeConcernMax(), function(
            err,
            collection
          ) {
            test.equal(null, err);
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('test_strict_create_collection', function(err, collection) {
        ...
        db.createCollection('test_strict_create_collection', { strict: true }, function(
          err,
          collection
        ) {
          test.ok(err instanceof Error);
      ...
      db.createCollection('test_invalid_key_names', function(err, collection) {
        ...
        collection.insert(
          ...
          function(err, r) {
            ...
            collection.insert({ $hello: 'world' }, configuration.writeConcernMax(), function(
              err,
              doc
            ) {
              test.ok(err instanceof Error);
              ...
              collection.insert(
                ...
                function(err, doc) {
                  ...
                  collection.insert({ he$llo: 'world' }, configuration.writeConcernMax(), function(
                    err,
                    docs
                  ) {
                    test.equal(null, err);
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('test_should_execute_insert_update_delete_safe_mode', function(
        ...
        collection.insert({ i: 1 }, configuration.writeConcernMax(), function(err, r) {
          ...
          // Update the record
          collection.update({ i: 1 }, { $set: { i: 2 } }, configuration.writeConcernMax(), function(
            err,
            result
          ) {
            test.equal(null, err);
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('save_error_on_save_test', function(err, collection) {
        ...
        collection.createIndex([['username', 1]], configuration.writeConcernMax(), function(
          err,
          result
        ) {
          var doc = {
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('shouldCorrectlyCreateTTLCollectionWithIndexUsingEnsureIndex', function(
        ...
        collection.ensureIndex({ createdAt: 1 }, { expireAfterSeconds: 1, w: 1 }, function(
          err,
          result
        ) {
          test.equal(null, err);
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('shouldCorrectlyCreateTTLCollectionWithIndexCreateIndex', {}, function(
        ...
        collection.createIndex({ createdAt: 1 }, { expireAfterSeconds: 1, w: 1 }, function(
          err,
          result
        ) {
          test.equal(null, err);
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db
        ...
        .update({ 'item.i': 1 }, { $set: { a: 1 } }, function(err, result, full) {
          test.equal(null, err);
...
exports['Should correctly execute update with elemMatch field in selector'] = {
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db
        .collection('executeUpdateWithElemMatch')
        .update({ item: { $elemMatch: { name: 'my_name' } } }, { $set: { a: 1 } }, function(
          err,
          result,
          full
        ) {
