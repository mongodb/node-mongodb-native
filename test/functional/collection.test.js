...
describe('Collection', function () {
  ...
  describe('standard collection tests', function () {
    ...
    it('should fail due to illegal listCollections', function () {
      expect(() => {
        db.collection(5);
      }).to.throw('collection name must be a String');

      expect(() => {
        db.collection('');
      }).to.throw('collection names cannot be empty');

      expect(() => {
        db.collection('te$t');
      }).to.throw("collection names must not contain '$'");

      expect(() => {
        db.collection('.test');
      }).to.throw("collection names must not start or end with '.'");
