...
import { MongoError } from '../../../lib/core/error';
import { Binary, Long, ObjectID as ObjectId } from 'bson'
import {
  CommandFailedEvent,
  CommandStartedEvent,
...
import { ExpectedError, ExpectedEvent, Document } from './schema';

function isLong(maybeLong?: any): maybeLong is Long {
  return maybeLong && typeof maybeLong === 'object' && maybeLong._bsontype === 'Long'
}
...
TYPE_MAP.set(
  'long',
  actual => (typeof actual === 'number' && Number.isInteger(actual)) || isLong(actual)
);
...
export function resultCheck(
  ...
  if (typeof expected === 'object' && expected) {
    ...
    if (isLong(actual) || isLong(expected)) {
      // <Long> case (if either is a long than its safe to convert the other to confirm a match)
      const actLong = Long.fromNumber((actual as unknown) as number)
      const expLong = Long.fromNumber((expected as unknown) as number)
      // Long requires special equality check
      expect(actLong.equals(expLong), `at ${path.join('')}`).to.be.true;
    } else {
      // <number | string | boolean | null> case
      expect(actual, `at ${path.join('')}`).to.equal(expected);
    }
  }
}
