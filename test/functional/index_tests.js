        ...
        collection.insert(
          ...
          function(err, ids) {
            ...
            db.createCollection('test_index_on_subfield2', function(err, collection) {
              db.createIndex(collection.collectionName, 'hello_a', { w: 1, unique: true }, function(
                err,
                indexName
              ) {
                test.equal(null, err);
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('geospatial_index_altered_test', function(err, r) {
        db.collection('geospatial_index_altered_test', function(err, collection) {
          collection.ensureIndex({ loc: '2d' }, { min: 0, max: 1024, w: 1 }, function(
            err,
            indexName
          ) {
            collection.insert({ loc: [100, 100] }, configuration.writeConcernMax(), function(
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('shouldThrowDuplicateKeyErrorWhenCreatingIndex', function(
        ...
        collection.insert([{ a: 1 }, { a: 1 }], configuration.writeConcernMax(), function(
          err,
          result
        ) {
          test.equal(null, err);
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db.createCollection('shouldCorrectlyUseMinMaxForSettingRangeInEnsureIndex', function(
        ...
        collection.ensureIndex({ loc: '2d' }, { min: 200, max: 1400, w: 1 }, function(
          err,
          indexName
        ) {
          test.equal(null, err);
...
exports[
  'should correctly return error message when applying unique index to duplicate documents'
] = {
  metadata: {
    requires: { topology: ['single', 'replicaset', 'sharded', 'ssl', 'heap', 'wiredtiger'] }
  },
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db
        ...
        .createIndex({ a: 1 }, { partialFilterExpression: { a: 1 } }, function(err, r) {
          test.equal(null, err);
  ...
  test: function(configuration, test) {
    ...
    client.connect(function(err, client) {
      ...
      db
        ...
        .createIndex({ 'accessControl.get': 1 }, { background: true }, function(err, r) {
          test.equal(null, err);
