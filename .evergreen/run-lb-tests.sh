...
TEST_NPM_SCRIPT="test:load-balancer"
if [[ -z "${NO_EXIT}" ]]; then
...
SINGLE_MONGOS_LB_URI=${SINGLE_MONGOS_LB_URI} MULTI_MONGOS_LB_URI=${MULTI_MONGOS_LB_URI} MONGODB_API_VERSION=${MONGODB_API_VERSION} MONGODB_UNIFIED_TOPOLOGY=${UNIFIED} MONGODB_URI=${MONGODB_URI} FAKE_MONGODB_SERVICE_ID="true" npm run ${TEST_NPM_SCRIPT}
