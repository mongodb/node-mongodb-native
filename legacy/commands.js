...
var Insert = function(requestId, ismaster, bson, ns, documents, options) {
  ...
  if(!Array.isArray(documents) || documents.length == 0) throw new Error("documents array must contain at least one document to insert");
  ...
  this.toBin = function() {
    ...
    for(var i = 0; i < documents.length; i++) {
      ...
      if(docsize > ismaster.maxBsonObjectSize) {
        throw new Error("Document exceeds maximum allowed bson size of " + ismaster.maxBsonObjectSize + " bytes");        
      }
    ...
    if(length > ismaster.maxBsonObjectSize) {
      throw new Error("Command exceeds maximum message size of " + ismaster.maxMessageSizeBytes + " bytes");
    }
    ...
    for(var i = 0; i < documents.length; i++) {
      // Serialize the entry
      var docSize = bson.serializeWithBufferAndIndex(documents[i], checkKeys, buffer, index, serializeFunctions) - index + 1;
      // Adjust the index
      index = index + docSize;
...
var Update = function(requestId, ismaster, bson, ns, update, options) {  
  ...
  this.toBin = function() {
    ...
    index += bson.serializeWithBufferAndIndex(q, checkKeys, buffer, index, serializeFunctions) - index + 1;
    // Serialize the update statement
    index += bson.serializeWithBufferAndIndex(u, false, buffer, index, serializeFunctions) - index + 1;
