...
var LegacySupport = function() {
  ...
  //
  var aggregateInsertResults = function(results, connection) {
    var final = { ok: 1, n: 0 }
  ...
  var executeOrdered = function(command, ismaster, ns, bson, pool, callbacks, ops, options, callback) {
    ...
    var executeOp = function(list, _callback) {
      ...
      if(list.length == 0) return _callback(null, aggregateInsertResults(getLastErrors, connection));
      ...
      try {
        ...
        callbacks.once(getLastErrorOp.requestId, function(err, result) {
          ...
          if(doc.ok == 0 || doc.err || doc.errmsg) return callback(null, aggregateInsertResults(getLastErrors, connection));
        ...
        return callback(null, aggregateInsertResults(getLastErrors, connection));
      }
    }
  ...
  var executeLegacy = function(ismaster, ns, bson, pool, callbacks, ops, options, callback) {
  }
  ...
  this.insert = function(ismaster, ns, bson, pool, callbacks, ops, options, callback) {
    ...
    // We are executing in legacy mode (no reporting)
    if(legacy) {
      return executeLegacy(ismaster, ns, bson, pool, callbacks, ops, options, callback);
    }
