...
var checkCollectionName = require('./utils').checkCollectionName
  ...
  , GeneralCursor = require('./general_cursor')
...
var listIndexesTranforms = {
  query: function(result) {
    if(result.documents.length == 1 && result.documents[0].indexes) {
      return result.documents[0].indexes;          
    }

    return resul.documents;
  }
}
...
Collection.prototype.listIndexes = function(options) {
  ...
  if(this.s.db.serverConfig.capabilities().hasListIndexesCommand) {
    ...
    options.cursorFactory = GeneralCursor;
    // Filter out the correct field values
    options.transforms = listIndexesTranforms;
    // Execute the cursor
    return this.s.topology.cursor(f('%s.$cmd', this.s.dbName), command, options);
  }

  // Create a cursor for the indexes collection using this namespace
  var cursor = this.s.db.collection('system.indexes').find({ns: this.s.namespace});
