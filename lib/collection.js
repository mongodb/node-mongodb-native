...
var Collection = function(db, topology, dbName, name, pkFactory, options) {  
  ...
  this.update = function(selector, document, options, callback) {
    ...
    // level or db
    options['serializeFunctions'] = options['serializeFunctions'] || this.serializeFunctions;
  ...
  this.remove = function(selector, options, callback) {
    ...
    // Build the op
    var op = {q: selector, multi: 0};
    if(options.single) op.multi = 1;
    ...
    db.admin().command(cmd, opt, function(err, doc) {
      ...
      try {
        if(opt.new_collection) {
          return callback(null, new Collection(db, topology, dbName, name, pkFactory, options));
        }
        name = newName;
        callback(null, self);      
      } catch(err) {
