...
var Collection = function(db, topology, dbName, name, pkFactory, options) {  
  ...
  this.options = function(callback) {
    var cursor = db.collectionsInfo(name);
  ...
  this.aggregate = function(pipeline, options, callback) {
    ...
    // if((typeof callback == 'function' && callback.pipeline) || args[0] == undefined) 
    ...
    //   && !command.explain
    //   && (topology.capabilities().hasAggregationCursor || typeof callback != 'function')) {
    // console.log("-------------------------------------- 1")
    //   command.cursor = options.cursor;
    //   if(typeof options.allowDiskUse == 'boolean') command.allowDiskUse = options.allowDiskUse;
    //   // Execute the cursor
