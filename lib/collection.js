...
Collection.prototype.insert = function(docs, options, callback) {
  ...
  for(var i = 0; i < docs.length; i++) {
    if(docs[i]._id == null) docs[i]._id = this.s.pkFactory.createPk();
  }

  // File inserts
  this.s.topology.insert(this.s.namespace, docs, finalOptions, function(err, result) {
...
Collection.prototype.update = function(selector, document, options, callback) {
  ...
  // Update options
  this.s.topology.update(this.s.namespace, [op], finalOptions, function(err, result) {
    if(callback == null) return;
 ...
 * @return {null}
 */
Collection.prototype.removeOne = function(filter, options, callback) {
  if(typeof options == 'function') callback = options, options = {};
 ...
 * @return {null}
 */
Collection.prototype.removeMany = function(filter, options, callback) {
  if(typeof options == 'function') callback = options, options = {};
  var options = shallowClone(options);    
...
Collection.prototype.remove = function(selector, options, callback) {
  ...
  // Execute the remove
  this.s.topology.remove(this.s.namespace, [op], finalOptions, function(err, result) {
    if(callback == null) return;
    if(err) return handleCallback(callback, err, null);
    if(result == null) return handleCallback(callback, null, null);
    if(result.result.code) return handleCallback(callback, toError(result.result));
