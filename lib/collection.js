...
Collection.prototype.find = function() {
  ...
  if(len === 1) {
    ...
    } else if(Array.isArray(fields) && !Array.isArray(fields[0])) {
      var newFields = {};
      // Rewrite the array
  ...
  if(Buffer.isBuffer(selector)) {
    var object_size = selector[0] | selector[1] << 8 | selector[2] << 16 | selector[3] << 24;
    if(object_size != selector.length)  {
      var error = new Error("query selector raw message size does not match message header size [" + selector.length + "] != [" + object_size + "]");
  ...
  if(Buffer.isBuffer(fields)) {
    object_size = fields[0] | fields[1] << 8 | fields[2] << 16 | fields[3] << 24;
    if(object_size != fields.length)  {
      error = new Error("query fields raw message size does not match message header size [" + fields.length + "] != [" + object_size + "]");
      error.name = 'MongoError';
      throw error;
    }
  }
  ...
  if(options) {
    if (options.fields && !(Buffer.isBuffer(options.fields))) {
      fields = {};
...
var updateOne = function(self, filter, update, options, callback) {
  ...
  updateDocuments(self, filter, update, options, function(err, r) {
    ...
    if(err && callback) return callback(err);
    if(r == null) return callback(null, {result: {ok:1}});
    var isArrayResultUpserted = (Array.isArray(r.result.upserted));
    r.modifiedCount = r.result.nModified != null ? r.result.nModified : r.result.n;
    r.upsertedId = isArrayResultUpserted && r.result.upserted.length > 0 ? r.result.upserted[0] : null;
    r.upsertedCount = isArrayResultUpserted && r.result.upserted.length ? r.result.upserted.length : 0;
    r.matchedCount = isArrayResultUpserted && r.result.upserted.length > 0 ? 0 : r.result.n;
    if(callback) callback(null, r);
  });
}
