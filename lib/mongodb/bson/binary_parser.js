...
var p = exports.BinaryParser = function BinaryParser (bigEndian, allowExceptions) {
	this.bigEndian = bigEndian;
	this.allowExceptions = allowExceptions;
};

var Buffer = exports.BinaryParser.Buffer = function Buffer (bigEndian, buffer) {
  this.bigEndian = bigEndian || 0;
  this.buffer = [];
  this.setBuffer(buffer);
};

Buffer.prototype.setBuffer = function setBuffer (data) {
  var l, i, b;
...
Buffer.prototype.hasNeededBits = function hasNeededBits (neededBits) {
	return this.buffer.length >= -(-neededBits >> 3);
};

Buffer.prototype.checkBuffer = function checkBuffer (neededBits) {
	if (!this.hasNeededBits(neededBits)) {
		throw new Error("checkBuffer::missing bytes");
  }
};

Buffer.prototype.readBits = function readBits (start, length) {
...
p.decodeFloat = function decodeFloat (data, precisionBits, exponentBits) {
...
p.decodeInt = function decodeInt (data, bits, signed, forceBigEndian) {
...
p.encodeFloat = function encodeFloat (data, precisionBits, exponentBits) {
	var bias = maxBits[exponentBits - 1] - 1
    , minExp = -bias + 1
    , maxExp = bias
...
p.encodeInt = function encodeInt (data, bits, signed, forceBigEndian) {
...
p.encode_int32 = function encode_int32 (number) {
...
p.encode_int64 = function encode_int64 (number) {
...
p.decode_utf8 = function decode_utf8 (binaryStr) {
...
// Encode a cstring
p.encode_cstring = function encode_cstring (s) {
  return unescape(encodeURIComponent(s)) + p.fromByte(0);
};

// Take a utf8 string and return a binary string
p.encode_utf8 = function encode_utf8 (s) {
  var a = ""
    , c;
