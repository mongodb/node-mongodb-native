    ...
    while(keys.length > 0) {
      ...
      if(Array.isArray(value)) {
        ...
        if(value >= BSON.BSON_INT32_MAX || value < BSON.BSON_INT32_MIN) {      
          ...
          // debug("---------------- :: " + name + " = " + (typeof value) + " = " + (totalLength - old));
          // totalLength += 8;
        } else {                
          // debug("--------------------------------------------------------- int")
        ...
        // debug("--------------------------------------------------------- string")
        // debug("---------------- :: " + name + " = " + (typeof value) + " = " + (name.length + 1 + value.length + 1))
        // debug(name.length + 1 + value.length + 1)
...
// exports.encodeCString = function(buffer, index, string) {
//   buffer.we
...
BSON.encodeObjectNoRec = function(buffer, object, checkKeys) {
  ...
  while(!done) {
    ...
    // continue
    if(stack.length == 0) {
      if(currentObjectStored != null) {
        // Calculate object final size
        size = currentObjectStored.endIndex - currentObjectStored.index;            
...
BSON.serialize = function serialize(object, checkKeys, asBuffer) {
  ...
  if(object instanceof Object) {
    ...
    //   debug("[" + i + "] :: " + buffer.toString('ascii', i, i+1) + " :: [" + buffer[i] + "]" + " = [" + finalBuffer[i] + "]")      
