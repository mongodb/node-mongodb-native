...
BSON.serialize = function serialize(object, checkKeys, asBuffer) {
  ...
  // var buffers = BSON.encodeValue([], null, data, true, checkKeys == null ? false : checkKeys)
  // debug(inspect(buffers))
  // return BSON.encodeValue('', null, object, true, checkKeys == null ? false : checkKeys);

  // // debug("=================== r :: " + r.length)
  // 
  // // Store all binary buffers used for serialization
  var buffers = [];
  // 
...
BSON._encodeFloat = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length : 0) + 8;  
}
...
BSON._encodeInt32 = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length : 0) + 4;
}
...
BSON._encodeBoolean = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length : 0) + 1;  
}
...
BSON._encodeString = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length : 0) + valueBuffers[0].length + 1 + 4;
}
...
BSON._encodeArray = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length : 0) + arrayTotalLength + 4 + 1;
}
...
BSON._encodeObject = function(buffers, name, object) {
  ...
  if(name != null) {
    ...
    // Build up the object
    buffers.push(BSON._encodeInt(objectLength + 4 + 1 + 1));    
  }
