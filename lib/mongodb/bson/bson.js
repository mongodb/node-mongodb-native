  ...
  while(!done) {
    ...
    while(keys.length > 0) {
      ...
      if(value == null) {
        ...
        if(name != null) {
          ...
          // Encode the name
          index =  index + unescape(encodeURIComponent(name)).length + 1;
          buffer[index - 1] = 0;          
        }        
      } else if(typeof value == 'number' && value === parseInt(value, 10)) {
        // Write the type
        ...
        // Calculate size
        size = unescape(encodeURIComponent(codeString)).length + 1;
        ...
        // Update index
        index =  index + unescape(encodeURIComponent(codeString)).length;
        // Set terminating 0
        buffer[index++] = 0;
      ...
      if(keys.length == 0 && stack.length <= 2) {
        // // For Code Objects
        // if(codeStartIndex > 0) {
        //   // Calculate code object size
        //   size = index - codeStartIndex + 1;                            
        //   // Write the size of the string to buffer
    ...
    if(stack.length == 0) {
      ...
      if(currentObjectStored != null && currentObjectStored.index > 0) {
        size = index - currentObjectStored.index;
      ...
      // if(currentObject instanceof Code) {
      //   debug("=============== it's a code object")
      // }
