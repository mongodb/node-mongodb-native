...
BSON.prototype.deserialize = function(data, is_array_item) {
  ...
  while(index < data.length) {
    ...
    if(type == BSON.BSON_DATA_STRING) {
      ...
      var bin_data = data.substr(index, number_of_bytes - 1);
...
BSON.prototype.encodeCode = function(code, checkKeys) {  
  ...
  var scope = this.parser.fromInt(5) + this.parser.fromByte(0);
...
BSON.prototype.encodeBinary = function(binary) {
  var data = binary.value() + this.parser.fromByte(0);
...
BSON.prototype.encodeValue = function(encoded_string, variable, value, top_level, checkKeys) {
  ...
  if(value == null) {
    ...
    encoded_string += this.parser.fromByte(BSON.BSON_DATA_CODE_W_SCOPE) + variable_encoded + this.encodeCode(value, checkKeys);
...
Binary.prototype.write = function(string) {
  for(var i = 0; i < string.length; i++) {
    this.value_array.push(this.parser.fromByte(string.charCodeAt(i)));
  }
}
Binary.prototype.read = function(position, length) {
  return this.value_array.splice(position == 0 ? position : position - 1, length);
}
