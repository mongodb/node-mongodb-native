...
BSON.deserialize = function(data) {
  return BSON.deserialize(data, false);
};
...
BSON.encodeValue = function(encoded_string, variable, value, top_level, checkKeys) {
  ...
  if(value == null) {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_NULL) + variable_encoded;
  } else if(value.constructor == String) {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_STRING) + variable_encoded + BSON.encodeString(value);
  } else if(value.className == "Long") {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_LONG) + variable_encoded + BSON.encodeLong(value);
    ...
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_NUMBER) + variable_encoded + BSON.encodeFloat(value);
  } else if(value.constructor == Array) {
    var object_string = BSON.encodeArray(value, checkKeys);
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_ARRAY) + variable_encoded + BSON.encodeInt(mongo.Integer.fromInt(object_string.length + 4 + 1)) + object_string + mongo.BinaryParser.fromByte(0);
  } else if(value.constructor == Boolean) {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_BOOLEAN) + variable_encoded + BSON.encodeBoolean(value);
  } else if(value.constructor == Date) {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_DATE) + variable_encoded + BSON.encodeDate(value);
  } else if(value.constructor == RegExp) {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_REGEXP) + variable_encoded + BSON.encodeRegExp(value);
  } else if(value.className == "ObjectID") {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_OID) + variable_encoded + BSON.encodeOid(value);
  } else if(value.className == "Code") {
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_CODE_W_SCOPE) + variable_encoded + BSON.encodeCode(value, checkKeys);
  } else if(value.className == "OrderedHash") {
    var object_string = BSON.encodeOrderedObject(value, checkKeys);
    encoded_string += (!top_level ? mongo.BinaryParser.fromByte(BSON.BSON_DATA_OBJECT) : '') + variable_encoded + BSON.encodeInt(mongo.Integer.fromInt(object_string.length + 4 + 1)) + object_string + mongo.BinaryParser.fromByte(0);
  } else if(value.className == "Binary") {
    var object_string = BSON.encodeBinary(value);
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_BINARY) + variable_encoded + BSON.encodeInt(mongo.Integer.fromInt(object_string.length - 1)) + object_string;
  } else if(value.className == "DBRef") {
    var object_string = BSON.encodeDBRef(value);
    encoded_string += mongo.BinaryParser.fromByte(BSON.BSON_DATA_OBJECT) + variable_encoded + BSON.encodeInt(mongo.Integer.fromInt(object_string.length + 4 + 1)) + object_string + mongo.BinaryParser.fromByte(0);
...
ObjectId.createFromHexString= function(hexString) {
    ...
    for(var index=0 ; index < hexString.length; index+=2) {
        ...
        result+= mongo.BinaryParser.fromByte(number);
    }
