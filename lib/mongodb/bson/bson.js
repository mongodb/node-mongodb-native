    ...
    while(!done) {
      ...
      while(keys.length > 0) {
        ...
        if(value == null) {
          ...
          totalLength += (name != null ? (Buffer.byteLength(name) + 1) : 0) + (Buffer.byteLength(value.code, 'utf8') + 4 + 1 + 1);
        } else if(value instanceof Code) {
          // Calculate the length of the code string
          totalLength += (name != null ? (Buffer.byteLength(name) + 1) : 0) + 4 + (Buffer.byteLength(value.code, 'utf8') + 1) + 4;
          totalLength += (4 + 1 + 1);
          // Push the current object
          stack.push({keys:keys, object:currentObject});
          currentObject = value.scope;
    ...
    while(!done) {
      ...
      while(currentObjectStored.keys.length > 0) {
        ...
        if(value == null) {
          ...
          // Calculate size
          size = Buffer.byteLength(value.code) + 1;
          // Write the size of the string to buffer
          buffer[index + 3] = (size >> 24) & 0xff;     
          buffer[index + 2] = (size >> 16) & 0xff;
          buffer[index + 1] = (size >> 8) & 0xff;
          ...
          index = index + 4;
          // Write the string
          buffer.write(value.code, index, 'utf8');
          // Update index
          index = index + size - 1;
          // Write zero
          buffer[index++] = 0;          
...
BSON.deserialize = function(data) {
  ...
  while(index < dataLength) {
    ...
    if(type === BSON.BSON_DATA_STRING) {
      ...
      index = index + 4;
      // Read the string
      value = new Symbol(data.toString('utf8', index, index + string_size - 1));
      // Adjust the index with the size of the string
      index = index + string_size;
