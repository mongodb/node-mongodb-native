...
BSON.deserialize = function(data) {
  ...
  while(index < data.length) {
    ...
    // debug("========================================= index")
    // debug("type = " + type)
    // debug("object_end_index = " + object_end_index)
    // debug("index = " + index)
    ...
    if(object_end_index != 0 && index > object_end_index) {
      ...
      // Keep popping until we get to a proper object that's is larger than the current index
      // while(currentObjectInstance != null && currentObjectInstance.index < index) {
      //   currentObjectInstance = stack.pop();        
      // }
      
      // Let's set the parent object as the current Object      
      if(currentObjectInstance != null) {        
        // debug("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        // debug(currentObjectInstance.index)
    ...
    if(type == BSON.BSON_DATA_STRING) {
      ...
      // debug("========================================= object_name :: " + object_name)
      // debug(Array.isArray(currentObject))
      // debug("object_end_index = " + object_end_index)
      // debug("index = " + index)
