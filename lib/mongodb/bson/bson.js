...
BSON.serialize = function serialize(object, checkKeys, asBuffer) {
  ...
  if(object instanceof Object) {
    var totalLength = BSON._encodeObject(buffers, null, object);
...
BSON._encodeNull = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0);    
}
...
BSON._encodeFloat = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0) + 8;  
}
...
BSON._encodeInt32 = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0) + 4;
}
...
BSON._encodeBoolean = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0) + 1;  
}
...
BSON._encodeString = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0) + valueBuffers[0].length + 1 + 4;
}
...
BSON._encodeArray = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0) + arrayTotalLength + 4 + 1;
}

BSON._encodeValue = function(buffers, elementName, value) {
  var objectLength = 0;
...
BSON._encodeCode = function(buffers, name, value) {
  ...
  if(null != value.scope) {
    totalObjectLength += BSON._encodeObject(scopeBuffers, null, value.scope);
  } else {
    scopeBuffers.push(BSON._encodeInt(5), new Buffer([0]));
  }
  
  // Add the variables
  var objectTotalSize = 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0) + totalObjectLength + 4;

  // Add buffers to list
  buffers.push(BSON._encodeInt(objectTotalSize));
...
BSON._encodeBinary = function(buffers, name, value) {
  ...
  return 1 + (nameBuffers != null ? nameBuffers[0].length + 1: 0) + data.length + 1 + 4;    
}
...
BSON._encodeObject = function(buffers, name, object) {
  ...
  for(var elementName in object) {
    ...
    objectLength += BSON._encodeValue(objectBuffers, elementName, value);
  }
      
  // Push name if it's defined
  if(name != null) {
    // Encode name buffer
    var nameBuffers = BSON.encodeCString(name);    
    buffers.push(nameBuffers[0], nameBuffers[1]);
    ...
    totalObjectLength += nameBuffers[0].length + nameBuffers[1].length;
  } else {
    // Build up the object
    buffers.push(BSON._encodeInt(objectLength + 4 + 1));    
  }
  ...
  for(var i = 0; i < objectBuffers.length; i++) {
    ...
    totalObjectLength += objectBuffers[i].length; 
  }
  
  // Push terminating null
  buffers.push(new Buffer([0]));
  // Return length of object (to help allocate buffer)
  return totalObjectLength + 4 + 1;
}
