...
// Does not do recursion, uses a stack to handle depth
// Experiment for performance
BSON._calculateObjectSize = function(object) {
  var totalLength = (4 + 1);
  var done = false;
  ...
  while(!done) {
    ...
    } else if(finished || (keysIndex == (keyLength - 1))){      
...
// In place serialization with index to starting point of serialization
BSON._serializeWithBufferAndIndex = function serializeWithBufferAndIndex(object, checkKeys, buffer, startIndex) {
  if(null != object && 'object' === typeof object) {
    // Encode the object using single allocated buffer and no recursion
    var index = startIndex == null ? 0 : startIndex;
    var done = false;
    ...
    while(!done) {
      ...
      // debug("=================================================")
      // debug(inspect(keysIndex))
      // debug(inspect(keyLength))
      ...
      while(keysIndex < keyLength) {
        ...
        // debug(keysIndex)
        // debug(keyLength)
        // debug(currentObjectStored.keys)
      ...
      // while(currentObjectStored.keys.length > 0) {
        ...
        // if(currentObjectStored.keys.length == 0) {
          ...
          // if(stack.length > 0) {  
            ...
            // debug("============================================ popped object :: " + stackIndex)
...
BSON.serialize = function(object, checkKeys, asBuffer) {
  var buffer = new Buffer(BSON.calculateObjectSize(object));
  BSON._serializeWithBufferAndIndex(object, checkKeys, buffer, 0);
  return buffer;
}
