...
BSON.prototype.encodeOrderedObject = function(object) {  
  ...
  if(object.get('_id') != null) encoded_string += this.encodeValue('', '_id', object.get('_id'), false);
  // Encode all other objects in the order provided
  for(var index = 0; index < object.keys().length; index++) {
    var key = object.keys()[index];
    if(key != '_id') encoded_string += this.encodeValue('', key, object.get(key), false);
  }
  return encoded_string;
}
...
BSON.prototype.encodeCode = function(code) {  
  ...
  if(code.scope != null) {
    scope = this.encodeValue('', null, code.scope, false);
