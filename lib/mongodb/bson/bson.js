 ...
 * @return {TODO}
 */
var calculateObjectSize2 = BSON.calculateObjectSize2 = function(object, serializeFunctions) {
  var totalLength = (4 + 1);
    ...
    case 'object':   
      if(value instanceof ObjectID) {
        ...
        if(value.scope !== null && Object.keys(value.scope).length > 0) {
          return (name != null ? (Buffer.byteLength(name) + 1) : 0) + 1 + 4 + 4 + Buffer.byteLength(value.code.toString(), 'utf8') + 1 + calculateObjectSize2(value.scope);
        } else {
        ...
        return (name != null ? (Buffer.byteLength(name) + 1) : 0) + 1 + calculateObjectSize2(ordered_values, serializeFunctions);
      } else if(value instanceof RegExp) {
          return (name != null ? (Buffer.byteLength(name) + 1) : 0) + 1 + Buffer.byteLength(value.source, 'utf8') + 1
            + (value.global ? 1 : 0) + (value.ignoreCase ? 1 : 0) + (value.multiline ? 1 : 0) + 1        
      } else {
        return (name != null ? (Buffer.byteLength(name) + 1) : 0) + calculateObjectSize2(value, serializeFunctions) + 1;        
      }
    case 'function':
      if(value.scope != null && Object.keys(value.scope).length > 0) {
        return (name != null ? (Buffer.byteLength(name) + 1) : 0) + 1 + 4 + 4 + Buffer.byteLength(value.toString(), 'utf8') + 1 + calculateObjectSize2(value.scope);
      } else {
...
var serializeWithBufferAndIndex2 = BSON.serializeWithBufferAndIndex2 = function serializeWithBufferAndIndex(object, checkKeys, buffer, index, serializeFunctions) {
  ...
  return serializeObject(object, checkKeys, buffer, index, serializeFunctions);
}
  ...
  switch(typeof value) {
    ...
    case 'object':   
      if(value instanceof ObjectID) {
        ...
        // Message size
        var size = calculateObjectSize2(ordered_values);
        // Serialize the object
        var endIndex = serializeWithBufferAndIndex2(ordered_values, checkKeys, buffer, index, serializeFunctions);
        // Write the size of the string to buffer
        buffer[index++] = size & 0xff;
        buffer[index++] = (size >> 8) & 0xff;
        buffer[index++] = (size >> 16) & 0xff;
    ...
    while(!done) {
      ...
      while(keysIndex < keyLength) {
        ...
        if(value == null) {
          ...
          // Calculate the scope size
          var scopeSize = BSON.calculateObjectSize2(value.scope);
          // Write the type
          buffer[index++] = BSON.BSON_DATA_CODE_W_SCOPE;
          // Write the name
          if(name != null) {
...
BSON.serialize = function(object, checkKeys, asBuffer, serializeFunctions) {
  var buffer = new Buffer(BSON.calculateObjectSize2(object, serializeFunctions));
  BSON.serializeWithBufferAndIndex(object, checkKeys, buffer, 0, serializeFunctions);
  return buffer;
}

BSON.serialize2 = function(object, checkKeys, asBuffer, serializeFunctions) {
  var buffer = new Buffer(BSON.calculateObjectSize2(object, serializeFunctions));
  BSON.serializeWithBufferAndIndex2(object, checkKeys, buffer, 0, serializeFunctions);
  return buffer;
}
...
BSON.deserialize = function(data, options) {
  ...
  while(index < dataLength) {
    ...
    if(type === BSON.BSON_DATA_OBJECT || type === BSON.BSON_DATA_ARRAY) {
      ...
      while(data[index] !== 0 && data[index] != null) {
...
BSON.prototype.serialize2 = function(object, checkKeys, asBuffer, serializeFunctions) {
  return BSON.serialize2(object, checkKeys, asBuffer, serializeFunctions);
}

BSON.prototype.calculateObjectSize = function(object, serializeFunctions) {
  return BSON.calculateObjectSize2(object, serializeFunctions);
}
...
BSON.prototype.serializeWithBufferAndIndex2 = function(object, checkKeys, buffer, startIndex, serializeFunctions) {
  return BSON.serializeWithBufferAndIndex2(object, checkKeys, buffer, startIndex, serializeFunctions);
}

BSON.prototype.calculateObjectSize2 = function(object, serializeFunctions) {
  return BSON.calculateObjectSize2(object, serializeFunctions);
}
