if(typeof window === 'undefined') {
  ...
  Buffer.prototype.write = function(data, index, encoding) {    
    ...
    for(var i = 0; i < data.length; i++) {
      this[index + i] = typeof data[i] == 'string' ? data[i].charCodeAt(0) : data[i];
    }
  ...
  Buffer.prototype.toString = function(encoding, startIndex, endIndex) {
    var string = '';
    for(var i = startIndex; i < endIndex; i++) {
      string = string + String.fromCharCode(this[i]);
    }  
    return string;
  }
 ...
 * @api private
 */
var convertUint8ArrayToString = function(bytes, startIndex, endIndex) {
  var result = "";
  for(var i = startIndex; i < endIndex; i++) {
...
BSON.deserialize = function(buffer, options, isArray) {  
  ...
  var readCStyleString = function() {
    ...
    var string = buffer instanceof Uint8Array ? convertUint8ArrayToString(buffer, index, i) : buffer.toString('utf8', index, i);
  ...
  while(true) {
    ...
    switch(elementType) {
      case BSON.BSON_DATA_OID:
        var string = buffer instanceof Uint8Array ? convertUint8ArrayToString(buffer, index, index + 12) : buffer.toString('binary', index, index + 12);
        // Decode the oid
      ...
      case BSON.BSON_DATA_STRING:
        ...
        object[name] = buffer instanceof Uint8Array ? convertUint8ArrayToString(buffer, index, index + stringSize - 1) : buffer.toString('utf8', index, index + stringSize - 1);
      ...
      case BSON.BSON_DATA_CODE:
        ...
        var stringSize = buffer[index++] | buffer[index++] << 8 | buffer[index++] << 16 | buffer[index++] << 24;
        // Function string
        var functionString = buffer instanceof Uint8Array ? convertUint8ArrayToString(buffer, index, index + stringSize - 1) : buffer.toString('utf8', index, index + stringSize - 1);
