...
//    console.log( 'got data ' + self.port );
...
/*	var new_connection  = new Connection*/
	...
	new_connection.addListener( "error", function( err ) {
	  if ( command.db.listeners(command.getRequestId().toString()).length ){
	    command.db.removeAllListeners(command.getRequestId().toString());
            console.log( 'remove listener for ' + command.getRequestId().toString().length );
          }
	    console.log( "Error da dam da dam");
            //self.emit( "error", err ); 
            self.readyState = "closed";
...
var ReplSetServers = exports.ReplSetServers = function(servers) {
  ...
  this.__defineGetter__("masterConnection", function() {
	...
	//console.log( server.host + ':' + server.port + ' ' + ( server.master ? ' is':' is not') + ' master');
        if(server.master == true && ( server.connection.getReadyState() == "open") ) finalServer = server;
      });
      return finalServer != null ? finalServer.masterConnection : finalServer;
    }
  });
