...
var setupConnectionPool = function(self, poolSize) {
  ...
  var receiveListener = function(result) {    
    ...
    // debug("------------------------------------------------------ con object :: " + inspect(this))
    // debug(inspect(conObj))
...
Connection.prototype.close = function() {
  if(this.connection != null) this.connection.end();
};
...
Connection.prototype.send = function(command) { 
  ...
  try {
    ...
    if(command.constructor == String) {
      connection.write(command, "binary");      
    } else {
    ...
    // debug("============================= err :: " + err)
    // Check if the connection is closed
    if(connection.readyState != "open" && this.autoReconnect) {
    //   // Add the message to the queue of messages to send
    //   this.messages.push(command);
    //   // Initiate reconnect if no current running
    //   if(connection.currently_reconnecting == null) {
    //     connection.currently_reconnecting = true;
    //     // Create the associated connection
