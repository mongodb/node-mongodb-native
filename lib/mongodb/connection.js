...
Connection.prototype.getReadyState = function () {
...
Connection.prototype.send = function(command) {
  ...
  try {
    ...
    // Check if the connection is closed
    console.log( this.connection.readyState + ' '+  this.autoReconnect);
    if(this.connection.readyState != "open" && this.autoReconnect) {
      console.log( 'Try to reconnect to ' + this.host + ':' + this.port);
      ...
      if(this.connection.currently_reconnecting == null) {
        console.log( 'Create connection .... right now!' );
	    ...
	    new_connection.addListener( "error", function( err ) {
	      ...
	      while( self.messages.length > 0 ) { 
                 var aCommand = self.messages.shift();
		    // send error to all initiatiors
	        if ( aCommand.db.listeners(aCommand.getRequestId().toString()).length ) {
                   aCommand.db.emit( aCommand.getRequestId().toString(), 'Error', null );
	     /*     aCommand.db.removeAllListeners(aCommand.getRequestId().toString());
              console.log( 'remove listener for ' + aCommand.getRequestId().toString().length + ' ' + (++i )); */
            } 
          }	
        ...
        new_connection.addListener("data", self.receiveListener);
        // Add connnect listener
        new_connection.addListener("connect", function() {
/*          console.log( 'Reconnects!!!' );
          console.log( 'State: ' + this.readyState );
          console.log( 'no. of messages: ' + self.messages.length );
*/
          this.setEncoding("binary");
          this.setTimeout(0);
          this.setNoDelay();
        ...
        console.log( 'Allready trying to reconnect.Just add message in queue' );  
...
Connection.prototype.sendwithoutReconnect = function(command) {
  ...
  // no need to reconnect since called by latest master  <-- this is bullshit
...
var ReplSetServers = exports.ReplSetServers = function(servers) {
  ...
  this.__defineGetter__("masterConnection", function() {
    ...
    if(this.target != null && this.target instanceof Server) {
      ...
      var finalServer = null;
      this.servers.forEach(function(server) {
	//console.log( server.host + ':' + server.port + ' ' + ( server.master ? ' is':' is not') + ' master');
