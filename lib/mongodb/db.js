...
var Db = exports.Db = function(databaseName, serverConfig, options) {
  ...
  this.numberOfRetries = this.options.numberOfRetries != null ? this.options.numberOfRetries : 3;
...
Db.prototype.authenticate = function(username, password, callback) {
  ...
  // Allow one auth pr connection only
...
Db.prototype.createCollection = function(collectionName, options, callback) {
  ...
  this.collectionNames(collectionName, function(err, collections) {    
    ...
    // console.log("======================= Db.prototype.createCollection :: 0")
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  if(callback instanceof Function && !onAll) {
    ...
    console.log("-------------------------------------------- to :: 2 :: " + (connection != null))
    ...
    self.serverConfig.connect(self, {firstCall: false}, function(err, result) {
      ...
      if(err != null && numberOfRetriesDone <= 0) {
        ...
        if(Array.isArray(self.auths) && self.auths.length > 0) {
          // console.log("---------------------------------------------- __retryCommandOnFailure :: -1")
          // Get number of auths we need to execute
          var numberOfAuths = self.auths.length;
          // Apply all auths
          for(var i = 0; i < self.auths.length; i++) {
            // console.log("---------------------------------------------- __retryCommandOnFailure :: 0")
            // console.log("=============== username :: " + self.auths[i].username)
            // console.log("=============== password :: " + self.auths[i].password)
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  console.log(this.serverConfig.isConnected())
  console.log(this.serverConfig.autoReconnect)
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {  
  ...
  // console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Db.prototype._executeInsertCommand")
...
// }
// 
// Db.prototype.emit = function(event, err, object) {
//   // console.log("===================================================== Db emit :: " + event)
//   // console.dir(err)
//   // console.log(err.stack)
//   // console.log("===================================================================== db emit :: " + event)
//   // console.dir(this.eventHandlers[event])
//   
...
// }
// 
// Db.prototype.removeListeners = function(event) {
//   // console.log("===================================================== Db removeListeners :: " + event)
//   if(this.eventHandlers[event] == null) throw "Event handler only accepts values of " + Object.keys(this.eventHandlers);
//   // Throw away all handlers
//   this.eventHandlers[event] = [];
// }
// 
// Db.prototype.removeAllListeners = function() {
