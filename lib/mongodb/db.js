...
Db.prototype.authenticate = function(username, password, callback) {
  ...
  for(var i = 0; i < numberOfExpectedReturns; i++) {
    ...
    var createNonceCallback = function(index) {
      return function(err, reply) {
        if(err == null) {
          ...
          // Execute command
          self._executeCommand(DbCommand.createAuthenticationCommand(self, username, password, nonce), {writer: rawConnections[index].connection}, function(err, result) {
            // Ajust the number of expected results
            numberOfExpectedReturns = numberOfExpectedReturns - 1;
            // If we are done let's evaluate
            if(numberOfExpectedReturns <= 0) {
    ...
    this._executeCommand(DbCommand.createGetNonceCommand(self), {writer: rawConnections[i].connection}, createNonceCallback(i));    
  }
};
...
Db.prototype.executeDbCommand = function(command_hash, options, callback) {
  ...
  this._executeCommand(DbCommand.createDbCommand(this, command_hash), options, callback);
};
...
Db.prototype.executeDbAdminCommand = function(command_hash, callback) {
  this._executeCommand(DbCommand.createAdminDbCommand(this, command_hash), callback);
};
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {
  ...
  if(callback instanceof Function) {
    ...
    if(safe != null && safe != false) {
      ...
      // Add a entry of non-replied to messages
      this.notReplied[db_command[1].getRequestId().toString()] = {start: new Date().getTime(), 'raw': raw};      
    }
  }
