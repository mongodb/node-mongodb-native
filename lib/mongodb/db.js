...
Db = function(databaseName, server, options) {  
...
Db.prototype.open = function() {
  ...
  var receiveCallback = function(messages) {
  ...
  if(self.server instanceof Server) {
    self.server.connection = new Connection(self.server.host, self.server.port, self.server.autoReconnect);    
    self.server.connection.addListener("connect", function() {
      ...
      var callback = function(reply) {
        ...
        if(reply[0].documents[0].get('ismaster') == 1) {
          self.master_connection = self.server.connection;
    ...
    self.server.connection.addListener("receive", receiveCallback);
    // Open the connection
    self.server.connection.open();
  } else if(server instanceof ServerPair) {
    
  } else if(server instanceof ServerCluster) {
...
Db.prototype.executeCommand = function(db_command, callback) {
  // Add the callback to the list of callbacks by the request id (mapping outgoing messages to correct callbacks)
  if(callback != null) this.callbacks[db_command.getRequestId()] = callback;  
  // Execute command
