...
var Db = exports.Db = function(databaseName, serverConfig, options) {
  ...
  console.log("DB Options = "+inspect(options));
...
Db.prototype.executeCommand = function(db_command, options, callback) {
    ...
    if(safe == true) {
      errorCommand = DbCommand.createGetLastErrorCommand(options, this);
    }

    // If we have a callback execute
    if(callback instanceof Function) {

      var findAndModifyReply = null;
      var listenToCommand = db_command.query && db_command.query.findandmodify ? db_command : errorCommand != null ? errorCommand : db_command;

      // Add the callback to the list of callbacks by the request id (mapping outgoing messages to correct callbacks)
      this.on(listenToCommand.getRequestId().toString(), function(err, reply) {
          if (err) return callback(err, reply);
          
          // Ensure that findAndModify works properly by caching the return value and waiting for the getLastError call if necessary
          if (db_command.query && db_command.query.findandmodify) {
//            console.log("executeCommand: findandmodify callback: reply = "+inspect(reply));
  //          console.log("executeCommand: findandmodify callback: errorCommand = "+inspect(errorCommand));
            if (!errorCommand) return callback(err, reply);
            // Save the result of the findAndModify and wait for the errorCommand
            return findAndModifyReply = reply;
          } else {
      ...
      if (db_command.query && db_command.query.findandmodify && errorCommand) {
        this.on(errorCommand.getRequestId().toString(), function(err, errReply) {
          if (!findAndModifyReply) return; // There was an error in the original findAndModify and the callback already occurred
//          console.log("executeCommand: errorCommand for findAndModify: err = "+err+" errReply = "+inspect(errReply));
          if (err) return callback(err, errReply);
          return callback(null, findAndModifyReply);
        });

        if (self.serverConfig.primary) {
          this.notReplied[errorCommand.getRequestId().toString()] = self.serverConfig.primary;
        }
      }
