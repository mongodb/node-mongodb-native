...
Db.connect = function(url, options, callback) {
  ...
  if(options.db) {
    for(var name in options.db) object.db_options[name] = options.db[name];
  }

  // Merge in any options for server in options object
  if(options.server) {
    for(var name in options.server) object.server_options[name] = options.server[name];
  }
  
  // Check if we have a single server connection
