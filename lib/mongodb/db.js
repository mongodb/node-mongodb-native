...
Db.prototype.createIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  createIndexUsingCreateIndexes.call(self, collectionName, fieldOrSpec, options, function(err, result) {
    if(err == null) {
      return callback(err, result);
    }
    
    // Create command
    var command = DbCommand.createCreateIndexCommand(self, collectionName, fieldOrSpec, options);
...
var createIndexUsingCreateIndexes = function(collectionName, fieldOrSpec, options, callback) {
  // console.log("+++++++++++++++++++++++++++++++++++++++++ createIndexes 0")
  // console.dir(collectionName)
  // console.dir(fieldOrSpec)
  // console.dir(options)

  // Build the index
  var indexParameters = utils.parseIndexOptions(fieldOrSpec);
  // console.log("+++++++++++++++++++++++++++++++++++++++++ createIndexes 1")
  // console.dir(indexParameters)
  ...
  for(var name in options) {
    if(name == 'w' || name == 'wtimeout' || name == 'j' || name == 'fsync') {
  ...
  // Build the command
  this.executeDbCommand(command, function(err, result) {
    // console.log("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    // console.dir(err)
    // console.dir(result)
    if(err) return callback(err, null);
    if(result.documents[0].ok == 0) {
      return callback(utils.toError(result.documents[0]), null);
    }
