...
Db.prototype.indexInformation = function(callback, collectionName) {
  ...
  var selector = {'ns':(this.databaseName + "." + collectionName)};
  var info = {};
  ...
  new Cursor(this, new Collection(this, DbCommand.SYSTEM_INDEX_COLLECTION), selector).each(function(index) {
    // sys.puts(sys.inspect(index));
    // sys.puts("__________________ key: " + sys.inspect());
    ...
    if(index == null) {
      ...
      for(var name in index.key.toArray()) {
        info[index.name] = [name, index.key[name]];
      }      
    }
  }); 
}
