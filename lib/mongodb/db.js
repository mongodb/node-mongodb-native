...
Db.prototype.open = function(callback) {
  ...
  if(self.serverConfig instanceof Server) {
  ...
  } else if(self.serverConfig instanceof ServerPair || self.serverConfig instanceof ServerCluster || self.serverConfig instanceof ReplSetServers ) {
    ...
    var initServer = function(server)  {
      ...
      server.connection.addListener("error", function(err) {
        if ( self.serverConfig instanceof ReplSetServers ) {
          //we only have on error, if the rest are ok there is no problem
...
Db.prototype.executeCommand = function(db_command, callback) {
    ...
    if(callback instanceof Function) {
      ...
      // if does not respond in 1s emit null data
      setTimeout( function() { 
          if ( self.listeners( db_command.getRequestId().toString()).length )   {
            //remove listener for this command
            self.removeListener(db_command.getRequestId().toString(), self.listeners( db_command.getRequestId().toString())[0] );
            callback( new Error('Timed out' ), { documents:[ ]  } );
          }
        }, 1000);
    }
      ...
      if (err == null){
        try{ 
            if ( dbinstance.backup.server ) { // use slave this ONE time 
              ...
              dbinstance.backup.server.connection.send( db_command.toBinary());
              dbinstance.backup.server = null;
            } else {
              dbinstance.serverConfig.masterConnection.send(db_command.toBinary());          
            }
        } catch ( err ) {
            //console.log('this fails' +  err );
        }
        //db_command = null;
        //checkMasterHandler = null;
      } else {
        // XXX : LOOP!!!!!!
        //console.log( 'goes to LOOP(no connection available)' );
        setTimeout( self.checkMaster_(self, checkMasterHandler), 50 );
      }
    };
    
    try{
      self.serverConfig.masterConnection.send(msg);   
    } catch(err){
      self.checkMaster_(self, checkMasterHandler);
    }
    //db_command = null;
    //checkMasterHandler = null;
};
...
Db.prototype.checkMaster_ = function(dbcopy, returnback){
  ...
  dbcopy.serverConfig.servers.forEach(function(server) {
    ...
    if ( server.connection.connection.readyState == "open" || server.connection.autoReconnect ) {
      //console.log( 'checkMaster_ on ' + server.host + ':' + server.port );
        ...
        if(err != null) {
          if ( !hasReturned && ( ++answers == dbcopy.serverConfig.servers.length )) {
            if ( dbcopy.backup.server && dbcopy.backup.reply ) { 
              ...
              //console.log( 'PITFALL0 ' + err );
              return returnback( new Error( 'No master found' ) );
            }
          }
        } else {
          //console.log( reply.documents[0] );
          if(reply.documents[0].ismaster == 1) {
            // Locate the master connection and save it
      ...
      //console.log( 'server ' + server.host + ':' + server.port +' is ' + server.connection.connection.readyState);
