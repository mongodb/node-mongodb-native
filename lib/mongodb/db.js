...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  if(readFromPrimary && !config.canWrite() && config.isAutoReconnect()) {
  ...
  } else if(!config.canRead() && !config.canWrite() && !config.isAutoReconnect()) {
    ...
    self.serverConfig._commandsStore.execute_queries(self, __executeQueryCommand, __executeInsertCommand);
    // Execute the command
  ...
  if(!config.canWrite() && config.isAutoReconnect()) {
    ...
    processor(function() {
      self.serverConfig._commandsStore.write(
          {type:'insert', 'db_command':db_command, 'options':options, 'callback':callback}
        );
    })  
    ...
    // Execute any waiting commands
    self.serverConfig._commandsStore.execute_writes(self, __executeQueryCommand, __executeInsertCommand);
