...
Db.prototype._callHandler = function(id, document, err) {
  ...
  if(this._callBackStore.listeners(id).length >= 1) {
    ...
    //cleanup the extraneous references created by node's listeners()
...
Db.prototype._hasHandler = function(id) {
  var hasHandler = this._callBackStore.listeners(id).length >= 1;
  if (!hasHandler) {
    //cleanup the extraneous references created by node's listeners()
    delete this._callBackStore._events[id];
  }
...
Db.prototype._findHandler = function(id) {
  var info = this._callBackStore._notReplied[id];
  // Return the callback
  var cb = (this._callBackStore.listeners(id).length >= 1) ? this._callBackStore.listeners(id)[0] : null
  if (!cb) {
    //cleanup the extraneous references created by node's listeners()
    delete this._callBackStore._events[id];
  }
  return {info:info, callback: cb}
}
