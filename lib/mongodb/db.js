...
// Db.prototype.on = function(event, func) {
//   this.serverConfig.on(event, func);
// }

// Db.prototype.once = function(event, func) {
//   this.serverConfig.once(event, func);
// }
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  if(typeof callback === 'function' && !onAll) {
    ...
    // console.log("======================================= execute query :: " + (connection != null))
    ...
    // console.log("========================== perform read with :: " + read)
...
//         });
//       } else if(err == null && _self.serverConfig.isConnected() == true && _self.serverConfig.auth.length() > 0) {
//         _self._state = 'connected';
//         // Get number of auths we need to execute
//         var numberOfAuths = _self.serverConfig.auth.length();
//         // Apply all auths
//         for(var i = 0; i < _self.serverConfig.auth.length(); i++) {
//           var auth = _self.serverConfig.auth.get([i]);
//           // Authenticate
//           _self.authenticate(auth.username, auth.password
//             , {'authdb': auth.authdb, 'authMechanism': auth.authMechanism}, function(err, authenticated) {
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  // var readFromPrimary = options.read == null 
  //       || options.read == false
  //       || options.read == ReadPreference.PRIMARY
  //       || options.read == ReadPreference.PRIMARY_PREFERRED
  //       || options.read == ReadPreference.SECONDARY_PREFERRED

  // console.log("_executeQueryCommand SETTINGS")
  // console.dir("readFromPrimary :: " + readFromPrimary);
  // console.dir("config.canWrite() :: " + config.canWrite());
  // console.dir("config.canRead() :: " + config.canRead());
  // console.dir("config.isAutoReconnect() :: " + config.isAutoReconnect());

  // // Attempt to checkout a connection
  // if(read == ReadPreference.PRIMARY_PREFERRED
  //   || read)

  //
  // We have auto reconnect one and need to read from primary
  // if(readFromPrimary && !config.canWrite() && config.isAutoReconnect()) {
  //   console.log("================================================ _executeQueryCommand :: 0")
  //   // console.dir(Object.keys(this.serverConfig._state.addresses));
  //   // console.dir(Object.keys(this.serverConfig._state.secondaries));
  //   // console.dir(this.serverConfig._state.master != null)

  //   processor(function() {
  //     self.serverConfig._commandsStore.read_from_writer(
  //         {type: 'query', db_command: db_command, options: options, callback: callback }
  //       );
  //   });
  // } else if(!readFromPrimary && !config.canRead() && config.isAutoReconnect()) {
  ...
  // console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&")
  // console.log("config.canRead() :: " + config.canRead(read))
  // console.log("config.canWrite() :: " + config.canWrite())
  // console.log("config.isAutoReconnect() :: " + config.isAutoReconnect())
  ...
  if(!config.canRead(read) && !config.canWrite() && config.isAutoReconnect()) {
    ...
    // Execute any waiting commands
    // self.serverConfig._commandsStore.execute_queries();
    // Execute the command
...
var __executeInsertCommand = function(self, db_command, options, callback) {
  ...
  // console.log("__executeInsertCommand ========= connection checkout :: " + connection.socketOptions.host + ":" + connection.socketOptions.port)
  // Get safe mode
  ...
  // Override connection if needed
  connection = specifiedConnection != null ? specifiedConnection : connection;
  // console.log("__executeInsertCommand ========= connection after override check :: " + connection.socketOptions.host + ":" + connection.socketOptions.port)
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {
  ...
  if(!config.canWrite() && config.isAutoReconnect()) {
    ...
    // console.log("================================================ _executeInsertCommand :: 2 :: " + config._state.master.name + " :: " + config._state.master.port)

    // Execute any waiting commands
    // self.serverConfig._commandsStore.execute_writes();
