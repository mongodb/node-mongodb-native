...
Db.prototype.checkMaster_ = function(dbcopy, returnback){
  ...
  dbcopy.serverConfig.servers.forEach(function(server) {
        server.master = false;
	if ( server.connection.getReadyState() == "open" || server.connection.autoReconnect ) {
	  ...
	  dbcopy.addListener(db_cmnd.getRequestId().toString(), connect_Callback);
          console.log( 'server ' + server.host + ':' + server.port +' is ' + server.connection.getReadyState());
          if ( server.connection.readyState != "closed" ) {
	  ...
	  server.connection.addListener("close", function() { dbcopy.emit("close", this); });      
         ...
         server.master = false;
	 returnback( new Error(' Server down'), null, null );
   }
 });    
};
