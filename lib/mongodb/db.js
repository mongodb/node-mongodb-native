...
Db.prototype.open = function(callback) {
  ...
  if(self.serverConfig instanceof Server) {
    ...
    serverlist = self.serverConfig.servers;
    serconfig = self.serverConfig;
...
var connectCallback1 = function(err, reply) {
          if(err != null) {
          callback(err, null);
          } else {
            if(reply.documents[0].ismaster == 1) {
            ...
            // emit a message saying we got a master and are ready to go and change state to reflect it
            if(++numberOfConnectedServers == serverlist.length && dbcopy.state == 'notConnected') {
              dbcopy.state = 'connected';
              callback(null, null);
            }            
          }
        };

dbcopy.addListener(db_cmnd.getRequestId().toString(), connectCallback1);
server.connection.sendwithoutReconnect(db_cmnd);

server.connection.addListener("data", function(message) {
        // Parse the data as a reply object
        var reply1 = new MongoReply(dbcopy, message);
        // Emit error if there is one
        
        reply1.responseHasError ? dbcopy.emit(reply1.responseTo.toString(), reply1.documents[0], reply1, null) : dbcopy.emit(reply1.responseTo.toString(), null, reply1);
        // Remove the listener
    //    dbcopy.removeListener(reply.responseTo.toString(), dbcopy.listeners(reply.responseTo.toString())[0]);
