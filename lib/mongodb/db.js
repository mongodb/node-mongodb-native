...
exports.Db = Class({ 
  ...
  open: function(callback) {
    ...
    // Set up connections
    if(self.serverConfig instanceof mongo.Server) {
      ...
      self.serverConfig.connection.addListener("receive", function(messages) {          
        for(var index in messages) {  
          // Parse the data as a reply object
          var reply = new mongo.MongoReply(messages[index]);
      ...
      return;
    } else if(self.serverConfig instanceof mongo.ServerPair || self.serverConfig instanceof mongo.ServerCluster) {
  ...
  **/
  collections: function(callback) {
    ...
    self.collectionNames(function(documents) {
      var collections = [];
      documents.forEach(function(document) {
        collections.push(new mongo.Collection(self, document.get('name').replace(self.databaseName + ".", '')));        
      })
  ...
  **/
  eval: function(callback, code, parameters) {
    ...
    if(!(finalCode instanceof mongo.Code)) {
    ...
    new mongo.Cursor(this, new mongo.Collection(this, mongo.DbCommand.SYSTEM_COMMAND_COLLECTION), selector, {}, 0, -1).nextObject(function(result) {
      if(result.get('ok') == 1) {
        ...
        callback(new Error("eval failed: " + result.get('errmsg'))); return;
      }
    });
  },
  ...
  **/
  ...
  command: function(callback, selector) {
    if(!(selector instanceof mongo.OrderedHash)) {
      callback(new Error("command must be given an OrderedHash"));
    } else {
      var cursor = new mongo.Cursor(this, new mongo.Collection(this, mongo.DbCommand.SYSTEM_COMMAND_COLLECTION), selector, {}, 0, -1, null, null, null, null, QueryCommand.OPTS_NO_CURSOR_TIMEOUT);
