...
Db.prototype.logout = function(options, callback) {
  ...
  for(var index = 0; index < numberOfExpectedReturns; index++) {
    ...
    self._executeQueryCommand(logoutCommand, {writer: rawConnections[index].connection}, function(err, result) {      
    ...
    if(err == null) {
      ...
      self._executeQueryCommand(DbCommand.createAuthenticationCommand(self, username, password, nonce), {connection:connection}, function(err, result) {
        ...
        if(err) {
          ...
          errorObject = this.wrap(result.documents[0]);
        }
...
Db.prototype.createIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  this._executeQueryCommand(command, {read:false, safe:true}, function(err, result) {
    if(err != null) return callback(err, null);
...
Db.prototype.ensureIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  this.indexInformation(collectionName, function(err, collectionInfo) {
    ...
    if(!collectionInfo[index_name])  {
      self._executeQueryCommand(command, {read:false, safe:true}, function(err, result) {
        if(err != null) return callback(err, null);
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  if(callback instanceof Function && !onAll) {
    ...
    self.on(db_command.getRequestId().toString(), callback);
    // Add a entry of non-replied to messages
    self.notReplied[db_command.getRequestId().toString()] = {start: new Date().getTime(), 'raw': raw, 'connection':connection};
    ...
    connection.write(db_command, function(err) {
      if(err != null) {
        ...
        self.removeListener(db_command.getRequestId().toString(), callback);
        // Return error
        callback(err);
      }
    });    
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  // console.log("============= pool connected :: " + this.serverConfig.isConnected())  
  var self = this;
  ...
  if(!this.serverConfig.isConnected()) {
    __retryCommandOnFailure(this, 1000, 3, __executeQueryCommand, db_command, options, callback);
  } else {
...
var __executeInsertCommand = function(self, db_command, options, callback) {
  ...
  if(callback instanceof Function) {
    ...
    if(safe != null && safe != false) {
      ...
      self.on(db_command[1].getRequestId().toString(), callback);
      // Add a entry of non-replied to messages
      self.notReplied[db_command[1].getRequestId().toString()] = {start: new Date().getTime(), 'raw': raw, 'connection':connection};
    }
  }
  ...
  connection.write(db_command, function(err) {
    ...
    if(callback instanceof Function && (safe == null || safe == false)) {
      ...
      // Clean up listener
      delete self.notReplied[db_command[1].getRequestId().toString()];
      self.removeListener(db_command[1].getRequestId().toString(), callback);
      callback(err, null);
    } else {
      self.emit("error", err);
    }
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {
  ...
  // If the pool is not connected, attemp to reconnect to send the message
  if(!this.serverConfig.isConnected()) {
    __retryCommandOnFailure(this, 1000, 3, __executeInsertCommand, db_command, options, callback);
  } else {
