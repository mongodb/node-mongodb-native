...
var transformCollections = function(results) {
...
Db.prototype.listCollections = function(filter, options) {
  ...
  if(connection && connection.serverCapabilities.hasListCollectionsCommand) {
    ...
    return new CommandCursor(this, this.collection(DbCommand.SYSTEM_NAMESPACE_COLLECTION), command, {transform:transformCollections});
  }

  // We cannot use the listCollectionsCommand
  if(connection && !connection.serverCapabilities.hasListCollectionsCommand) {
    // If we have legacy mode and have not provided a full db name filter it
    if(typeof filter.name == 'string' && filter.name.indexOf(this.databaseName) == -1) {
  ...
  // Get the cursor
  var cursor = this.collection(DbCommand.SYSTEM_NAMESPACE_COLLECTION).find(filter);
...
Db.prototype.createCollection = function(collectionName, options, callback) {
  ...
  this.listCollections({name:collectionName}).toArray(function(err, collections) {
    ...
    self.command(cmd, options, function(err, result) {
      ...
      if(err && !err.code && err.errmsg) return callback(err, null);
