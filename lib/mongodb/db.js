...
Db.prototype.logout = function(options, callback) {
  ...
  for(var index = 0; index < numberOfExpectedReturns; index++) {
    ...
    self._executeCommand(logoutCommand, {writer: rawConnections[index].connection}, function(err, result) {      
...
Db.prototype.authenticate = function(username, password, callback) {
  ...
  this._executeQueryCommand(DbCommand.createGetNonceCommand(self), {onAll:true}, function(err, result, connection) {
    ...
    // console.dir(err)
    // console.log("------------------------------------------------- execute all commands __________ 1")
    // console.dir(result)
    // console.log("------------------------------------------------- execute all commands __________ 2")
    // console.dir(connection)
    
    if(err == null) {
      // console.log("------------------------------------------------- execute all commands :: 0 :: " + connection.id)
      // Nonce used to make authentication request with md5 hash
      var nonce = result.documents[0].nonce;
      // console.log("------------------------------------------------- execute all commands :: 1 :: " + nonce)
      ...
      self._executeQueryCommand(DbCommand.createAuthenticationCommand(self, username, password, nonce), {connection:connection}, function(err, result) {
        ...
        // 
        // 
        // console.log("------------------------------------------------- execute all commands :: 2")
        // console.dir(err)
        // console.dir(result)
      });
    } else {      
  ...
  // var rawConnections = self.serverConfig.allRawConnections();
  // // Get all the connections we have
  // var keys = Object.keys(rawConnections);
  // console.log("----------------------------------- authenticate :: -1")
  // // console.dir(rawConnections)
  // 
  // var numberOfExpectedReturns = keys.length;
  ...
  //         // Execute command
  //         self._executeQueryCommand(DbCommand.createAuthenticationCommand(self, username, password, nonce), {writer: rawConnections[index].connection}, function(err, result) {
  //           // Ajust the number of expected results
  //           numberOfExpectedReturns = numberOfExpectedReturns - 1;
  //           // If we are done let's evaluate
  //           if(numberOfExpectedReturns <= 0) {
  ...
  //   
  //   console.log("----------------------------------- authenticate :: 0")
  //   this._executeQueryCommand(DbCommand.createGetNonceCommand(self), {}, createNonceCallback(i));    
  // }
};
...
Db.prototype.dropCollection = function(collectionName, callback) {
  this._executeCommand(DbCommand.createDropCollectionCommand(this, collectionName), function(err, result) {
    if(err == null && result.documents[0].ok == 1) {
...
Db.prototype.renameCollection = function(fromCollection, toCollection, callback) {
  this._executeCommand(DbCommand.createRenameCollectionCommand(this, fromCollection, toCollection), function(err, doc) { callback(err, doc); });
};
...
Db.prototype.lastError = function(options, connectionOptions, callback) {
  ...
  this._executeCommand(DbCommand.createGetLastErrorCommand(options, this), connectionOptions, function(err, error) {
...
Db.prototype.lastStatus = function(callback) {
  this._executeCommand(DbCommand.createGetLastStatusCommand(this), callback);
};
...
Db.prototype.previousErrors = function(callback) {
  this._executeCommand(DbCommand.createGetPreviousErrorsCommand(this), function(err, error) {
...
Db.prototype.executeDbCommand = function(command_hash, options, callback) {
  ...
  this._executeQueryCommand(DbCommand.createDbCommand(this, command_hash), options, callback);
};
...
Db.prototype.resetErrorHistory = function(callback) {
  this._executeCommand(DbCommand.createResetErrorHistoryCommand(this), callback);
};
...
Db.prototype.createIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  this._executeCommand(command, {read:false, safe:true}, function(err, result) {
    if(err != null) return callback(err, null);
...
Db.prototype.ensureIndex = function(collectionName, fieldOrSpec, options, callback) {
  ...
  this.indexInformation(collectionName, function(err, collectionInfo) {
    ...
    if(!collectionInfo[index_name])  {
      self._executeCommand(command, {read:false, safe:true}, function(err, result) {
        if(err != null) return callback(err, null);
...
Db.prototype.cursorInfo = function(callback) {
  this._executeCommand(DbCommand.createDbCommand(this, {'cursorInfo':1}), function(err, result) {
...
Db.prototype.dropIndex = function(collectionName, indexName, callback) {
  this._executeCommand(DbCommand.createDropIndexCommand(this, collectionName, indexName), callback);
};
...
//   console.log("====================================================================================== _executeCommand 111111111111111")
//   // console.dir(db_command)
//   var self = this;
//   
//   var args = Array.prototype.slice.call(arguments, 1);
//   callback = args.pop();
...
Db.prototype._executeQueryCommand = function(db_command, options, callback) {
  ...
  if(callback instanceof Function && !onAll) {
    ...
    var connection = read ? this.serverConfig.checkoutReader() : this.serverConfig.checkoutWriter();
    // console.log(" --------------------------------------------------- 2")
    ...
    // Add a entry of non-replied to messages
    this.notReplied[db_command.getRequestId().toString()] = {start: new Date().getTime(), 'raw': raw, 'connection':connection};
    // Write the message out
    // console.log(" --------------------------------------------------- 3")
    connection.write(db_command);    
  } else if(callback instanceof Function && onAll) {
    var connections = this.serverConfig.allRawConnections();
...
Db.prototype._executeInsertCommand = function(db_command, options, callback) {
  // console.log("====================================================================================== _executeInsertCommand")
  ...
  if(callback instanceof Function) {
    ...
    if(safe != null && safe != false) {
      ...
      db_command = [db_command, DbCommand.createGetLastErrorCommand(safe, this)];
      // Register a listener on the last Error command
      this.on(db_command[1].getRequestId().toString(), callback);
      // Add a entry of non-replied to messages
      this.notReplied[db_command[1].getRequestId().toString()] = {start: new Date().getTime(), 'raw': raw, 'connection':connection};
    }
  }
...
//     this.on(db_command.getRequestId().toString(), callback);
//     // Add a entry of non-replied to messages
//     this.notReplied[db_command.getRequestId().toString()] = {start: new Date().getTime(), 'raw': raw};
//     // Write the message out
//     connection.write(db_command);    
//   } else {
//     
