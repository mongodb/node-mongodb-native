...
Db.prototype.open = function(callback) {
  ...
  if(self.serverConfig instanceof Server) {
    ...
    self.serverConfig.connection.addListener("data", function(message) {
      ...
      // Remove the listener
      var listenerFunction = self.listeners(reply.responseTo.toString())[0];
      if(typeof(listenerFunction)==="function") {
	      self.removeListener(reply.responseTo.toString(), listenerFunction);
	  }
    });
...
Db.prototype.enableSharding = function(dbName, callback) {
	this.executeCommand(DbCommand.createEnableShardingCommand(this, dbName), function(err, result) {
		callback(err, result);
	});
};

Db.prototype.shardCollection = function(dbName, collectionName, callback) {
	this.executeCommand(DbCommand.createShardCollectionCommand(this, dbName, collectionName), function(err, result) {
		callback(err, result);	
	});
};
