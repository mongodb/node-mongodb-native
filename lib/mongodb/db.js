...
var __retryCommandOnFailure = function(self, retryInMilliseconds, numberOfTimes, command, db_command, options, callback) {
  ...
  var retryFunction = function() {
    ...
    self.serverConfig.connect(self, {firstCall: false}, function(err, result) {
      console.log("_____________________________________________________________ __retryCommandOnFailure :: _" + self.serverConfig.isConnected())
      console.dir(err)
      
      // Definitively restart
      if(err != null && numberOfRetriesDone > 0) {
        console.log("_____________________________________________________________ __retryCommandOnFailure = 0")
        // Adjust the number of retries left
        numberOfRetriesDone = numberOfRetriesDone - 1;
        console.log("_____________________________________________________________ __retryCommandOnFailure = 0:1")
        // Force close the current connections
        self.serverConfig.close(function(err) {
          console.log("_____________________________________________________________ __retryCommandOnFailure = 0:2")
          // Retry the connect
          setTimeout(function() {
            console.log("_____________________________________________________________ __retryCommandOnFailure = 0:3")
            retryFunction();
          }, retryInMilliseconds);
        });
      } else if(err != null && numberOfRetriesDone <= 0) {
        console.log("_____________________________________________________________ __retryCommandOnFailure = 1")
        // Force close the current connections
        // self.serverConfig.close(function(err) {
          // Callback with error
          callback(err, null);
        // });
      } else if(err == null && self.serverConfig.isConnected() == true && Array.isArray(self.auths) && self.auths.length > 0) {
        console.log("_____________________________________________________________ __retryCommandOnFailure = 2")
      ...
      } else if(err == null && self.serverConfig.isConnected() == true) {
        console.log("_____________________________________________________________ __retryCommandOnFailure = 3")
        command(self, db_command, options, callback);
      } else {
        console.log("_____________________________________________________________ __retryCommandOnFailure = 0")
        // Adjust the number of retries left
  ...
  //     numberOfRetriesDone = numberOfRetriesDone - 1;
  //     // If we have run out of errors
  //     if(err != null && numberOfRetriesDone <= 0) {
  //       // Ensure all connections are close
  //       self.serverConfig.close();
  //       // No more retries finish up
  //       callback(err, null);          
  //     } else if(err != null && numberOfRetriesDone > 0) {
  //       console.log("======================================= FORCE CLOSE")
  //       // Close current connection set
  //       self.serverConfig.close();
  //       // Retry function
  //       setTimeout(retryFunction, retryInMilliseconds);
  //     } else if(err == null) {
