...
Db.prototype.executeCommand = function(db_command, options, callback) {
    ...
    // If we have a callback to execute
    if(callback instanceof Function) {

      var isFindAndModify = db_command.query && db_command.query.findandmodify;
      var findAndModifyReply = null;
      var listenToCommand = isFindAndModify ? db_command : errorCommand != null ? errorCommand : db_command;
      ...
      this.on(listenToCommand.getRequestId().toString(), function(err, reply) {
        ...
        if (isFindAndModify && errorCommand) {
      ...
      // If this is a findAndModify operation and it is waiting for getLastError we need to setup a second listener for
      // getLastError. 
      if (isFindAndModify && errorCommand) {
        this.on(errorCommand.getRequestId().toString(), function(err, errReply) {
          ...
          if (errReply.documents[0].err) return callback(this.wrap(errReply.documents[0]));
            ...
            if(err != null && retries > 0) {
              ...
              try {
                ...
                if(callback instanceof Function) {
                  ...
                  if (isFindAndModify) delete self.notReplied[listenToCommand.getRequestId().toString()];
                  return callback(err, null);
                }
              }
            }            
