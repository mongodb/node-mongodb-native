...
Db.prototype._callHandler = function(id, document, err) {
  // console.log("--------------------------------------- hello dog :: " + db_command.responseTo)
  // console.dir(this._callBackStore)
  
  // If there is a callback peform it
  // if(this._callBackStore[db_command.responseTo] != null) {
    // Get info object
    var info = this._callBackStore._notReplied[id];
    // Delete the current object
    delete this._callBackStore._notReplied[id];
    // console.log("---------------------------------------------------------------")
    // console.dir(this._callBackStore._notReplied)
    // console.dir(info)
...
// }
// 
// // Remove and return all handlers
// Db.prototype._removeAllHandlers = function() {
//   // Handle to current handlers
//   var currentHandlers = this._mongodbHandlers;
//   // Clean up all callbacks
//   this._mongodbHandlers = {_mongodbCallbacks : {}, _notReplied : {}};  
//   // Return the previous mongo handlers
...
//   return Object.keys(this._mongodbHandlers._notReplied).length;
// }
...
var __executeQueryCommand = function(self, db_command, options, callback) {
  ...
  if(callback instanceof Function && !onAll) {
    ...
    // self.on(db_command.getRequestId(), callback);
    // self._callBackStore.on(db_command.getRequestId(), callback);
...
var __retryCommandOnFailure = function(self, retryInMilliseconds, numberOfTimes, command, db_command, options, callback) {
  // console.log("------------------------------------------------------------- retryFunction :: " + retryInMilliseconds)
  ...
  var retryFunction = function(_self, _numberOfRetriesDone, _retryInMilliseconds, _numberOfTimes, _command, _db_command, _options, _callback) {
    // console.log("------------------------------------------------------------- retryFunction :: 0 :: " + _retryInMilliseconds)    
    
    _self.serverConfig.connect(_self, {}, function(err, result) {    
      // console.log("------------------------------------------ retryFunction :: " + _numberOfRetriesDone)
      // console.dir(err)
      ...
      if(err != null && _numberOfRetriesDone > 0) {
        // console.log("------------------------------------------ retryFunction :: " + _numberOfRetriesDone + " :: 0")
        // Force close the current connections
        _self.serverConfig.close(function(err) {
          // console.log("------------------------------------------ retryFunction :: " + _numberOfRetriesDone + " :: 1")
          // Retry the connect
          setTimeout(function() {
            // console.log("------------------------------------------ retryFunction :: " + _numberOfRetriesDone + " :: 2")
...
var __executeInsertCommand = function(self, db_command, options, callback) {
  ...
  if(callback instanceof Function) {
    ...
    // Ensure we have a valid connection
    if(connection == null) {
      // console.log("-----------------------------------------------__executeInsertCommand :: [no open connections]")
      return callback(new Error("no open connections"));
    }
    ...
    if(safe != null && safe != false) {
      ...
      // self.on(db_command[1].getRequestId(), callback);
      // self._callBackStore.on(db_command[1].getRequestId(), callback);
    }
  }
