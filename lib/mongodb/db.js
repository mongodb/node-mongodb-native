...
Db.prototype.open = function() {
  ...
  } else if(self.serverObject instanceof ServerPair) {
  //   var serverConnections = [self.serverObject.leftServer, self.serverObject.rightServer];
  //   var messages = [];
  //   
  ...
  //       var callback = function(reply) {
  //         sys.puts("--------------------------------------------------------------------------");
  //         sys.puts(sys.inspect(reply[0].documents[0]));
  //         sys.puts("--------------------------------------------------------------------------");
  //         // Locate the master connection and save it
  //         if(reply[0].documents[0].get('ismaster') == 1) {
  //           server.master = true;
  ...
  //       // self.callbacks[db_command.getRequestId()] = callback;
  //       //event
  //       //self.addListener("d", callback);
  //       sys.puts("---------------------- add event");
  //       emitter.addListener("d", callback);
  //       // sys.puts("============================= send message: " + db_command.getRequestId());
  //       // Let's send a request to identify the state of the server
  //       this.send(db_command);
  //     });      
  //     
  ...
  //       for(var index in messages) {  
  //         sys.puts("----------------- received message " + messages.length);
  //         // sys.puts("xxxxxxxxxxxxxxxxxxxxxxxxx RECEIVED MESSAGE");
  //         // new BinaryParser().pprint(messages[index]);          
  //         // Parse the data as a reply object
  //         var reply = new MongoReply(messages[index]);
  //         sys.puts("response to: " + reply.responseTo);
  //         emitter.emit("d", [reply]);
  ...
  //       }
  //     });
  //     // // Connect the server and return to caller if we are done connecting
  //     // server.connection.addListener("receive", function(message) {
  //     //   // sys.puts("----------------------------- message");
  //     //   // sys.puts(sys.inspect(message));
  //     //   // 
  //     //   // messages.push(message[0]);
  //     //   // 
  //     //   // if(messages.length == serverConnections.length) {
  //     //   //   receiveCallback(messages);
  //     //   // }
  //     // });
  //     // Open the connection
  //     server.connection.open();      
  //   });    
    // sys.puts("=================================================== server pair");
  } else if(self.serverObject instanceof ServerCluster) {
    sys.puts("=================================================== server cluster");    
  } else {
