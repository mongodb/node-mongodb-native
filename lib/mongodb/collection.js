...
exports.Collection = Class({  
  ...
  rename: function(collectionName, callback) {
    var self = this;
  ...
  **/
  distinct: function(key, query, callback) {
    ...
    var mapCommandHash = new mongo.OrderedHash();
    mapCommandHash.add('distinct', this.collectionName)
          .add('key', key).add('query', query);
          
    this.db.executeCommand(mongo.DbCommand.createDbCommand(this.db.databaseName, mapCommandHash), function(results) {
  ...
  count: function(query, callback) {
    ...
    this.db.executeCommand(queryCommand, function(result) {
      callback(null, result[0].documents[0].n);
    }); 
  },
  ...
  find: function(selector, options, callback) {
    ...
    // Create cursor
    callback(null, new mongo.Cursor(this.db, this, finalSelector, finalFields, skip, limit, sort, finalHint, explain, snapshot, timeout));
  },
  ...
  findOne: function(queryObject, callback) {
    ...
    finalQueryObject = finalQueryObject.className == "ObjectID" ? new mongo.OrderedHash().add('_id', finalQueryObject) : finalQueryObject;
    // Execute the command
    ...
    this.db.executeCommand(queryCommand, function(documents) {
      callback(null, documents[0].documents[0]);
    });
  },
  
  createIndex: function(fieldOrSpec, unique, callback) {
  ...
  group: function(keys, condition, initial, reduce, command, callback) {
    ...
    if(command) {
      ...
      var reduceFunction = reduce != null && reduce.className == "Code" ? reduce : new mongo.Code(reduce);
      var selector = {'group': {
      ...
      var scope = reduce != null && reduce.className == "Code" ? reduce.scope : new mongo.OrderedHash();    
      ...
      // Execute group
      this.db.eval(new mongo.Code(groupFunctionString, scope), function(err, results) {
