...
Collection.prototype.find = function find () {
  ...
  options.raw = options.raw != null && typeof options.raw === 'boolean' ? options.raw : false;
...
Collection.prototype.findOne = function findOne () {
 ...
 var fields;
 var queryObject = selector;
 options = options ? options : {};

 if(!(options.fields instanceof Buffer) && options.fields && Array.isArray(options.fields)) {
   fields = {};
 ...
 var query = queryObject instanceof Buffer ? queryObject : { 'query': queryObject };
 ...
 this.db._executeQueryCommand(queryCommand, {read:true, 'raw':raw}, function (err, result) {
