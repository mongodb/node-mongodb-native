...
Collection.prototype.insert = function(docs, checkKeys, callback) {
  var args = Array.prototype.slice.call(arguments, 1);
  callback = args.pop();
  checkKeys = args.length ? args.shift() : true;

  this.insertAll(Array.isArray(docs) ? docs : [docs], checkKeys, callback);
  return this;
};
...
Collection.prototype.insertAll = function(docs, checkKeys, callback) {
  var args = Array.prototype.slice.call(arguments, 1);
  callback = args.pop();
  checkKeys = args.length ? args.shift() : true;
  var error= null;
  ...
  try {
    ...
    var insertCommand = new InsertCommand(this.db.databaseName + "." + this.collectionName, checkKeys);
...
Collection.prototype.save = function(doc, options, callback) {
  ...
  var checkKeys = options != null && options.check_keys != null ? options.check_keys : true;

  if(id != null) {
    this.update({'_id':id}, doc, {upsert: true, safe: options != null ? options.safe : false}, callback);
  } else {
    this.insert(doc, checkKeys, function(err, docs) { Array.isArray(docs) ? callback(err, docs[0]) : callback(err, docs); });
  }
};
