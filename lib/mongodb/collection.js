...
var InsertCommand = require('./commands/insert_command').InsertCommand
  ...
  , batch = require('./collection/batch');
...
/**
 * Initiate a Out of order batch write operation, As you add documents to the batch it will
 * store them until you switch type of operation where upton it will execute all the operation of the previous type
 *
 * Options
 *  - **w**, {Number/String, > -1 || 'majority' || tag name} the write concern for the operation where < 1 is no acknowlegement of write and w >= 1, w = 'majority' or tag acknowledges the write
 *  - **wtimeout**, {Number, 0} set the timeout for waiting for write concern to finish (combines with w option)
 ...
 * @param {Function} callback this will be called after executing this method. The first parameter will contain the Error object if an error occured, or null otherwise. The second argument will be a UnorderedWriteOperationBatch object.
 * @return {null}
 * @api public
 */
Collection.prototype.initializeUnorderedBatch = batch.initializeUnorderedBatch;
