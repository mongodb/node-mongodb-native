...
/**
  ...
  remove_doc:   set to true to remove the object before returning 
  fields:       hash of fieldnames where 0 = don't retrieve field, 1 = retrieve field or an array of fieldnames TO retrieve
  upsert:       set to true to upsert the object (update if it exists; insert if it doesn't)
**/
Collection.prototype.findAndModify = function(query, sort, update, new_doc, remove_doc, fields, upsert, callback) {
  var args = Array.prototype.slice.call(arguments, 1);
  callback = args.pop();
  sort = args.length > 0 ? args.shift() : [];  
  update = args.length > 0 ? args.shift() : null;  
  new_doc = args.length > 0 ? args.shift() : false;  
  remove_doc = args.length > 0 ? args.shift() : false;
  fields = args.length > 0 ? args.shift() : null;
  upsert = args.length > 0 ? args.shift() : false;
  ...
  if (fields) {
    ...
    if (fields.constructor == Array){
      for(var i = 0, l = fields.length, fieldsObject = {}; i < l; i++) fieldsObject[fields[i]] = 1;
      fields = fieldsObject;
    }
