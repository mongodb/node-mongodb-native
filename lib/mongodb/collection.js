...
Collection.prototype.findAndModify = function findAndModify (query, sort, doc, options, callback) {
  ...
  // allow use of db.strict options such as w = 2, wtimeout = 5000
  var errorOptions = this.db.strict || null;
...
//      console.log("findAndModify commandOptions = "+inspect(commandOptions));
    // Execute command with safe options (rolls up both command and safe command into one and executes them on the same connection)
  ...
  //      console.log("findAndModify err = "+err);
    ...
    //    console.log("findAndModify notOK");
        callback(new Error(result.documents[0].errmsg));
      } else {
      //  console.log("findAndModify result.documents[0].value = "+inspect(result.documents[0].value));
        callback(null, result.documents[0].value);
      }      
    });    
  } else {
