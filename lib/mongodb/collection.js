 ...
 *
 * Options
 *  - **safe**, execute insert with a getLastError command returning the result of the insert command.
 *   - values = *true | {w:n, wtimeout:n} | {fsync:true}*
 *   - default = *false*
 *  - **keepGoing**, keep inserting documents even if one document has an error, **mongodb 1.9.1 >**.
 *   - values = *true | false* 
 *   - default = *false*
 *  - **serializeFunctions**, serialize functions on the document.
 *   - values = *true | false*
 *   - default = *false*
 *
 ...
 *   - values = *true | false* 
 *   - default = *false*
 *  - **multi**, update all documents matching the selector.
 *   - values = *true | false*
 *   - default = *false*
 *  - **serializeFunctions**, serialize functions on the document.
 *   - values = *true | false*
 ...
 *   - values = *true | {w:n, wtimeout:n} | {fsync:true}*
 *   - default = *false*
 *  - **remove**, set to true to remove the object before returning.
 *   - values = *true | false* 
 *   - default = *false*
 *  - **upsert**, perform an upsert operation.
 *   - values = *true | false*
 ...
 *   - values = *A Number > 0*
 *   - default = *0* all documents or up to the max allowed by MongoDB.
 *  - **sort**, set to sort the documents coming back from the query.
 *   - values = *Array of indexes, [['a', 1]] etc* 
 *   - default = *Not Applied*
 *  - **fields**, the fields to return in the query.
 *   - values = *Object of fields to include or exclude (not both), {'a':1}*
 *   - default = *Not Applied*
 *  - **skip**, set to skip N documents ahead in your query (useful for pagination).
 *   - values = *A Number > 0*
 *   - default = *0*
 *  - **hint**, tell the query to use specific indexes in the query.
 *   - values = *Object of indexes to use, {'_id':1}*
 *   - default = *Not Applied*
 *  - **explain**, explain the query instead of returning the data.
 *   - values = *true|false*
 *   - default = *Not Applied*
 *  - **snapshot**, snapshot query.
 *   - values = *true|false*
 *   - default = *Not Applied*
 *  - **timeout**, specify if the cursor can timeout.
 *   - values = *true|false*
 *   - default = *true*
 *  - **tailable**, specify if the cursor is tailable.
 *   - values = *true|false*
 *   - default = *Not Applied*
 *  - **batchSize**, set the batchSize for the getMoreCommand when iterating over the query results.
 *   - values = *A Number > 0*
 *   - default = *0*, return the max number of documents that MongoDB allows
 ...
 *   - values = *String*
 *   - default = *Not Applied*
 *  - **raw**, Return all BSON documents as Raw Buffer documents.
 *   - values = *true|false*
 *   - default = *false*
 *  - **read**, Tell the query to read from a secondary server.
 *   - values = *true|false*
 *   - default = *false*
 *
 * @param {Object} query query object to locate the object to modify
...
Collection.prototype.find = function find () {
  ...
  if(len === 2) {
    // prefered way to extend the object
    var test = ['limit', 'sort', 'fields', 'skip', 'hint', 'explain', 'snapshot'
		, 'timeout', 'tailable', 'batchSize', 'raw', 'read'
 ...
 *
 * Options
 *  - **unique**, creates an unique index.
 *   - values = *true | false* 
 *   - default = *Not Applied*
 ...
 *   - values = *Number*
 *   - default = *Not Applied*
 *  - **v**, specify the format version of the indexes.
 *   - values = *Number*
 *   - default = *1*, Default is used except in unusual situations
 *
 * @param {Object} fieldOrSpec fieldOrSpec that defines the index.
 * @param {Object} [options] additional options during update.
 * @param {Function} callback for results.
 ...
 *
 * Options
 *  - **full**, returns the full raw index information.
 *   - values = *true | false* 
 *   - default = *false*
 *
 * @param {Object} [options] additional options during update.
...
Collection.prototype.reIndex = function(callback) {
  this.db.reIndex(this.collectionName, function(err, result) {
    if(err != null) {
      callback(err, false);
    } else if(result.documents[0].errmsg == null) {
      callback(null, true);
 ...
 *
 * Options
 *  - **out**, sets the output target for the map reduce job.
 *   - values = *{inline:1} | {replace:'collectionName'} | {merge:'collectionName'} | {reduce:'collectionName'}*
 *   - default = *{inline:1}*, for backward compatibility.
 ...
 *   - values = *Object*
 *   - default = *Not Applied*
 *  - **jsMode**, it is possible to make the execution stay in JS. Provided in MongoDB > 2.0.X.
 *   - values = *true|false*
 *   - default = *Not Applied*
...
Collection.prototype.mapReduce = function mapReduce (map, reduce, options, callback) {
  ...
  // Set default to go to the inline table otherwise it will break against
