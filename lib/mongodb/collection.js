...
Collection.prototype.save = function(doc, options, callback) {
  ...
  if(id != null) {
    ...
    this.insert(doc, {safe: options != null ? options.safe : false}, function(err, docs) { 
      // Only callback when there is one defined
      if(callback != null) {
        if(err != null) return callback(err, null);
        Array.isArray(docs) ? callback(err, docs[0]) : callback(err, docs);         
      }
    });
  }
};
...
Collection.prototype.findOne = function(queryObject, options, callback) {
  ...
  this.db.executeCommand(queryCommand, function(err, result) {
    ...
    return callback(err, result.documents[0]);
  });
};
...
Collection.prototype.group = function(keys, condition, initial, reduce, command, callback) {
  ...
  // Konvert object keys to array
  if(!Array.isArray(keys) && keys instanceof Object) {
    keys = Object.keys(keys);
  }

  if(command) {
    // Ensure the reduce function is a string
    reduce = reduce instanceof Function ? reduce.toString() : reduce;
    // Set up hash
    var hash = {},
