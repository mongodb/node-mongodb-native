...
MongoClient.connect = function(url, options, callback) {
  ...
  for(var i = 0; i < object.servers.length; i++) {
    ...
    var _server_options = {poolSize:1, socketOptions:{connectTimeoutMS:30000}, auto_reconnect:false};
...
var _finishConnecting = function(serverConfig, object, options, callback) {
  ...
  // Set default never timeout
  var socketTimeoutMS = null;
  // Set the socketTimeout for connection to the same as the connection timeout
  // This is to work around the secondary stalling ismaster, reset after connection
  // var socketTimeoutMS = serverConfig.socketTimeoutMS;
  if(serverConfig instanceof ReplSet) {
    socketTimeoutMS = object.rs_options.socketOptions.connectTimeoutMS || 5000;
  }
  ...
  db.open(function(err, db){
    ...
    serverConfig.socketTimeoutMS = socketTimeoutMS;
