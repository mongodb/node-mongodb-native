...
Cursor.prototype.nextObject = function(callback) {
  ...
  if(self.state == Cursor.INIT) {
    ...
    if(this.connection == null) {
      ...
      this.connection = this.read == true || this.read === 'secondary' ? self.db.serverConfig.checkoutReader() : self.db.serverConfig.checkoutWriter(true);      
    }
    // Execute the command
    self.db._executeQueryCommand(cmd, {read:self.read, raw:self.raw, connection:this.connection}, commandHandler);
    commandHandler = null;
