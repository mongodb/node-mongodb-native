...
Cursor.prototype.formattedOrderClause = function() {
  ...
  } else if(this.sortValue.className == "OrderedHash") {    
  ...
  if(self.state == Cursor.INIT) {    
    ...
    self.db.executeCommand(queryCommand, function(results) {            
      ...
      if(self.limitValue > 0 && self.limitValue > numberReturned) {
        ...
        self.fetchFirstResults(results, callback);
...
Cursor.prototype.fetchFirstResults = function(results, callback) {
  var self = this;
...
Cursor.prototype.close = function(callback) {
  ...
  if(self.cursorId.className == "Long" && self.cursorId.greaterThan(new mongo.Long.fromInt(0))) {
