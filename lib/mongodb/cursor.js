...
function Cursor(db, collection, selector, fields, skip, limit
	...
	, returnKey, maxScan, min, max, showDiskLoc, comment, awaitdata) {
  this.db = db;
...
Cursor.prototype.nextObject = function(callback) {
  ...
  if(self.state == Cursor.INIT) {
    ...
    if(this.connection == null) {
      // console.log("===================================== COURSER :: " + this.read)      
      // Fetch either a reader or writer dependent on the specified read option
      // this.connection = this.read == null || this.read == 'primary' ? self.db.serverConfig.checkoutWriter() : self.db.serverConfig.checkoutReader(this.read);
      this.connection = this.read == null ? self.db.serverConfig.checkoutWriter() : self.db.serverConfig.checkoutReader(this.read);
    }

    // Execute the command
    self.db._executeQueryCommand(cmd, {raw:self.raw, read:this.read, connection:this.connection}, commandHandler);
...
var getMore = function(self, callback) {
  ...
  try {
    ...
    // Set up options
    var options = { read: self.read, raw: self.raw, connection:this.connection };
      ...
      try {
        ...
        if(result.numberReturned > 0) {
          ...
          if(self.numberOfRetries == 0) {
            self.close(function() {callback(new Error("tailable cursor timed out"), null);});            
          } else {
...
Cursor.prototype.streamRecords = function(options) {
  ...
  function execute(command) {
    self.db._executeQueryCommand(command, {read:self.read, raw:self.raw, connection:this.connection}, function(err,result) {
      if(err) {
...
Cursor.prototype.close = function(callback) {
  ...
  if(this.cursorId instanceof Long && this.cursorId.greaterThan(Long.fromInt(0))) {
    try {
      ...
      this.db._executeQueryCommand(command, {read:self.read, raw:self.raw, connection:this.connection}, null);
    } catch(err) {}
  }
