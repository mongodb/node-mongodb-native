...
Cursor.prototype.limit = function(callback, limit) {
  if(this.queryRun == true || this.state == Cursor.CLOSED) {
    ...
    if(limit != null && limit.constructor != Number) {
      callback({ok:false, err:true, errmsg:"limit requires an integer"});
    } else {
...
Cursor.prototype.skip = function(callback, skip) {
  if(this.queryRun == true || this.state == Cursor.CLOSED) {
    ...
    if(skip != null && skip.constructor != Number) {
      callback({ok:false, err:true, errmsg:"skip requires an integer"});
    } else {
      this.skipValue = skip;
      callback(this);      
    }
...
Cursor.prototype.fetchAllRecords = function(callback) {
  ...
  if(self.state == Cursor.INIT) {    
    ...
    if(self.cursorId.greaterThan(Long.fromInt(0))) {
      // sys.puts("============================= fetchAllRecords");
      // sys.puts("============================= cursorId: " + self.cursorId.toString());
      // sys.puts("============================= name: " + self.db.databaseName + "." + self.collection.collectionName);
      // sys.puts("============================= limit: " + self.limitValue);
...
Cursor.prototype.fetchFirstResults = function(callback, results) {
  ...
  // sys.puts("========================================= fetchFirstResults: " + results[0].documents.length);
...
Cursor.prototype.nextObject = function(callback) {  
  ...
  if(self.state == Cursor.INIT) {   
    // sys.puts("= 1 called each [state] = " + (this.state == Cursor.CLOSED));
    ...
    if(self.items.length > self.index) {
      // sys.puts("= 2 called each [state] = " + (this.state == Cursor.CLOSED));
      callback(self.items[self.index++]);
    } else {
      // sys.puts("= 3 called each [state] = " + (this.state == Cursor.CLOSED));
      callback(null);
    }
  }    
}
