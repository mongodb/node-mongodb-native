...
Cursor.prototype.each = function(callback) {
  ...
  if(this.state != Cursor.CLOSED) {
    ...
    process.nextTick(function(){
      var item;
      while (item = self.items.shift())
        callback(null, item);
      nextBatch(self, function(err, items) {
        if(err != null) return callback(err, null);
...
Cursor.prototype.setReadPreference = function(readPreference, tags, callback) {
  ...
  if(this.queryRun == true || this.state == Cursor.CLOSED) {
    ...
    && readPreference != 'secondaryOnly' && readPreference != 'secondary') {
      if(callback == null) throw new Error("only readPreference of primary, secondary or secondaryOnly supported");
...
Cursor.prototype.nextObject = function(callback) {
  ...
  nextBatch(self, function (err, items) {
    if(err != null) return callback(err, null);
    if(items != null) return callback(null, items.shift());
    return callback(null, null);
  });
}

var nextBatch = function(self, callback) {
  if(self.state == Cursor.INIT) {
    var cmd;
    try {
