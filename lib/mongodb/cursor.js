...
/**
 * Executes client-side map reduce on cursor.
 * @param {Function} mapper A function that accepts next document from the cursor and process it.
 * @param {Object} output An output document used to store reduce result.
 * @param {Function} callback A callback that accepts modified output object.
 * @api public
*/
Cursor.prototype.mapReduce = function(fn, output, callback){
  if(output instanceof Function) {callback = output; output = new Array();}
  else if(output === null || output === undefined) output = new Array();

  //this function is used to process the next document returned by the cursor
  function next(){
    this.nextObject(function(err, doc){
      if(err || doc === null || doc === undefined) acceptor(output);	//no document from db, return
      else fn(doc, output, function(){
             //avoiding stack overflow (put next processing into the process loop)
             process.nextTick(function(){ next.call(this); }.bind(this));
	}.bind(this));
    }.bind(this));
  }
  //force cursor processing
  next.call(this);
};
