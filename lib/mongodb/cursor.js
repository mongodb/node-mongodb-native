...
Cursor.prototype.nextObject = function(callback) {
  ...
  if(self.state == Cursor.INIT) {
    ...
    // console.log("------------------------------------------------------------ nextObject :: 0")
    self.db._executeQueryCommand(cmd, {read:true}, commandHandler);
    commandHandler = null;
...
Cursor.prototype.getMore = function(callback) {
  ...
  try {
    ...
    // Execute the command
    self.db._executeQueryCommand(getMoreCommand, {read:true}, function(err, result) {
      try {
...
Cursor.prototype.streamRecords = function(options) {
  ...
  function execute(command) {
    self.db._executeQueryCommand(command, {read:true}, function(err,result) {
      if(err) {
...
Cursor.prototype.close = function(callback) {
  ...
  if(this.cursorId instanceof self.db.bson_serializer.Long && this.cursorId.greaterThan(self.db.bson_serializer.Long.fromInt(0))) {
    try {
      ...
      this.db._executeQueryCommand(command, {read:true}, null);
    } catch(err) {}
  }
