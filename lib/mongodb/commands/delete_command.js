...
DeleteCommand.prototype.toBinary = function() {
  ...
  if(this.selector instanceof Buffer) {
    var object_size = this.selector[0] | this.selector[1] << 8 | this.selector[2] << 16 | this.selector[3] << 24;
    if(object_size != this.selector.length) throw new Error("raw message size does not match message header size [" + this.selector.length + "] != [" + object_size + "]");
