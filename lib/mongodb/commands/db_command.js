...
process.inherits(DbCommand, mongo.QueryCommand);
...
DbCommand.createIsMasterCommand = function(databaseName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('ismaster', 1), null);
};

DbCommand.createCollectionInfoCommand = function(databaseName, selector) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_NAMESPACE_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, 0, selector, null);
};

DbCommand.createGetNonceCommand = function(databaseName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('getnonce', 1), null);
};
...
DbCommand.createAuthenticationCommand = function(databaseName, username, password, nonce) {
  ...
  // Create db command
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, selector, null);
};

DbCommand.createLogoutCommand = function(databaseName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('logout', 1), null);
};
...
DbCommand.createDropCollectionCommand = function(databaseName, collectionName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('drop', collectionName), null);
};

DbCommand.createRenameCollectionCommand = function(databaseName, fromCollectionName, toCollectionName) {
  return new DbCommand("admin." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('renameCollection', databaseName + "." + fromCollectionName).add('to', databaseName + "." + toCollectionName), null);
};

DbCommand.createGetLastErrorCommand = function(databaseName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('getlasterror', 1), null);
};
...
DbCommand.createGetPreviousErrorsCommand = function(databaseName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('getpreverror', 1), null);
};

DbCommand.createResetErrorHistoryCommand = function(databaseName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('reseterror', 1), null);
};
...
DbCommand.createCreateIndexCommand = function(databaseName, collectionName, fieldOrSpec, unique) {
  ...
  return new mongo.InsertCommand(databaseName + "." + DbCommand.SYSTEM_INDEX_COLLECTION, false).add(selector);
};

DbCommand.createDropIndexCommand = function(databaseName, collectionName, indexName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('deleteIndexes', collectionName).add('index', indexName), null);
};

DbCommand.createDropDatabaseCommand = function(databaseName) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, new mongo.OrderedHash().add('dropDatabase', 1), null);
};

DbCommand.createDbCommand = function(databaseName, command_hash) {
  return new DbCommand(databaseName + "." + DbCommand.SYSTEM_COMMAND_COLLECTION, mongo.QueryCommand.OPTS_NO_CURSOR_TIMEOUT, 0, -1, (command_hash.className == "OrderedHash" ? command_hash : new mongo.OrderedHash(command_hash)), null);
};
