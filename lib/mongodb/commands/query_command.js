...
QueryCommand.prototype.toBinary = function() {
  ...
  if(this.query instanceof Buffer) {
    ...
    totalLengthOfCommand = 4 + Buffer.byteLength(this.collectionName) + 1 + 4 + 4 + this.db.bson_serializer.BSON.calculateObjectSize(this.query) + (4 * 4);    
  }
  ...
  if(object instanceof Buffer) {
    ...
    documentLength = this.db.bson_serializer.BSON.serializeWithBufferAndIndex(object, this.checkKeys, _command, _index) - _index + 1;
  }
