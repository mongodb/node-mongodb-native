...
BaseCommand.prototype.toBinary = function() {
  if(this instanceof require('./insert_command').InsertCommand || 
    ...
    //   // BinaryParser.ilprint(this.db.bson_serializer.BSON.serialize(this.query, false, true).toString())
    //   
    //   for(var j = 0; j < commandBuffers[i].length;j++) {
    //     // debug("------")
    //     debug(commandBuffers[i][j] + " :: " + commandBuffers[i].toString('ascii', j, j + 1))
    //   }
    // }
