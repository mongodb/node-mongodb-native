...
PingStrategy.prototype.checkoutSecondary = function() {  
  ...
  var candidateServers = this.replicaset._state.master != null ? [this.replicaset._state.master] : [];
  ...
  for(var i = 0; i < candidateServers.length; i++) {
    if(candidateServers[i].runtimeStats['pingMs'] > candidateServers[0].runtimeStats['pingMs'] + this.secondaryAcceptableLatencyMS) {
      // Slice out the array and break
      finalCandidates = candidateServers.slice(i);
      break;
    }
  }
