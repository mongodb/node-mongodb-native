...
ReplSetServers.prototype.connect = function(parent, options, callback) {
      ...
      if(err == null && result.documents[0].hosts != null) {
        ...
        for(var i = 0; i < candidateServers.length; i++) {
          ...
          if(replSetSelf._state.addresses[candidateServerString] == null) {            
            ...
            //TODO TODO CLONE SOCKET OPTIONS
            // var newServer = new Server(parts[0], parseInt(parts[1]), {auto_reconnect:false, socketOptions:replSetSelf.socketOptions});
  ...
  for(var i = 0; i < serverConnections.length; i++) {    
    try {    
      // TODO CLONE SOCKET OPTIONS AND MODIFY CORRECTLY
      // // Set the socket options
      // serverConnections[i].socketOptions = this.socketOptions == null ? {} : this.socketOptions;
...
ReplSetServers.prototype.checkoutReader = function() {
  ...
  console.log(this.readSecondary)
