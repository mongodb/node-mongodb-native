...
ReplSetServers.prototype.connect = function(parent, options, callback) {
  ...
  if(firstCall || replSetSelf._state == null) {
    replSetSelf._state = {'master':null, 'secondaries':{}, 'arbiters':{}, 'passives':{}, 'errors':{}, 'addresses':{}, 'byTags':{}, 'setName':null, 'errorMessages':[], 'runtimeStats': {}, 'pingIntervalId':null};
  } else {
    ...
    return function(err, result) {
      ...
      if((numberOfServersLeftToInitialize == 0) && replSetSelf._serverState === 'connecting' && replSetSelf._state.errorMessages.length == 0) {
        ...
        if(replSetSelf.masterNotNeeded || replSetSelf._state.master != null) {
          // We have connected successfully, start the ping server
          startPingServer(replSetSelf, function(err) {
...
var startPingServer = function(self, callback) {
  console.log("-------------------------------------------------------- start ping server")
  // Start up the ping timer
  self._state['pingIntervalId'] = setInterval(function() {
    console.log("------------------------------------------------------- ping interval started")
    // Grab all servers
    var serverKeys = Object.keys(self._state.addresses);
    // We need to ping for all servers
    for(var i = 0; i < serverKeys.length; i++) {
      var server = self._state.addresses[serverKeys[i]];
      // Let's grab the stat connection
      var connection = server.checkoutStatsConnection();      
      // If we have a connection and a db instance
      if(connection != null && self.db != null) {
        ...
        new function(replSet, serverInstance, connectionInstance) {
          ...
          self.db.admin().ping(function(err, result) {
            // Get end time of the command
            var endTime = new Date().getTime();
            // Server-connection id
            var serverConnectionId = server.host + ":" + server.port;
            // Let's ensure we have an object for this connection
            if(replSet._state.runtimeStats[serverConnectionId] == null) replSet._state.runtimeStats[serverConnectionId] = {};
            // Save the ping time
            replSet._state.runtimeStats[serverConnectionId]['pingMs'] = (endTime - startTime);
  ...
  console.log("-------------------------------------------------------- start ping server : 2")
...
ReplSetServers.prototype.allRawConnections = function() {
  ...
  allPoolConnections = allPoolConnections.concat(allMasterConnections);
  ...
  for(var i = 0; i < allServers.length; i++) {
    ...
    server.close(function() {
      ...
      if(numberOfServersToClose == 0) {
        // Ensure we clear out the interval check if there is any
        if(self._state.pingIntervalId != null) clearInterval(self._state.pingIntervalId);
        // Clear out state
        self._state = {'master':null, 'secondaries':{}, 'arbiters':{}, 'passives':{}, 'errors':{}, 'addresses':{}, 'byTags':{}, 'setName':null, 'errorMessages':[], 'runtimeStats': {}, 'pingIntervalId':null};
      }
