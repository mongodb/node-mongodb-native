...
ReplSetServers.prototype.connect = function(parent, options, callback) {
  ...
  var timeoutHandler = function(err, server) {
    ...
    console.log("============== got function :: " + (typeof callback == 'function'))

    // Shut down the server and emit the error to the dbInstance
    replSetSelf.close(); 
    
    // Fire off all the callbacks sitting with no reply
    // Get all non-replied to messages
    var dbInstance = parent;
    ...
    for(var i = 0; i < keys.length; i++) {
      ...
      process.nextTick(function() {
        _callback(new Error("operation timed out"), null);                        
      });
    }
  }
  ...
  var errorHandler = function(err, server) {
    ...
    if(typeof callback == 'function') console.log("============== got function")
    // Remove all listeners
    replSetSelf.removeAllListeners();
    // Shut down the server and emit the error to the dbInstance
    replSetSelf.close();
  ...
  var connectionHandler = function(instanceServer) {
    return function(err, result) {
      ...
      if(err == null && result.documents[0].hosts != null) {
        ...
        // var timeoutHandler = function(err) {
        //   console.log("---------------------------------------------------------------- timeout received")
        //   
        //   if(err.stack != null) console.log(err.stack)
        //   // Shut down the server and emit the error to the dbInstance
        //   replSetSelf.close();          
        //   // Emit error
        //   parent.emit("timeout", err);
        //   // Remove listener on parent
        //   parent.removeAllListeners("timeout");
        // }
  ...
  for(var i = 0; i < serverConnections.length; i++) {    
    try {    
      ...
      // Add error handler to the instance of the server
      // instanceServer.on("error", errorHandler);
      serverConnections[i].on("timeout", timeoutHandler);      
      ...
      console.log("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 1")
      // console.dir(serverConnections[i])
