...
var ReplSetServers = exports.ReplSetServers = function(servers, options) {
  ...
  this.replicasetChangeEnabled = false;
  this.replicaSetChangeCheckedTime = new Date().getTime();
...
var _checkForReplicaChange = function(self, connection) {
  // console.log("============================================================= CHANGE CHECK");
  self.replicaSetChangeCheckedTime = new Date().getTime();

  // var db = new Db('integration_tests', new Server("127.0.0.1", 27017, 
  //  {auto_reconnect: false, poolSize: 4, ssl:useSSL}), {native_parser: native_parser});

  // If we have a connection
  if(connection != null && self.replicasetChangeEnabled && !self.replicaSetChanged) {
    var Db = require('../db').Db;
    // Create a new db instance to check the replicaset status
    var _db = new Db(self.db.databaseName, new Server(connection.socketOptions.host, connection.socketOptions.port, {poolSize:1, auto_reconnect: false}));
    _db.open(function(err, _db) {
      if(err != null) {
        // console.log("------------------------------------------------------------ ERROR")
        // console.dir(err)
      } else {
        _db.admin().command({replSetGetStatus:1}, {connection:connection}, function(err, result) {      
          ...
          if(self._state.addresses != null && Object.keys(self._state.addresses).length != members.length) {
            ...
            // console.log("Object.keys(self._state.addresses).length = " + Object.keys(self._state.addresses).length)
  ...
  var connectionHandler = function(instanceServer) {
    return function(err, result) {
      ...
      if(err == null && result.documents[0].hosts != null) {
        ...
        if(isMaster == true || secondary == true || arbiterOnly == true) {
            ...
            var closeServers = function() {
             // Set the state to disconnected
              ...
              if(replSetSelf._serverState == 'connected') {
                // Disable the check for configuration change
                replSetSelf.replicasetChangeEnabled = false;
          ...
          var timeoutHandler = function(err, server) {
            ...
            //     } else {
            //       // If the parent has listeners trigger an event
            //       if(parent.listeners("timeout").length > 0) {
            //         parent.emit("timeout", new Error("connection timed out"));
            //       }
            //     }
            //   });          
      ...
      if((numberOfServersLeftToInitialize == 0) && replSetSelf._serverState === 'connecting' && replSetSelf._state.errorMessages.length == 0) {
        ...
        replSetSelf.replicaSetChanged = false;
        // Set db as connected
        ...
        if(replSetSelf.masterNotNeeded || replSetSelf._state.master != null) {
          // Enable the check for configuration change
          replSetSelf.replicasetChangeEnabled = true;
...
ReplSetServers.prototype.checkoutWriter = function() {
  ...
  // If it's time to check the replicaset peform the check
  if(new Date().getTime() > (this.replicaSetChangeCheckedTime + this.replicaSetCheckInterval)) {
    _checkForReplicaChange(this, connection);
  }
