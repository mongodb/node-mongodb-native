...
function Server(host, port, options) {
  ...
  // console.dir("=========================== POOLSIZE :: " + this.poolSize)
  // console.dir("=========================== auto_reconnect :: " + this.isAutoReconnect())
...
Server.prototype.close = function(callback) {
  ...
  // console.log("tag :: " + this.TAGG)
  // Set server status as disconnected
  this._serverState = 'destroyed';
  // Remove all local listeners
  this.removeAllListeners();

  // console.dir(this.connectionPool)
...
Server.prototype.inheritReplSetOptionsFrom = function (replset) {
  this.socketOptions = {};
  this.socketOptions.connectTimeoutMS = replset.options.socketOptions.connectTimeoutMS;
...
Server.prototype.connect = function(dbInstance, options, callback) {
  ...
  // if(this.connectionPool.bson == null) {
  //   this.connectionPool.bson = dbInstance.bson;
  // } else {
  ...
  connectionPool.on("message", function(message) {
    // console.log("=============================== connection pool message")
  ...
  connectionPool.on("error", function(message, connection, error_options) {
    ...
    // console.dir(message)
    // console.dir(server._serverState)
    // If pool connection is already closed
    // if(server._serverState === 'disconnected') return;
    if(server._serverState === 'disconnected' 
      || server._serverState === 'destroyed') return;
  ...
  connectionPool.on("close", function() {
    // console.log("===================================== SERVER CLOSED :: " + server.port)
    // console.dir(server._serverState)
    // console.log("tag :: " + server.TAGG)
    // If pool connection is already closed
    // if(server._serverState === 'disconnected') return;
    if(server._serverState === 'disconnected' 
      || server._serverState === 'destroyed') return;
    ...
    if(server.isAutoReconnect() 
      ...
      if(!server._reconnectInProgreess) {
        ...
        // console.log("=============================== CONNECT TO :: " + server.port)
        // console.log("=============================== CONNECT TO :: " + server.port)
        // console.log(server.db.retryMiliSeconds)
        // console.log(server.options)
        // console.log(server.isAutoReconnect())
  ...
  var __attemptReconnect = function(server) {
    return function() {
      // console.log("###################################### __attemptReconnect :: " + server.db.retryMiliSeconds + " :: " + server.port)
      // console.log("server._serverState :: " + server._serverState)
      // console.log("_reconnectInProgreess :: " + server._reconnectInProgreess)
      // console.log(new Date())
      // console.log("server._serverState2 :: " + server._serverState2)
      // console.log("tag :: " + server.TAGG)
      ...
      server.connect(server.db, server.options, function(err, result) {
        ...
        if(err) {
          ...
          // console.log("2@@@@@@@@@22222")
          // console.dir(server._commandsStore)
          // Execute all saved operations
          // console.dir(server)
          server._reconnectInProgreess = false;
