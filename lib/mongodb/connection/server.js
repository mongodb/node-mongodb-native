...
var Server = exports.Server = function(host, port, options) {
  ...
  this.poolSize = this.ssl ? 1 : this.poolSize;
...
Server.prototype.isConnected = function() {
  // console.log("-------------------------- isConnected :: " + (this.connectionPool && this.connectionPool.isConnected()))
...
Server.prototype.connect = function(dbInstance, options, callback) {
  ...
  if(this.ssl == true) {
    ...
    this.socketOptions.ssl = true;  
    // Restrict to a poolSize of 1 due to what seems like a bug  
    this.poolSize = 1;  
  }
  ...
  connectionPool.on("timeout", function(err) {
    // console.log("===================================== server:timeout :: " + firstCall)
    // Set timeout
    // server._timeout = true;
    // Emit timeout event
    if(firstCall && typeof callback === 'function') {
  ...
  connectionPool.on("error", function(message) {   
    ...
    if(server._serverState !== 'disconnected' && firstCall && (typeof callback === 'function')) {
      ...
      if(typeof internalCallback === 'function') internalCallback(new Error(message && message.err ? message.err : message));
  ...
  connectionPool.on("close", function() {
    ...
    if(firstCall && typeof callback === 'function') {
      ...
      if(eventEmitterIsDb) {
        // Issue close across all the db instances registered in server instance
        for(var i = 0; i < server.dbInstances.length; i++) {
          server.dbInstances[i].emit("close", server);
        }        
      } else {
