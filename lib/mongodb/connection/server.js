...
Server.prototype.connect = function(dbInstance, options, callback) {
  ...
  // Set that function was called with a callback
  ...
  connectionPool.on("poolReady", function() {   
    ...
    // dbInstance.on(db_command.getRequestId(), connectHandler);
    // dbInstance._callBackStore.on(db_command.getRequestId(), callback);    
    ...
    try {
      ...
      if(mongoReply.messageLength != message.length) {
        ...
        for(var i = 0; i < server.dbInstances.length; i++) {
          ...
          // console.log("-------------------------------------------------------------- :::: " + server.dbInstances.length + " :: " + callbackInfo.callback)
          // console.log("mongoReply.responseTo.toString() = " + mongoReply.responseTo.toString())
          ...
          if(callbackInfo.callback && Array.isArray(callbackInfo.info.chained)) {
            ...
            mongoReply.parseBody(message, connectionPool.bson, callbackInfo.info.raw, function(err) {
              ...
              if(firstResult[0].err != null || firstResult[0].errmsg != null) {
                ...
                // Clean up all chained callbacks, ensuring no more callbacks happening
                // for(var i = 0; i < chainedIds.length; i++) {
                //   dbInstanceObject._removeHandler(chainedIds[i]);
                // }
                
                // console.dir(dbInstanceObject._callBackStore)
                // console.dir(chainedIds)
                // console.dir(dbInstanceObject._callBackStore.listeners(chainedIds[0]))
                // console.dir(dbInstanceObject._callBackStore.listeners(chainedIds[1]))                
                ...
                // console.log("--------------------------------------------------------------------------- 2")
                // console.dir(chainedIds)
                // console.dir(mongoReply.responseTo)
                
                if(chainedIds.length > 0 && chainedIds[chainedIds.length - 1] == mongoReply.responseTo) {
                  // var callbackInfo = dbInstanceObject._findHandler(mongoReply.responseTo);
                  // console.log("============================ last callback")
                  // console.dir(dbInstanceObject._findHandler(mongoReply.responseTo))
                ...
                //     handler.info.results = Array.isArray(callbackInfo.info.results) ? callbackInfo.info.results : [];
                //     handler.info.results.push(mongoReply);
                //     foundChainedMethod = true;
                //     break;
                //   }
                // }
                // 
                // console.log("========================================================================= 3 :: " + foundChainedMethod)
                // console.dir(handler.info)
                //       
                ...
                //   console.log("---------------------------------------------------- " + mongoReply.responseTo)
                //   
                //   // Trigger the callback for the error
                //   dbInstanceObject._callHandler(mongoReply.responseTo, callbackInfo.info.results.shift(), null);
            ...
            // Parse the body
            mongoReply.parseBody(message, connectionPool.bson, callbackInfo.info.raw, function(err) {
              // console.log("---------------------------------------------------- 1 :: " + (new Date().getTime() - startDate));
              // Get the callback instance
              // var callbackInstance = dbInstanceObject._removeHandler(mongoReply.responseTo);
              // console.log("---------------------------------------------------- 2 :: " + (new Date().getTime() - startDate));
              ...
              // console.log("---------------------------------------------------- 3 :: " + (new Date().getTime() - startDate));
            
              // Trigger the callback
              // callbackInstance.callback(null, mongoReply, callbackInstance.info.connection);              
              dbInstanceObject._callHandler(mongoReply.responseTo, mongoReply, null);
              // console.log("---------------------------------------------------- 4 :: " + (new Date().getTime() - startDate));
            });          
          }
        }
      }      
      ...
      //         mongoReply = new MongoReply(dbInstance, message);        
      // 
      //         // Emit the object
      //         // dbInstance.emit(mongoReply.responseTo, null, mongoReply);
      //         // dbInstance._callBackStore.emit(mongoReply.responseTo, null, mongoReply);
      //         dbInstance._callHandler(mongoReply.responseTo, mongoReply, null);
      //         // Get the callback instance
      //         // var callbackInstance = dbInstanceObject._removeHandler(mongoReply.responseTo);
      ...
      //         
      // 
      //       // console.log("---------------------------------------------------- 0 :: " + (new Date().getTime() - startDate));
      //       // 
      //       // // Parse the body
      //       // mongoReply.parseBody(message, connectionPool.bson, callbackInfo.info.raw, function(err) {
      //       //   console.log("---------------------------------------------------- 1 :: " + (new Date().getTime() - startDate));
    ...
    for(var i = 0; i < server.dbInstances.length; i++) {
      ...
      for(var j = 0; j < keys.length; j++) {
        ...
        if(info.connection.socketOptions.host === server.host && info.connection.socketOptions.port === server.port) {
          console.log("--------------------------------------------------------------------------- CLOSE 1")
          console.dir(dbInstance._callBackStore.listeners(keys[j]))
          // process.nextTick(function() {
            dbInstance._callBackStore.emit(keys[j], new Error("connection closed"), null);
          // })
          // 
          // callbackslist.push(keys[j]);
        }
      }
    }
    
    console.log("--------------------------------------------------------------------------- CLOSE 2")
    // console.dir(callbackslist)
