 ...
 *
 * Options
 *  - **readPreference** {String, default:null}, set's the read preference (Server.PRIMARY, Server.PRIMARY_PREFERRED, Server.SECONDARY, Server.SECONDARY_PREFERRED, Server.NEAREST)
...
function Server(host, port, options) {
  ...
  this.poolSize = this.options.poolSize == null ? 10 : this.options.poolSize;
...
Server.PRIMARY_PREFERRED = 'primaryPreferred';
Server.SECONDARY = 'secondaryOnly';
Server.SECONDARY_PREFERRED = 'secondary';
Server.NEAREST = 'nearest'
...
var canCheckoutReader = function(self) {  
  ...
  if(self.isMasterDoc && self.isMasterDoc['arbiterOnly'] == true) {
    ...
    if((self._readPreference == Server.PRIMARY) && self.isMasterDoc['ismaster'] != true) {
