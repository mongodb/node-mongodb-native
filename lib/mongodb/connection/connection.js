...
Connection.prototype.write = function(command, callback) {
  ...
  // var binaryCommands = [];
  // console.log("----------------------------- connection.write :: 0")
  // try {
  //   var commands = !Array.isArray(command) ? [command] : command;
  //   // Create binary object for all, if we fail doing this catch the error
  //   for(var i = 0; i < commands.length; i++) {
  //     binaryCommands[i] = commands[i].toBinary();
  //   }
  // } catch(err) {
  //   
  // }
  ...
  try {
    ...
    if(Array.isArray(command)) {
      for(var i = 0; i < command.length; i++) {
        this.connection.write(command[i].toBinary());
      }
    } else {
      this.connection.write(command.toBinary());    
    }    
  } catch (err) {
    console.log("================================ error writing command");
    console.dir(err)
    callback(err);
  }
  // console.log("----------------------------- connection.write :: 1")
}
