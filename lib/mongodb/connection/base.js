...
NonExecutedOperationStore.prototype.execute_queries = function(db, query_func, write_func) {
  ...
  while(this.commands.read.length > 0) {
    ...
    query_func(db, command['db_command'], command['options'], command['callback']);
  }
}

NonExecutedOperationStore.prototype.execute_writes = function(db, query_func, write_func) {
...
Base.prototype._registerHandler = function(db_command, raw, connection, exhaust, callback) {
  // console.log('----- _registerHandler :: ' + this._callBackStore.id)
