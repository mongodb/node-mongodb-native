...
Base.prototype._registerHandler = function(db_command, raw, connection, exhaust, callback) {
  ...
  if(chained) {
    ...
    for(var i = 0; i < db_command.length; i++) {
      ...
      this._callBackStore.once(command.getRequestId().toString(), callback);
    ...
    this._callBackStore.once(db_command.getRequestId().toString(), callback);
    // Add the information about the reply
...
Base.prototype._callHandler = function(id, document, err) {
  ...
  if(id && this._callBackStore.listeners(id.toString()).length >= 1) {
    // Get info object
