...
function createServer (host, replset) {
  ...
  if(replset.socketOptions) {
    ...
    for(var k = 0; k < keys.length; k++) {
      socketOptions[keys[i]] = replset.socketOptions[keys[i]];
    }
  }
...
var _handler = function(event, self) {
  return function(err, server) {
    if (!self._state.master) return;

    // If it's a primary we need to close the set to allow reconnection later
    if(self._state.master.name == server.name) {
      self.close();
...
ReplSet.prototype.connect = function(parent, options, callback) {
  ...
  this.once("fullsetup", function() {
    ...
    // Manage the ha pings
    if(self.haEnabled) {
      var haFunction = function() {
        if(self._serverState == "disconnected") return;
        // If we are connected let's perform a healthcheck
        if(Array.isArray(self.dbInstances) && self.dbInstances.length > 0 && self._serverState == "connected") {
          self.updateHealth(self.dbInstances[0], function() {
            setTimeout(haFunction, self.replicasetStatusCheckInterval);
          });
        } else {
          setTimeout(haFunction, self.replicasetStatusCheckInterval);        
        }
      ...
      setTimeout(haFunction, this.replicasetStatusCheckInterval);
    }
