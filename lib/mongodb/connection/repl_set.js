 ...
 *  - **readPreference** {String}, the prefered read preference (Server.PRIMARY, Server.PRIMARY_PREFERRED, Server.SECONDARY, Server.SECONDARY_PREFERRED, Server.NEAREST).
...
ReplSet.prototype.connect = function(parent, options, callback) {
      ...
      if(err == null && result.documents[0].hosts != null) {
        ...
        if(isMaster == true || secondary == true || arbiterOnly == true) {
          ...
          replSetSelf.closeHandler = function(err, server) {
            // if(parent._state == 'disconnected') return;
  ...
  this.replicasetCheckFunction = function() {
    try {
      ...
      var con = self.checkoutReader(Server.READ_SECONDARY);
...
var pickFirstConnectedSecondary = function pickFirstConnectedSecondary(self, tags) {
  ...
  if(self._readPreference == Server.READ_SECONDARY) {
    connection = self._state.master.checkoutReader();
  }     

  if(connection == null) {
    var preferenceName = self._readPreference == Server.SECONDARY ? 'secondary' : self._readPreference;
...
ReplSet.prototype.checkoutReader = function(readPreference, tags) {
  ...
  finalReadPreference = finalReadPreference == true ? Server.READ_SECONDARY : finalReadPreference;

  // console.log("--------------------------------------------------------- finalReadPreference")
  // console.log(finalReadPreference)
  ...
  if((this.readSecondary == true || finalReadPreference == Server.READ_SECONDARY || finalReadPreference == Server.READ_SECONDARY_ONLY) && Object.keys(this._state.secondaries).length > 0) {
    // console.log("=================================================================================== Server.SECONDARY")
    // console.dir(this._state.secondaries)
    // If we have tags, look for servers matching the specific tag
    if(tags != null && typeof tags == 'object') {
      // console.log("=============================================================== Server.SECONDARY")

      // Matching keys
      var matchingKeys = Object.keys(tags);
      ...
      for(var i = 0; i < keys.length; i++) {
        var server = this._state.secondaries[keys[i]];
        // console.log("--------------------------------------- tags")
        // console.dir(server.tags)
        ...
        if(server.tags != null) {
          ...
          for(var j = 0; j < matchingKeys.length; j++) {
            if(server.tags[matchingKeys[j]] != tags[matchingKeys[j]]) {
              matching = false;
              break;
            }
          }
      ...
      // If no candidate servers pick any server available
  ...
  } else if(finalReadPreference == Server.PRIMARY_PREFERRED) {
    // console.log("=================================================================================== Server.PRIMARY_PREFERRED")
    ...
    if(connection == null) {
      // console.log("=============================================================== Server.PRIMARY_PREFERRED :: SECONDARY")

      // If we have tags, look for servers matching the specific tag
      if(tags != null && typeof tags == 'object') {
        // console.log("=============================================================== Server.PRIMARY_PREFERRED with tags")        
  ...
  } else if(finalReadPreference == Server.SECONDARY_PREFERRED && tags == null && Object.keys(this._state.secondaries).length == 0) {
    // console.log("=================================================================================== Server.SECONDARY_PREFERRED")
    // console.dir(this._state.secondaries)
    ...
    if(connection == null) {
      var preferenceName = finalReadPreference == Server.SECONDARY ? 'secondary' : finalReadPreference;
  ...
  } else if(finalReadPreference == Server.SECONDARY_PREFERRED) {
    // console.log("=================================================================================== Server.SECONDARY_PREFERRED NO SECONDARIES")
    // console.log("=============================================================== Server.SECONDARY_PREFERRED")
    // If we have tags, look for servers matching the specific tag
    if(tags != null && typeof tags == 'object') {
      // console.log("=============================================================== Server.SECONDARY_PREFERRED with tags")
  ...
  } else if(finalReadPreference == Server.NEAREST && this.strategyInstance != null) {
    // console.log("=================================================================================== Server.NEAREST")
    // console.log("=============================================================== Server.NEAREST")
    connection = this.strategyInstance.checkoutSecondary(tags);
  } else if(finalReadPreference == Server.NEAREST && this.strategyInstance == null) {
    // console.log("=================================================================================== Server.NEAREST NO STRATEGY")
    return new Error("A strategy for calculating nearness must be enabled such as ping or statistical");
  } else if(finalReadPreference == Server.READ_SECONDARY_ONLY && Object.keys(this._state.secondaries).length == 0) {
    // console.log("=================================================================================== Server.READ_SECONDARY_ONLY NO SECONDARIES")
    if(tags != null && typeof tags == 'object') {
      var preferenceName = finalReadPreference == Server.SECONDARY ? 'secondary' : finalReadPreference;
      connection = new Error("No replica set member available for query with ReadPreference " + preferenceName + " and tags " + JSON.stringify(tags));      
    } else {
...
ReplSet.prototype.close = function(callback) {
  // if(this._serverState == 'disconnected') {
