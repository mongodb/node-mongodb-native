 ...
 * @ignore
 */
var pickFirstConnectedSecondary = function pickFirstConnectedSecondary(self) {
  var keys = Object.keys(self._state.secondaries);
  var connection = null;
...
ReplSet.prototype.checkoutReader = function(readPreference, tags) {
  ...
  if((this.readSecondary == true || finalReadPreference == Server.READ_SECONDARY || finalReadPreference == Server.READ_SECONDARY_ONLY) && Object.keys(this._state.secondaries).length > 0) {
    ...
    if(this.strategyInstance != null) {
      ...
      connection = connection == null ? pickFirstConnectedSecondary(this) : connection;
    } else {
      // Pick a random key
    ...
    if(connection == null) {
      ...
      // If we have a secondary strategy uses that one but passes in the tags
      if(this.strategyInstance != null) {
        ...
        this._currentServerChoice = this._currentServerChoice % keys.length;
        var key = keys[this._currentServerChoice++];
        // console.log("=============================================================== Server.PRIMARY_PREFERRED :: SECONDARY : 1")
  ...
  } else if(finalReadPreference == Server.NEAREST && this.strategy == 'ping') {
