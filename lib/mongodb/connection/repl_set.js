...
ReplSet.prototype._enableHA = function () {
  ...
  function ping () {
    ...
    var addresses = Object.keys(self._state.addresses);
    if(0 === addresses.length || 0 === self.dbInstances.length) {
      // TODO try again?
      return;
    }

    self.updateHealthServerTry++;
    if(self.updateHealthServerTry >= addresses.length) {
      self.updateHealthServerTry = 0;
    }

    var selectedServer = self._state.addresses[addresses[self.updateHealthServerTry]];
    ...
    _server.connect(db, function(err, result) {
      if(err) {
