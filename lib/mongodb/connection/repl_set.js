...
var _connectHandler = function(self, candidateServers, instanceServer) {
  return function(err, result) {
    console.log("=========================================== connect :: " + instanceServer.port)
    ...
    if(candidateServers.length > 0) {
      ...
      // console.dir(server.socketOptions)
      ...
      if(self._state.master == null) {
        ...
        console.log("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ 2")
        self.emit("fullsetup", null, self.db);        
        self.emit("open", null, self.db);
      }
    }    
  }
}
...
ReplSet.prototype.connect = function(parent, options, callback) {
  ...
  // Just save up callbacks if we are already connecting
  if(this._serverState == 'connecting') {
    if(typeof callback == 'function') this._callbackList.push(callback);
    return;
  }
  ...
  this.once("fullsetup", function() {
    ...
    self.db.emit("fullsetup", null, self.db);
    // Callback
    if(typeof callback == 'function') callback(null, parent, self);
  });
  
  this.once("connectionError", function(err) {
    self._serverState = 'disconnected';
    if(typeof callback == 'function') callback(err, parent, self);    
  });
  ...
  // var validServerConnections = [];
  // 
  // // Ensure we have all registered servers in our set
  // for(var i = 0; i < serverConnections.length; i++) {
  //   if(replSetSelf._state.addresses[serverConnections[i].host + ':' + serverConnections[i].port] == null) {
  //     replSetSelf._state.addresses[serverConnections[i].host + ':' + serverConnections[i].port] = serverConnections[i];
  //     validServerConnections.push(serverConnections[i]);
  //   }
  // }
  // 
  // // Set the valid set
  // serverConnections = validServerConnections;
  // 
