...
var ConnectionPool = exports.ConnectionPool = function(host, port, poolSize, bson, socketOptions) {
  ...
  this.socketOptions.poolSize = poolSize + 1;
...
ConnectionPool.prototype.checkoutConnection = function(id) {
  ...
  var keys = Object.keys(this.openConnections);
  return this.openConnections[(keys[(this.currentConnectionIndex++ % (keys.length - 1))])]
}

ConnectionPool.prototype.checkoutStatsConnection = function() {
  // Get all connection entry keys
  var connectionKeys = Object.keys(this.openConnections);
  // Pop the last one off (that's the one we want)
  return this.openConnections[connectionKeys.pop()];
}

ConnectionPool.prototype.getAllConnections = function() {
  // Get all keys
  var allKeys = Object.keys(this.openConnections);
  var allConnections = new Array(allKeys.length - 1);
  // Pop last one off as we are not returning the "stats connection"
  allKeys.pop();
