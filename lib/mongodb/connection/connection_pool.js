...
var connectHandler = function(self) {
  return function(err, connection) {
    if(err != null) {
      // require('util').debug("================================================= connectHandler :: 0")
      // require('util').debug(require('util').inspect(err))
      // require('util').debug(require('util').inspect(self.waitingToOpen))
      // console.log("================================================= connectHandler :: 0")
      // console.dir(err)      
      // console.log(self.waitingToOpen)
    }
    ...
    // require('util').debug("++++++++++++++++++++++++++++++++ connection.id = " + connection.id);
    ...
    if(err && Object.keys(self.waitingToOpen).length > 0 && self.openConnections[connection.id] == null && self.connectionsWithErrors[connection.id] == null) {
      ...
      // require('util').debug("self.waitingToOpen = " + Object.keys(self.waitingToOpen).length);
...
ConnectionPool.prototype.isConnected = function() {
  return Object.keys(this.waitingToOpen).length == 0 
    ...
    && Object.keys(this.openConnections).length > 0
}
