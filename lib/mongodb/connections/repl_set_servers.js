...
ReplSetServers.prototype.connect = function(parent, callback) {
      ...
      var connectCallback = function(err, reply) {
        ...
        if(reply.documents[0].hosts != undefined) {
          ...
          if(node["secondary"]) {
            ...
            // debug("===================================================== ismaster")
            // debug("  replSetSelf.replicaSet = " + replSetSelf.replicaSet)
            // debug("  node[\"setName\"] = " + node["setName"])
        ...
        if(numberOfConnectedServers == serverConnections.length && (parent.state == 'notConnected')) {
          ...
          // debug(inspect(replSetSelf.otherErrors))
  ...
  // debug("================================================= serverConnections")
  // debug(inspect(serverConnections))
...
ReplSetServers.prototype.checkoutReader = function() {
  // debug("============================================================ secondaries")
  // debug(inspect(this.secondaries))
  ...
  if(this.readSecondary && this.secondaries.length > 0) {
    // debug("=========================================================== checkout secondary reader")
    var server = this.secondaries[Math.floor(Math.random() * this.secondaries.length)];
    // debug("======================================== read pool")
    // debug(" server instance = " + server);
    // debug(" server = " + server.connection.host + ":" + server.connection.port);
    
    return server.connection;
    // return this.secondaries[Math.floor(Math.random()*(this.secondaries.length + 1))].connection;
    // var con = this.secondaries[0].connection
    // debug(inspect(con));
    // for(var i = 0; i < this.secondaries.length; i++) {
    //   var s = this.secondaries[i];
    //   if(s.port == 30002) {
    //     debug("=================================== sending out server :: " + s.port)
    //     return s.connection;
    //   }
    // }
  } else {
