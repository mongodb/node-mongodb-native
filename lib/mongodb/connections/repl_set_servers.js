...
ReplSetServers.prototype.connect = function(parent, options, callback) {
  ...
  var connectionHandler = function(instanceServer) {
    return function(err, result) {
      ...
      if(err == null && result.documents[0].hosts != null) {
        ...
        for(var i = 0; i < candidateServers.length; i++) {
          ...
          if(replSetSelf._state.addresses[candidateServerString] == null) {            
            ...
            newServer.connect(parent, {firstCall:true, returnIsMasterResults: true}, connectionHandler(newServer));
          }        
        }                
      } else {        
      }
      ...
      //   if(replSetSelf.replicaSet == null) {
      //     replSetSelf._state.setName = setName;          
      //   } else if(replSetSelf.replicaSet != setName && replSetSelf._state.errorMessages.length == 0) {
      //     replSetSelf._state.errorMessages.push(new Error("configured mongodb replicaset does not match provided replicaset [" + setName + "] != [" + replSetSelf.replicaSet + "]"))
      //     // Ensure no more callbacks
      //     numberOfServersLeftToInitialize = -1;
      //     // Return the callback
      //     return callback(replSetSelf._state.errorMessages[0], parent);
      //   }
      // 
      ...
      //   if(replSetSelf._state.errors[me] != null) {
      ...
      //     var candidateServerString = candidateServers[i];
      //   
      //     debug("---------------------------- candidateServer :: " + candidateServerString)
      //     inspect(replSetSelf._state.addresses)
      //     // debug("------------------------------------------------------------- ADDRESSES")
      //     // inspect(Object.keys(replSetSelf._state.addresses));
      //   
      //     // Skip this server if it's alreay defined
      //     if(replSetSelf._state.addresses[candidateServerString] != null) continue;
      //   
      //     // Split the server string
      //     var parts = candidateServerString.split(/:/);
      //     if(parts.length == 1) {
      ...
      //   }
      // } else {
      //   // Remove the server from the list of approved servers
      //   delete replSetSelf._state.addresses[instanceServer.host + ":" + instanceServer.port];
      //   // Add to list of error servers
      //   replSetSelf._state.errors[instanceServer.host + ":" + instanceServer.port] = instanceServer;
      //   // Close the instance Server
      //   instanceServer.close();
      // }
      // 
      // debug("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ numberOfServersLeftToInitialize :: " + numberOfServersLeftToInitialize)
      // 
      // if(numberOfServersLeftToInitialize == 0) {
      //   // debug("---------------------------------------------------------- callback")
      //   // inspect(replSetSelf._state.errors)
      //   // inspect(replSetSelf._state.errorMessages)
      // 
  ...
  for(var i = 0; i < serverConnections.length; i++) {
    serverConnections[i].connect(parent, {'firstCall':true, returnIsMasterResults: true}, connectionHandler(serverConnections[i]));
  }  
}
...
ReplSetServers.prototype.close = function(callback) {
  // debug("-------------------------------------------------------------- ReplSetServers :: close")  
  
  var self = this;  

  // debug("---------------------------------------------------------------- arbiters")
  // inspect(self._state.arbiters)
  // debug("---------------------------------------------------------------- secondaries")
  // inspect(self._state.secondaries)
  // debug("---------------------------------------------------------------- passives")
  // inspect(self._state.passives)
  ...
  for(var i = 0; i < allServers.length; i++) {
    // debug("-------------------------------------------------------------- ReplSetServers :: close :: 3")  
    var server = allServers[i];
    // inspect(server)
    // Close each server
    server.close(function() {
      // debug("-------------------------------------------------------------- ReplSetServers :: close :: 4")  
      ...
      if(numberOfServersToClose == 0 && typeof callback === 'function') {
        // debug("-------------------------------------------------------------- ReplSetServers :: close :: 5")  
        callback(null);
      } else if(numberOfServersToClose == 0) {
        // debug("-------------------------------------------------------------- ReplSetServers :: close :: 6")  
        self.emit("close");
      }
    })
  }
