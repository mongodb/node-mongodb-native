...
var insert = function insert (docs, options, callback) {
  ...
  if(connection && connection.serverCapabilities.hasWriteCommands) {
...
var insertWithWriteCommands = function(self, docs, options, callback) {
  ...
  var namespace = options['dbName'] == null ? self.db.databaseName : options['dbName'];
  namespace = namespace + "." + self.collectionName;
  ...
  self.db.command(write_command
    ...
    , function(err, result) {  
      ...
      if(!result.ok 
        && (result.err != null || result.errMessage != null))
...
var removeWithWriteCommands = function(self, selector, options, callback) {
  ...
  var top = options.single == true ? 0 : 1;
...
var remove = function remove(selector, options, callback) {
  ...
  var connection = this.db.serverConfig.checkoutWriter();
  // If we support write commands let's perform the insert using it
  if(connection.serverCapabilities.hasWriteCommands) {
...
var backWardsCompatibiltyResults = function(result) {
  var n = result.n + result.upserted;
  var updatedExisting = result.upserted == 0 ? true : false;  
  return {ok: true, n: n, updatedExisting: updatedExisting}
}
