...
var hasWriteCommands = function(connection) {
...
var insert = function insert (docs, options, callback) {
  ...
  if(hasWriteCommands(connection) 
...
var insertWithWriteCommands = function(self, docs, options, callback) {
  ...
  self.db.command(write_command
    , {connection:connection, checkKeys: checkKeys, serializeFunctions: serializeFunctions}
    , function(err, result) {  
...
var removeWithWriteCommands = function(self, selector, options, callback) {
  ...
  self.db.command(write_command
    ...
    , function(err, result) {  
      ...
      var r = backWardsCompatibiltyResults(result);
      // Return the results for a whole batch
      callback(null, r.n, r)
  });
}
...
var remove = function remove(selector, options, callback) {
  ...
  var connection = this.db.serverConfig.checkoutWriter();
  // If we support write commands let's perform the insert using it
  if(hasWriteCommands(connection) && !Buffer.isBuffer(selector)) {
...
var backWardsCompatibiltyResults = function(result) {
  // Upserted
  var upsertedValue = null;

  // Generate backwards compatible result
  var n = result.n;
  if(Array.isArray(result.upserted)) {
    n += result.upserted.length;
    upsertedValue = result.upserted[0];
  }
  
  if(!Array.isArray(result.upserted) && result.upserted != null) 
    n += 1;  

  // Did we update an existing element
  var updatedExisting = Array.isArray(result.upserted) ? false : true;
  ...
  // Final result
  var finalResult = {ok: true, n: n, updatedExisting: updatedExisting};
...
var update = function update(selector, document, options, callback) {
  ...
  var connection = this.db.serverConfig.checkoutWriter();
  // If we support write commands let's perform the insert using it
  if(hasWriteCommands(connection) && !Buffer.isBuffer(selector) && !Buffer.isBuffer(document)) {
