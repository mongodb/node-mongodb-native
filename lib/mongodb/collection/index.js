...
var transformIndexes = function(results) {
...
var listIndexes = function listIndexes(options) {
  ...
  if(connection && connection.serverCapabilities.hasListIndexesCommand) {
    // Cursor options
    var cursor = options.batchSize ? {batchSize: options.batchSize} : {}
    // Build the command
    var command = { listIndexes: this.collectionName, cursor: cursor };
    // Get the command cursor
    return new CommandCursor(this.db, this.db.databaseName + ".$cmd", command, {transform:transformIndexes});
  }
