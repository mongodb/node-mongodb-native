...
var mergeBatchResults = function(ordered, batch, bulkResult, err, result) {
  ...
  if(batch.batchType == UPDATE) {
    var nModified = result.nModified ? result.nModified : 0;
...
var mergeLegacyResults = function(_ordered, _op, _batch, _results, _result, _index) {
  ...
  if(_result.errmsg || _result.err || _result instanceof Error) {
    ...
    if(_result.wnote || _result.jnote || _result.code == 16544 || _result.code == 13) {
      _results.ok = 0;
    ...
    if(_result.upserted || (!_result.updatedExisting && _result.upserted == null)) {
      ...
      _results.nModified = _results.nModified + _result.n;
     }
