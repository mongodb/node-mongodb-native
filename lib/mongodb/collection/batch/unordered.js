...
var UnorderedBulkOperation = function(collection, options) {	
  ...
  var writeConcern = shared._getWriteConcern(collection, options);
	...
	var addToOperationsList = function(docType, document) {
		...
		if(currentBatch.size > maxBatchSizeDocuments 
			|| currentBatch.size + size > maxBatchSizeDocuments) {
	...
	this.execute = function(callback) {		
		...
		var executeBatch = function(_batch, _callback) {
			...
			// Execute the command
			db.command(command, {readPreference:'primary'}, function(err, result) {
				// console.log("==============================================")
				// console.dir(err)
				// console.dir(result)
...
// 		documents.writeConcern = writeConcern;
// 		documents.collection = collection;
// 		documents.index = 0;

// 		// Run in backward compatibility mode
// 		backwardsCompatibilityExecuteCommands(documents, callback);
// 	}

// 	//
// 	// Execute backward compatibility mode
// 	var backwardsCompatibilityExecuteCommands = function(documents, callback) {
// 		// Total number of batches to execute
// 		var totalNumberToExecute = documents[INSERT][0].length == 0 ? 0 : documents[INSERT].length;
// 		totalNumberToExecute += documents[UPDATE][0].length == 0 ? 0 : documents[UPDATE].length;
// 		totalNumberToExecute += documents[REMOVE][0].length == 0 ? 0 : documents[REMOVE].length;
...
// 		// Execute all the updates in backward compatible mode
// 		if(documents[UPDATE].length >= 1 && documents[UPDATE][0].length > 0) {
// 			executeOutOfOrderUpdatesLegacy(documents[UPDATE], handler(documents[UPDATE]));
// 		}
// 	}

// 	// Execute out of order updates
// 	var executeOutOfOrderUpdatesLegacy = function(batches, callback) {
// 		// var totalBatches = docs.length;
// 		// var errors = [];
// 		// var results = {type: docType, results: []};
...
// 		// Go through all the document batches
// 		for(var i = 0; i < batches.length; i++) {
// 			var batch = batches[i];
// 			console.dir(docs);
// 		}
