...
exports.Long.prototype.toNumber = function() {
  // require('sys').debug("JS:: =============================================================== TO NUMBER");
  // require('sys').debug("JS:: ====================================== low_bits: " + this.low_);
  // require('sys').debug("JS:: ====================================== low_bits unsigned: " + this.getLowBitsUnsigned());
  // require('sys').debug("JS:: ====================================== high_bits: " + this.high_);
  // require('sys').debug("JS:: --------------------------------------------------------------- TO NUMBER");
...
exports.Long.prototype.toString = function(opt_radix) {
  ...
  if (this.isNegative()) {
    if (this.equals(exports.Long.MIN_VALUE)) {
      ...
      // require('sys').debug("JS:: ======================== ToString:radixLong:START1 " + radixLong.toNumber())    
      // require('sys').debug("JS:: ======================== ToString:l:START1 " + this.toNumber())    
      var div = this.div(radixLong);
      // require('sys').debug("JS:: ======================== ToString:div:START2 " + div.toNumber())    
      var rem = div.multiply(radixLong).subtract(this);
      // require('sys').debug("JS:: ======================== ToString:rem:START3 " + rem.toNumber())    
      var a = div.toString(radix) + rem.toInt().toString(radix)
      // require('sys').debug("JS:: ======================== ToString:rem:START4 " + a)    
      return a;
    } else {
      // require('sys').debug("JS:: ======================== ToString:rem:START5 " + ('-' + this.negate().toString(radix)))    
  ...
  // require('sys').debug("JS:: ======================== radixToPower: " + require('sys').inspect(radixToPower))

  var rem = this;
  var result = '';
  while (true) {
    // require('sys').debug("JS:: ======================== rem: " + rem.toNumber())    
    var remDiv = rem.div(radixToPower);
    // require('sys').debug("======================================================================= DONE");
    // require('sys').debug("JS:: ======================== remDiv: " + remDiv.toNumber())    
    
    var intval = rem.subtract(remDiv.multiply(radixToPower)).toInt();
    var digits = intval.toString(radix);

    // require('sys').debug("================================== intval: " + intval);
    // require('sys').debug("================================== digits: " + digits);

    rem = remDiv;
    if (rem.isZero()) {
      // require('sys').debug("================================== digits + result: " + (digits + result));
      return digits + result;
    } else {
      while (digits.length < 6) {
        // require('sys').debug("=================== add zeros: " + (6 - digits.length));        
        digits = '0' + digits;
      }
...
exports.Long.prototype.div = function(other) {
  // require('sys').debug("======================================= result: [" + this.toNumber() + "/" + other.toNumber() + "]");    
  // require('sys').debug("======================================= DIV:0")
  if (other.isZero()) {
    // require('sys').debug("======================================= DIV:0-1")
    throw Error('division by zero');
  } else if (this.isZero()) {
    // require('sys').debug("======================================= DIV:0-2")
    return exports.Long.ZERO;
  }
  
  if (this.equals(exports.Long.MIN_VALUE)) {
    // require('sys').debug("======================================= DIV:2")
    if (other.equals(exports.Long.ONE) ||
        other.equals(exports.Long.NEG_ONE)) {
          // require('sys').debug("======================================= DIV:3")
      return exports.Long.MIN_VALUE;  // recall that -MIN_VALUE == MIN_VALUE
    } else if (other.equals(exports.Long.MIN_VALUE)) {
      // require('sys').debug("======================================= DIV:4")
      return exports.Long.ONE;
    } else {
      // require('sys').debug("======================================= DIV:5")
  ...
  if (this.isNegative()) {
    ...
    if (other.isNegative()) {
      // require('sys').debug("======================================= DIV:5-1")
      // require('sys').debug("======================================= NEGATIVE 1-1")
      return this.negate().div(other.negate());
    } else {
      // require('sys').debug("======================================= DIV:5-2")
      // require('sys').debug("======================================= NEGATIVE 1-2")
      // require('sys').debug("======================================= this: " + this.toNumber())
      // require('sys').debug("======================================= this: " + this.toNumber())
      // require('sys').debug("======================================= this.negate(): " + this.negate().toNumber())
      // require('sys').debug("======================================= this.negate().div(other): " + this.negate().div(other).toNumber())
      // require('sys').debug("======================================= this.negate().div(other).negate(): " + this.negate().div(other).negate().toNumber())
      return this.negate().div(other).negate();
    }
  } else if (other.isNegative()) {
    // require('sys').debug("======================================= DIV:6")
    // require('sys').debug("======================================= DIV:8")
  ...
  while (rem.greaterThanOrEqual(other)) {
    // require('sys').debug("----------------------------------------------------------------------------------");
    // require('sys').debug("JS:: ======================================== DIV:9");
    // Approximate the result of division. This may be a little greater or
    // smaller than the actual value.
    
    // require('sys').debug("JS:: ======================================== rem.toNumber(): " + rem.toNumber());
    // require('sys').debug("JS:: ======================================== other.toNumber(): " + other.toNumber());
    var a = rem.toNumber();
    var b = other.toNumber();
    var c = a / b;
    // require('sys').debug("JS:: ======================================== rem.toNumber(): " + a);
    // require('sys').debug("JS:: ======================================== other.toNumber(): " + b);
    // require('sys').debug("JS:: ======================================== rem.toNumber() / other.toNumber(): " + c);
    // require('sys').debug("JS:: ======================================== floor(c): : " + Math.floor(c));
    // require('sys').debug("JS:: ======================================== max(1, floor(c)): " + Math.max(1, Math.floor(c)));
    
    // var approx = Math.max(1, Math.floor(rem.toNumber() / other.toNumber()));
    var approx = Math.max(1, Math.floor(a / b));
    // require('sys').debug("JS:: ======================================== DIV:10");
    
    // require('sys').debug("JS:: ===================== rem.toNumber(): " + rem.toNumber());
    // require('sys').debug("JS:: ===================== other.toNumber(): " + other.toNumber());
    // require('sys').debug("JS:: ===================== approx: " + approx);
    // require('sys').debug("JS:: ===================== floor: " + Math.floor(rem.toNumber() / other.toNumber()));

    // We will tweak the approximate result by changing it in the 48-th digit or
    // the smallest non-fractional digit, whichever is larger.
    var log2 = Math.ceil(Math.log(approx) / Math.LN2);
    // require('sys').debug("JS:: ===================== log2: " + log2);
    var delta = (log2 <= 48) ? 1 : Math.pow(2, log2 - 48);
    // require('sys').debug("JS:: ===================== delta: " + delta);

    // Decrease the approximation until it is smaller than the remainder.  Note
    // that if it is too large, the product overflows and is negative.
    var approxRes = exports.Long.fromNumber(approx);
    var approxRem = approxRes.multiply(other);
    // require('sys').debug("JS:: ===================== approxRes: " + approxRes.toNumber());
    // require('sys').debug("JS:: ===================== approxRem: " + approxRem.toNumber());
    // require('sys').debug("JS:: ===================== rem: " + rem.toNumber());
    // require('sys').debug("JS:: ===================== approxRem.isNegative(): " + approxRem.isNegative());
    // require('sys').debug("JS:: ===================== approxRem.greaterThan(rem): " + approxRem.greaterThan(rem));
    ...
    // require('sys').debug("JS:: ===================== res: " + res.toNumber());    
    rem = rem.subtract(approxRem);
  }
  
  // require('sys').debug("======================================= DIV:1")
  // require('sys').debug("======================================= RESULT:: [" + this.toNumber() + "/" + other.toNumber() + "] = [" + res.toNumber() + "]");
