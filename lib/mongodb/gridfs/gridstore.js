...
GridStore.read = function(callback, db, name, length, offset) {
  ...
  gridStore.open(function(gridStore) {    
    if(offset != null) {
      // sys.puts("-------------------------------------------- gridStore:before:seek");
      gridStore.seek(function(gridStore) {
        // sys.puts("-------------------------------------------- gridStore:seek");
...
GridStore.prototype.read = function(callback, length, buffer) {
  ...
  // sys.puts("----------------- finalLength: " + finalLength);
  // sys.puts("----------------- position: " + self.position);
  
  // Check that we have enough data in our current chunk
  while(numberToRead > 0) {
    // sys.puts("----------------------------- self.currentChunk.length(): " + self.currentChunk.length());
    // sys.puts("----------------------------- self.currentChunk.position: " + self.currentChunk.position);
    // sys.puts("----------------------------- finalLength: " + finalLength);
    // sys.puts("----------------------------- (self.currentChunk.length() - self.currentChunk.position - 1):" + (self.currentChunk.length() - self.currentChunk.position));

    if((self.currentChunk.length() - self.currentChunk.position + 1) >= finalLength) {
      finalBuffer = self.currentChunk.read(finalLength);
...
GridStore.prototype.seek = function(callback, position, seekLocation) {
  ...
  if(seekLocationFinal == GridStore.IO_SEEK_CUR) {
    targetPosition = self.position + position;
  } else if(seekLocationFinal == GridStore.IO_SEEK_END) {
    targetPosition = self.length + position;
  } else {
