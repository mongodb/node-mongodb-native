...
GridStore.prototype.read = function(length, buffer, callback) {
  ...
  if((self.currentChunk.length() - self.currentChunk.position + 1 + finalBuffer.length) >= finalLength) {
    ...
    // debug("============= numberToRead :: " + numberToRead)
    // debug(finalBuffer.length)
  ...
  // var currentChunkPosition = (self.currentChunk.chunkNumber * self.chunkSize) + self.currentChunk.position;
  // 
  // // debug("=============== number :: " + self.currentChunk.chunkNumber)  
  // if(currentChunkPosition < finalLength) {
  //   debug("================================ finallength :: " + finalLength)
  //   // Read the data available
  //   finalBuffer = finalBuffer + self.currentChunk.read(finalLength);
