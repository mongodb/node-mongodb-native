...
Chunk = function(file, mongoObject) {  
  ...
  if(mongoObjectFinal.get('data') == null) {
    ...
    for(var i = 0; i < string.length; i++) { dataArray.push(this.parser.fromByte(string.charCodeAt(i)));}
...
Chunk.prototype.read = function(length) {
  // sys.puts("---------------------------------------- chunk:length " + this.length());
  // sys.puts("---------------------------------------- chunk:position " + this.position);
...
Chunk.prototype.eof = function() {
  // sys.puts("----------------------------------------- chunk:position " + this.position);
  // sys.puts("----------------------------------------- chunk:length " + this.length());
  // sys.puts(sys.inspect(this.data));
