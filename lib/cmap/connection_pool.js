...
class ConnectionPool extends EventEmitter {
  ...
  get totalConnectionCount() {
    return this[kConnections].length + this[kPending];
  }
...
function createConnection(pool, callback) {
  if (pool[kPending] === MAX_CONNECTING) {
    return;
  }
  ...
  pool[kPermits]--;
  pool[kPending]++;
  pool.emit('connectionCreated', new ConnectionCreatedEvent(pool, { id: connectOptions.id }));

  connect(connectOptions, pool[kCancellationToken], (err, connection) => {
    pool[kPending]--; // always decrement pending
...
function processWaitQueue(pool) {
  ...
  if (
    pool.waitQueueSize &&
    (maxPoolSize <= 0 || pool[kPermits] > 0) &&
    pool[kPending] < MAX_CONNECTING
  ) {
    createConnection(pool, (err, connection) => {
      ...
      // continue to process if there are more requests and not enough pending connections
      if (pool.waitQueueSize && pool.waitQueueSize > pool[kPending]) {
        setImmediate(() => processWaitQueue(pool));
      }
    });
