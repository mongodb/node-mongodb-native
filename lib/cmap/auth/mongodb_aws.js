...
const { MongoCredentials } = require('./mongo_credentials');
...
class MongoDBAWS extends AuthProvider {
  _authenticateSingleConnection(sendAuthCommand, connection, credentials, callback) {
    ...
    crypto.randomBytes(32, (err, nonce) => {
      ...
      sendAuthCommand(connection, `${db}.$cmd`, saslStart, (err, res) => {
        ...
        sendAuthCommand(connection, `${db}.$cmd`, saslContinue, err => {
          if (err) return callback(err);
          callback();
        });
      });
  ...
  auth(sendAuthCommand, connections, credentials, callback) {
    ...
    if (credentials.username == null) {
      makeTempCredentials(credentials, (err, tempCredentials) => {
        if (err) return callback(err);
        super.auth(sendAuthCommand, connections, tempCredentials, callback);
      });
