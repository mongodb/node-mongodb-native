...
class Server extends EventEmitter {
  ...
  killCursors(ns, cursorState, callback) {
    ...
    this.s.pool.withConnection((err, conn, cb) => {
      ...
      conn.killCursors(ns, cursorState, makeOperationHandler(this, conn, null, cb));
    }, callback);
  }
  ...
  return function handleOperationResult(err, result) {
    if (err && !connectionIsStale(server.s.pool, connection)) {
      if (err instanceof MongoNetworkError) {
        ...
        if (err.topologyVersion) {
          handleStateChangeError(server, err);
          return;
        }
...
function handleStateChangeError(server, err) {
  ...
  if (etv == null) {
    ...
    server.requestCheck();
  } else if (stv == null || !etv.processId.equals(stv.processId)) {
    server.emit('error', err);
    server[kMonitor].close();
    server[kMonitor].connect();
  } else if (etv.processId.equals(stv.processId) && etv.counter > stv.counter) {
    server.emit('error', err);
  } else {
    // this is a duplicate error from a server event that was already processed or
    // the monitor already processed a newer isMaster response.
  }
}
