...
const kAwaitable = Symbol('awaitable');
...
class Monitor extends EventEmitter {
  ...
  requestCheck() {
    // there is no such thing as an immediate check for awaitable ismaster
    if (this[kAwaitable]) {
      return;
    }
...
function checkServer(monitor, callback) {
  ...
  connect(monitor.connectOptions, monitor[kCancellationToken], (err, conn) => {
    ...
    if (conn.ismaster.topologyVersion) {
      monitor[kAwaitable] = true;
    }
...
function monitorServer(monitor) {
  ...
  const minHeartbeatFrequencyMS = monitor.options.minHeartbeatFrequencyMS;
  checkServer(monitor, e0 => {
    if (e0 == null) {
      if (monitor[kAwaitable]) {
        // clear the timer in case we are switching back to awaitable ismaster
        clearTimeout(monitor[kMonitorId]);

        monitorServerWithStreaming(monitor);
        return;
      }
    ...
    // wait minHeartbeatFrequencyMS to avoid flooding the server with connections and/or
    // isMaster requests. Reuse the `kMonitorId` for this so it's properly cleaned up on
    // close
    monitor[kMonitorId] = setTimeout(
      () =>
        checkServer(monitor, e1 => {
          let msUntilNextCheck = heartbeatFrequencyMS;
          if (e1) {
            if (e1 instanceof MongoNetworkError) {
              monitor.emit('resetConnectionPool');
            } else {
              // this is a command error, do not clear the pool and only wait `minHeartbeatFrequencyMS`
              msUntilNextCheck = minHeartbeatFrequencyMS;
            }

            monitor.emit('resetServer', e1);
          }

          rescheduleMonitoring(monitor, msUntilNextCheck);
        }),
      minHeartbeatFrequencyMS
    );
  });
...
function monitorServerWithStreaming(monitor) {
  const heartbeatFrequencyMS = monitor.options.heartbeatFrequencyMS;
  const connectTimeoutMS = monitor.options.connectTimeoutMS;
  const maxAwaitTimeMS = 10000; // TODO: make this configurable
  const topologyVersion = makeTopologyVersion(monitor[kServer].description.topologyVersion);

  monitor[kConnection].command(
    'admin.$cmd',
    { ismaster: true, maxAwaitTimeMS, topologyVersion },
    { socketTimeout: connectTimeoutMS + maxAwaitTimeMS, exhaustAllowed: true },
    (err, result) => {
      if (err) {
        monitor.emit(
          'serverHeartbeatFailed',
          new ServerHeartbeatFailedEvent(undefined, err, monitor.address)
        );

        // close the connection
        monitor[kConnection].destroy();

        // wait `heartbeatFrequencyMS` to reschedule awaitable ismaster
        monitor[kMonitorId] = setTimeout(() => monitorServer(monitor), heartbeatFrequencyMS);
        return;
      }
      ...
      monitor.emit(
        'serverHeartbeatSucceeded',
        new ServerHeartbeatSucceededEvent(undefined, isMaster, monitor.address)
      );

      // if this reply omits support for awaitable ismaster, revert to legacy
      // ismaster monitoring
      if (isMaster.topologyVersion == null) {
        monitor[kAwaitable] = false;
        monitorServer(monitor);
        return;
      }

      // at this point we are either awaiting the next response, or reattempting the
      // awaitable ismaster, so we should emit a heartbeat started event
      monitor.emit('serverHeartbeatStarted', new ServerHeartbeatStartedEvent(monitor.address));

      // if the result indicates there is no `moreToCome`, reattempt awaitable ismaster
      if (result.message.moreToCome === false) {
        monitorServerWithStreaming(monitor);
        return;
      }
    }
