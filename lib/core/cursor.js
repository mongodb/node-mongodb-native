...
var nextFunction = function(self, callback) {
  ...
  if (self.cursorState.limit > 0 && self.cursorState.currentLimit >= self.cursorState.limit) {
    ...
    self._getMore(function(err, doc, connection) {
      ...
      // save off the postBatchResumeToken if found
      if (doc && doc.cursor && doc.cursor.postBatchResumeToken) {
        self.cursorState.postBatchResumeToken = doc.cursor.postBatchResumeToken;
      }
    ...
    const queryCallback = (err, r) => {
      ...
      if (
        ...
        if (result.documents[0].cursor != null && typeof result.documents[0].cursor !== 'string') {
          ...
          // store the `postBatchResumeToken` if found
          if (result.documents[0].cursor && result.documents[0].cursor.postBatchResumeToken) {
