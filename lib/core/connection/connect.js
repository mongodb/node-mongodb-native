...
const makeAuthenticatorsMap = require('../auth').makeAuthenticatorsMap;
const MongoCredentials = require('../auth/mongo_credentials').MongoCredentials;
...
function connect(options, cancellationToken, callback) {
  ...
  if (AUTHENTICATORS == null) {
    AUTHENTICATORS = makeAuthenticatorsMap(options.bson);
  }
...
function performInitialHandshake(conn, options, _callback) {
  ...
  if (!(options.auth || options.credentials)) {
    const start = new Date().getTime();
    return runCommand(conn, 'admin.$cmd', handshakeDoc, handshakeOptions, (err, ismaster) => {
      if (err) return callback(err);
      if (ismaster.ok === 0) return callback(new MongoError(ismaster));
      ...
      resolveCompression(conn, compressors, ismaster, options);
  ...
  if (!credentials) {
    let mechanism = options.authMechanism || 'SCRAM-SHA-256';
    if (mechanism.toUpperCase() === 'DEFAULT') {
      mechanism = 'SCRAM-SHA-256';
    }
    credentials = new MongoCredentials({
      mechanism,
      password: options.auth.password,
      source: options.auth.db,
      username: options.auth.username
    });
  }

  let speculativeMechanism = credentials.mechanism;
  if (speculativeMechanism.toUpperCase() === 'DEFAULT') {
    speculativeMechanism = 'SCRAM-SHA-256';
    credentials.mechanism = speculativeMechanism;
  }

  if (!AUTHENTICATORS.has(speculativeMechanism)) {
    return callback(new MongoError(`authMechanism '${speculativeMechanism}' not supported`));
  }

  AUTHENTICATORS.get(speculativeMechanism).prepareHandshake(
    credentials,
    (error, speculativeAuthenticate, authCtx) => {
      if (error) callback(error);
      const start = new Date().getTime();

      handshakeDoc.speculativeAuthenticate = speculativeAuthenticate;
      runCommand(conn, 'admin.$cmd', handshakeDoc, handshakeOptions, (err, ismaster) => {
        ...
        if (!ismaster.arbiterOnly && credentials) {
          if (authCtx) {
            authCtx.speculativeAuthenticate = ismaster.speculativeAuthenticate;
          }
          credentials.resolveAuthMechanism(ismaster);
          return authenticate(conn, credentials, authCtx, callback);
        }
        return callback(undefined, conn);
      });
    }
...
function resolveCompression(connection, compressors, ismaster, options) {
  if (!isModernConnectionType(connection)) {
    // resolve compression
...
function authenticate(conn, credentials, authCtx, callback) {
  const mechanism = credentials.mechanism;
  if (!AUTHENTICATORS.has(mechanism)) {
    callback(new MongoError(`authMechanism '${mechanism}' not supported`));
    return;
  }

  AUTHENTICATORS.get(mechanism).auth(runCommand, [conn], credentials, authCtx, err => {
    if (err) return callback(err);
