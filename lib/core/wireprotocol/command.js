...
function isClientEncryptionSupported(server) {
  return !!(
    server.s.description &&
    server.s.description.maxWireVersion &&
    server.s.description.maxWireVersion >= 8
  );
}
...
function command(server, ns, cmd, options, callback) {
  ...
  if (!isClientEncryptionSupported(server)) {
    return callback(new MongoError('Auto-encryption requires a minimum MongoDB version of 4.2'));
  }
