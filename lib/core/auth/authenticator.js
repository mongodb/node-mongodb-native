...
/**
 * Creates a new Authenticator, which dictates how to authenticate for a given
 * mechanism.
 * @class
 */
class Authenticator {
   ...
   * @param {any} authCtx Context from the (speculative) started auth
  ...
  auth(sendAuthCommand, connections, credentials, authCtx, callback) {
    ...
    const execute = connection => {
      this._authenticateSingleConnection(
        sendAuthCommand,
        connection,
        credentials,
        authCtx,
  ...
  /**
   * If mechanism supports speculative authentication prepare the initial handshake
   *
   * @abstract
   * @param {MongoCredentials} credentials
   * @param {(error?: Error, speculativeAuthenticate?: object, authCtx?: object) => void} callback
   */
