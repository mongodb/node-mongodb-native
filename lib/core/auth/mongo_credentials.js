...
class MongoCredentials {
   ...
   * @param {string} [options.db] See options.source
  ...
  constructor(options) {
    ...
    this.mechanism = (options.mechanism || 'default').toUpperCase();
    this.mechanismProperties = options.mechanismProperties || {};

    if (this.mechanism === 'MONGODB-AWS') {
    ...
    if (this.mechanism === 'GSSAPI' || this.mechanism === 'SSPI') {
      this.mechanism = process.platform === 'win32' ? 'SSPI' : 'GSSAPI';
    }
  }
