...
/**
 * @typedef SCRAMConversationContinuation
 * @property {number} conversationId
 * @property {Buffer|Binary} payload
 * @property {boolean} done
 * @property {1|0} ok
 */

/**
 * Finish the SCRAM conversation after first message to server
 *
 * @param {any} ctx an object with the necessary pieces to run as a callback (for future more functional auth)
 * @param {SCRAMConversationContinuation} response
 */
function continueScram(ctx, response) {
 ...
 */
class ScramSHA extends Authenticator {
  ...
  _executeScram(sendAuthCommand, connection, credentials, nonce, saslStartCmd, callback) {
    ...
    sendAuthCommand(connection, `${db}.$cmd`, saslStartCmd, (error, response) => {
      if (error) return callback(error);
      if (response.ok === 0) return callback(new MongoError(response));
      return continueScram(scramCtx, response);
    });
  }
  ...
  makeSaslStartCommand(credentials, nonce) {
    ...
    const mechanism = this.cryptoMethod === 'sha1' ? 'SCRAM-SHA-1' : 'SCRAM-SHA-256';
