...
function executeScram(cryptoMethod, sendAuthCommand, connection, credentials, nonce, callback) {
  ...
  sendAuthCommand(connection, `${db}.$cmd`, saslStartCmd, (_err, r) => {
    ...
    sendAuthCommand(connection, `${db}.$cmd`, saslContinueCmd, (_err, r) => {
      const err = resolveError(_err, r);
      if (err) {
        return callback(err);
      }
