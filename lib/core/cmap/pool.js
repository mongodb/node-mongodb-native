...
const makeCounter = require('./counter').makeCounter;
...
class ConnectionPool extends EventEmitter {
  constructor(options) {
    ...
    process.nextTick(() => {
      this._emitMonitoringEvent(PoolCreatedEvent);
  ...
  // Public API
  checkOut(callback) {
    this._emitMonitoringEvent(ConnectionCheckOutStarted);

    if (this.s.isClosed) {
      this._emitMonitoringEvent(ConnectionCheckOutFailed, 'poolClosed');
  ...
  checkIn(connection, force, callback) {
    ...
    this._emitMonitoringEvent(ConnectionCheckedInEvent, connection);
  ...
  clear(callback) {
    this.s.generation += 1;
    this._emitMonitoringEvent(PoolClearedEvent);
    callback();
  }
  ...
  close(callback) {
    ...
    this._emitMonitoringEvent(PoolClosedEvent);
    callback();
  }
  ...
  _acquisitionHandler(callback, err, connection) {
    if (!err) {
      ...
      this._emitMonitoringEvent(ConnectionCheckedOutEvent, connection);
    ...
    this._emitMonitoringEvent(ConnectionCheckOutFailed, reason);
  ...
  _createConnection(callback) {
    ...
    this.s.connections.makeAvailable(connection);
    this._emitMonitoringEvent(ConnectionCreatedEvent, connection);
    ...
    connection.connect(err => {
      ...
      this._emitMonitoringEvent(ConnectionReadyEvent, connection);
    });
  ...
  _destroyConnection(connection, reason) {
    this.s.connections.remove(connection);
    this._emitMonitoringEvent(ConnectionClosedEvent, connection, reason);
  ...
  _emitMonitoringEvent() {
    // Node >=6 impl:
    // _emitMonitoringEvent(Ctor, ...args) {
    //   const ev = new Ctor(this, ...args);
    //   this.emit(Ctor.eventType, ev);
    // }
    const args = Array.from(arguments);
    const Ctor = args.shift();
    args.unshift(null, this);
    const BoundCtor = Function.bind.apply(Ctor, args);

    const ev = new BoundCtor();
    this.emit(Ctor.eventType, ev);
  }
}
