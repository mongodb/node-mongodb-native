...
ReplSetState.prototype.pickServer = function(readPreference) {
  ...
  if (readPreference.mode === 'primary' && readPreference.maxStalenessSeconds != null) {
  ...
  if (readPreference.mode === 'nearest' && readPreference.maxStalenessSeconds == null) {
    return pickNearest(this, readPreference);
  } else if (readPreference.mode === 'nearest' && readPreference.maxStalenessSeconds != null) {
...
function pickNearestMaxStalenessSeconds(self, readPreference) {
  ...
  if (self.primary && servers.length === 0 && readPreference.mode !== 'secondaryPreferred') {
    servers.push(self.primary);
  }
...
function pickNearest(self, readPreference) {
  ...
  // If we have a secondaryPreferred readPreference and no server add the primary
  if (servers.length === 0 && self.primary && readPreference.mode !== 'secondaryPreferred') {
    servers.push(self.primary);
  }
