...
const ReadPreference = function(mode, tags, options) {
  this.mode = ReadPreference.validateMode(mode);
...
ReadPreference.fromOptions = function(options) {
  ...
  const rp = ReadPreference.fromLike(readPreference).toJSON();
  if (maxStalenessSeconds) rp.maxStalenessSeconds = options.maxStalenessSeconds;
  if (readPreferenceTags) rp.tags = options.readPreferenceTags;
  return ReadPreference.fromJSON(rp);
};

ReadPreference.fromMode = function(mode) {
  return new ReadPreference(mode);
};

ReadPreference.fromJSON = function(json) {
  if ('mode' in json || 'tags' in json || 'maxStalenessSeconds' in json || 'hedge' in json) {
    const mode = json.mode || json.preference;
    const tags = json.tags;
    const options = {};
    if (json.maxStalenessSeconds) options.maxStalenessSeconds = json.maxStalenessSeconds;
    if (json.hedge) options.hedge = json.hedge;
    return new ReadPreference(mode, tags, options);
  }
  // NOTE: this does not throw / warn if extra properties are present in the object
  throw new TypeError(`Invalid ReadPreference JSON: ${JSON.stringify(json)}`);
};

ReadPreference.fromLike = function(like) {
  if (like instanceof ReadPreference) return like;
  if (typeof like === 'string') return ReadPreference.fromMode(like);
  if (typeof like === 'object' && like != null) return ReadPreference.fromJSON(like);
  throw new TypeError(`Invalid ReadPreference: ${JSON.stringify(like)}`);
};
...
ReadPreference.validateMode = function(mode) {
  // NOTE: spec tests use "Secondary" casing, we need a way to clean up casing and save mode as a standard.
  if (typeof mode === 'string') {
    const lowerMode = mode.toLowerCase();
    if (lowerMode === ReadPreference.PRIMARY.toLowerCase()) return ReadPreference.PRIMARY;
    if (lowerMode === ReadPreference.PRIMARY_PREFERRED.toLowerCase())
      return ReadPreference.PRIMARY_PREFERRED;
    if (lowerMode === ReadPreference.SECONDARY.toLowerCase()) return ReadPreference.SECONDARY;
    if (lowerMode === ReadPreference.SECONDARY_PREFERRED.toLowerCase())
      return ReadPreference.SECONDARY_PREFERRED;
    if (lowerMode === ReadPreference.NEAREST.toLowerCase()) return ReadPreference.NEAREST;
  }
  throw new TypeError(`Invalid ReadPreference mode: ${JSON.stringify(mode)}`);
};
