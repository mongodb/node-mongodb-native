...
ReadPreference.fromOptions = function(options) {
  ...
  const rpOptions = maxStalenessSeconds ? { maxStalenessSeconds } : {};
  const mode =
    (typeof modeOrObject === 'string' && modeOrObject) ||
    (modeOrObject && (modeOrObject.mode || modeOrObject.preference)) ||
    'primary';
  const tags = []
    .concat((modeOrObject && modeOrObject.tags) || [])
    .concat(readPreferenceTags || []);
...
// prettier-ignore
const x = ReadPreference.fromOptions({ readPreference: new ReadPreference('secondary', ['pp:pp'], {maxStalenessSeconds: 3000}), readPreferenceTags: ['ab:cd'] });
console.log(x);
