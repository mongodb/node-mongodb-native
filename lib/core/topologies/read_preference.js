 ...
 * @param {string | string[] | {[key: any]: string}} tags The tags object
...
const ReadPreference = function(mode, tags, options) {
  ...
  const getOptions = options => {
    ...
    const tagsHasMaxStalenessSeconds = typeof options.maxStalenessSeconds !== 'undefined';
    const tagsHasHedge = typeof options.hedge !== 'undefined';
    return tagsHasMaxStalenessSeconds || tagsHasHedge ? options : null;
  };

  options = getOptions(tags) || getOptions(options) || {};
  tags = getOptions(tags) ? [] : ReadPreference.tags(tags);
...
/** flatten read preference tags */
ReadPreference.tags = function(tags) {
...
ReadPreference.fromOptions = function(options) {
  if (!options || !options.readPreference) return null;
  const modeOrObject = options.readPreference;
  const readPreferenceTags = options.readPreferenceTags;
  const maxStalenessSeconds =
    (modeOrObject && modeOrObject.maxStalenessSeconds) || options.maxStalenessSeconds;
  const hedge = modeOrObject && modeOrObject.hedge;
  const rpOptions = {};
  if (maxStalenessSeconds) rpOptions.maxStalenessSeconds = maxStalenessSeconds;
  if (hedge) rpOptions.hedge = hedge;
  const mode =
    (typeof modeOrObject === 'string' && modeOrObject) ||
    (modeOrObject && (modeOrObject.mode || modeOrObject.preference));
  const tags = Array()
    .concat(ReadPreference.tags(modeOrObject && modeOrObject.tags))
    .concat(ReadPreference.tags(readPreferenceTags));
  return new ReadPreference(mode, tags, rpOptions);
};
