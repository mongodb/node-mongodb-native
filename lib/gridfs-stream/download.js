 ...
 * @param {{uploadDate?: number, [sortKey: string]: number}} [options.sort] Optional sort for the file find query
...
class GridFSBucketReadStream extends Readable {
   ...
   *
   * @function
   * @param {GridFSBucketErrorCallback} [callback] called when the cursor is successfully closed or an error occurred.
...
function init(self) {
  ...
  self.s.files.findOne(self.s.filter, findOneOptions, function(error, doc) {
    ...
    if (!doc) {
      ...
      //@ts-ignore TODO:(ts) better types (how should we handle assigning new properties?)
      err.code = 'ENOENT';
