...
const connectSchema = {
  ...
  compression: { type: 'string' }, // alias to compressors
  connectTimeoutMS: { type: 'number', default: 30000 },
  heartbeatFrequencyMS: { type: 'number', default: 60000 },
  j: { type: 'boolean' }, // alias to journal
  secondaryAcceptableLatencyMS: { type: 'number', default: 15 }, // alias to localThresholdMS
  maxIdleTimeMS: { type: 'number' },
  maxPoolSize: { type: 'number', default: 5 }, // alias to poolSize
  maxStalenessSeconds: { type: 'number' },
  readConcernLevel: { type: 'string' }, // alias to readConcern.level
  ...
  w: { type: 'number' },
  wtimeout: { type: 'number' }, // alias to wTimeoutMS
  ...
  validateOptions: { type: 'boolean', default: false },
 ...
 * @param {boolean} [options.serverSelectionTryOnce] Scan the topology only once after a server selection failure instead of repeatedly until the server selection times out
...
function MongoClient(url, options) {
  ...
  options = options || {};
  validate(connectSchema, options, { optionsValidationLevel: this.optionsValidationLevel });
