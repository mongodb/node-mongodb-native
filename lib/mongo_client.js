...
MongoClient.prototype.db = function(dbName, options) {
  ...
  // If no topology, set to a function that will provide a callback with topology when ready.
  // Can be called multiple times, and callbacks will be executed in the order they were called.
  if (!this.topology) {
    let onConnect = [];
    let connecting = false;
    this.topology = callback => {
      onConnect.push(callback);
      if (!connecting) {
        connecting = true;
        this.connect(err => onConnect.forEach(cb => cb(err, this.topology)));
      }
    };
  }
...
MongoClient.prototype.isConnected = function(options) {
  ...
  if (!this.topology || typeof this.topology === 'function') return false;
