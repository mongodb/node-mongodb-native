...
var mergeOptions = function(target, source, flatten) {
  for(var name in source) {
    if(source[name] && typeof source[name] == 'object' && flatten) {
      // console.log("====== mergeOptions 0 :: " + name)
      target = mergeOptions(target, source[name], flatten);
    } else {
      // console.log("====== mergeOptions 1 :: " + name)
...
var createUnifiedOptions = function(finalOptions, options) {
  ...
  for(var name in options) {
    if(childOptions.indexOf(name.toLowerCase()) != -1) {
      // console.log("====== createUnifiedOptions 0 :: " + name)
      finalOptions = mergeOptions(finalOptions, options[name], false);
    } else {
      if(options[name] && typeof options[name] == 'object' && !Buffer.isBuffer(options[name]) && !Array.isArray(options[name])) {
        // console.log("====== createUnifiedOptions 1 :: " + name)
        // console.log("Buffer.isBuffer(options[name] = " + Buffer.isBuffer(options[name]))
        // console.log("Array.isArray(options[name] = " + Array.isArray(options[name]))
        finalOptions = mergeOptions(finalOptions, options[name], true);
      } else {
        // console.log("====== createUnifiedOptions 2 :: " + name)
        ...
        if(totalNumberOfServers == 0) {
          ...
          if(totalNumberOfMongodServers == 0
            ...
            if(totalNumberOfMongodServers > 0) {
              ...
              // console.log("!!!!!!!!!!!!!!!!!!! MongoClient final servers")
              // console.dir(finalServers.map(function(x) { return x.port}))
