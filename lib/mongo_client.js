...
var serverOptions = ['poolSize', 'ssl', 'sslValidate', 'checkServerIdentity',
  ...
  'keepAlive', 'connectTimeoutMS', 'socketTimeoutMS', 'reconnectTries',
  'reconnectInterval'];
...
var socketOptions = ['noDelay', 'keepAlive', 'connectTimeoutMS', 'socketTimeoutMS'];
...
var mergeTopLevel = function(object, options) {
  ...
  // Map to options
  var mapToOptions = function(name, optionNames, finalOptions, options) {
    if(optionNames.indexOf(name) != -1) {
      if(socketOptions.indexOf(name) != -1 && !finalOptions['socketOptions'][name]) {
        finalOptions['socketOptions'][name] = options[name];
      }
  ...
  for(var name in options) {
    mapToOptions(name, serverOptions, object.server_options, options);
    mapToOptions(name, replsetOptions, object.rs_options, options);
    mapToOptions(name, mongosOptions, object.mongos_options, options);
    mapToOptions(name, dbOptions, object.db_options, options);
  }
...
var connect = function(url, options, callback) {
  ...
  options.connectTimeoutMS = typeof options.connectTimeoutMS == 'number'
    ? options.connectTimeoutMS : (1000 * 120);
  options.socketTimeoutMS = typeof options.socketTimeoutMS == 'number'
    ? options.socketTimeoutMS : (1000 * 120);
