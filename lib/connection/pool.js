...
var Pool = function(options) {
  ...
  var errorHandler = function(err, connection) {
    // console.log("---------------------------------- pool error handler")
    ...
    // console.log("============================== errorHandler :: " + dead + " :: " + options.port + " ID :: " + id)
    // console.log(self.listeners('error').length)
    // console.dir(err)
    if(!dead) {
      state = DISCONNECTED;
      dead = true;
  ...
  var closeHandler = function(err, connection) {
    // console.log("---------------------------------- pool close handler")
  ...
  var parseErrorHandler = function(err, connection) {
    // console.log("---------------------------------- pool parseError handler")
    if(logger.isDebug()) logger.debug(f('pool [%s] errored out [%s] with connection [%s]', dead, JSON.stringify(err), JSON.stringify(connection)));
    if(!dead) {
      state = DISCONNECTED;
      dead = true;
  ...
  this.connect = function(_options) {
    // console.log("--------------------- CONNECT POOL :: " + id)
    // console.log("------------------------------------ connection pool.errors :: " + this.listeners('error').length + " :: " + id)
    // Set to connecting
    state = CONNECTING
    // No dead
    dead = false;
  ...
  this.isConnected = function() {
    // console.log("======================= POOL isConnected :: " + id + " :: " + connections.length + " :: " + tag)

    for(var i = 0; i < connections.length; i++) {
      // console.log("----------- checking pool :: " + connections[i].id)
