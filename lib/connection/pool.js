...
var _execute = function(self) {
  return function() {
    ...
    if(self.availableConnections.length == 0
      ...
      return setTimeout(_execute(self), 100)
    }
    ...
    while(true) {
      ...
      if(connection.isConnected()) {
        ...
        // console.log("-------------------------- EXECUTE COMMAND 0 :: " + self.availableConnections.length)
        ...
        // console.dir(Object.keys(workItem.cb))
        // Get actual binary commands
        var buffer = workItem.buffer;

        if(connection !== self.monitorConnection) {
          // console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ PUSHING MONITOR CONNECTION")
    ...
    if(self.queue.length > 0) {
      return setTimeout(_execute(self), 10)
      // return execute(_execute(self));
    }
  }
}
...
Pool.prototype.write = function(buffer, cb, options) {
  ...
  this.queue.push({buffer:buffer, cb: cb});
