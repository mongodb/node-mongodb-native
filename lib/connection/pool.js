...
var _createConnection = function(self) {
  self.options.messageHandler = self.messageHandler;
  console.log("=== pool _createConnection 0")
  var connection = new Connection(self.options);
...
Pool.prototype.connectionAvailable = function(connection) {
  ...
  if(this.availableConnections.indexOf(connection) == -1) {
    ...
    // Reset socket timeout (in case connection was used for monitoring and timeout was overwritten to monitoringSocketTimeout)
    connection.socketTimeoutMS = this.socketTimeout;
  }
...
Pool.prototype.isConnected = function() {
  ...
  for(var i = 0; i < this.connectingConnections.length; i++) {
    if(this.connectingConnections[i].isConnected()) return true;
  }
