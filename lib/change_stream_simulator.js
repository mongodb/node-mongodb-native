...
var createChangeStreamCursorSimulation = function (serverConfig, namespace, pipeline, options) {
  ...
  var aggregationOutputCursor = aggregationOutputCollection.find({/*'_id.ts': { $gte: changeNotificationCursorCreationTimestamp }*/seed: {$exists: false}}, {
    tailable: true,
    awaitdata: true
  });
  ...
  oplogStream.on('data', function(changeNotification) {
    ...
    aggregationProcessorCollection.insert(changeNotification).then(function () {
      ...
      // Insert a random _id if the pipeline projected out the _id
      if (!aggregationOutput[0]._id) {
        aggregationOutput[0]._id = {idSuppressed: true, random: Math.random().toString().substring(2, 12)}
      }
  ...
  aggregationOutputCursor.map(function(doc) {
    ...
    if (doc._id.idSuppressed) delete doc._id;
    return doc;
  })
