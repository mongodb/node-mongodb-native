...
var createChangeStreamCursor = function (serverConfig, namespace, pipeline, options) {
  console.log('spawning change notification cursor')
  var DB = require('./db')

  // Validate inputs
  if (typeof serverConfig !== 'object') throw new Error('A server topology object must be provided to createChangeStreamCursor')
  if (!(typeof namespace === 'object' && typeof namespace.database === 'string')) throw new Error('A namespace object containing a database name must be provided to createChangeStreamCursor')
  ...
  var changeNotificationCursorCreationTimestamp = new Timestamp(0, localDB.serverConfig.s.replset.ismaster.localTime.getTime() / 1000 - 10)
  ...
  oplogStream.on('data', function(changeNotification) {
    ...
    console.log('new data item from the oplog', changeNotification)
...
module.exports.createChangeStreamCursor = createChangeStreamCursor
