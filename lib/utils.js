...
function deprecate(config) {
  ...
  if (!config.deprecatedParams) {
    return require('util').deprecate(config.fn, messageHandler(config.fName, config.msg));
  }
  ...
  if (config.both) {
    return require('util').deprecate(deprecated, messageHandler(config.fName, config.msg));
  }
  // just parameter deprecation
  return deprecated;
}
