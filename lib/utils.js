 ...
 *
 * @param {Function|undefined} PromiseLibrary a promise library.
 * @param {Function|undefined} callback an optional callback.
 ...
 */
function maybePromise(PromiseLibrary, callback, fn) {
  const ValidPromiseLibrary = PromiseLibrary || Promise;

  let result;
  if (typeof callback !== 'function') {
    result = new ValidPromiseLibrary((resolve, reject) => {
      callback = (err, res) => {
        if (err) return reject(err);
        resolve(res);
      };
