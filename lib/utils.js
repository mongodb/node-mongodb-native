...
function resolveReadPreference(options, sources) {
  ...
  if (options.readPreference) {
    ...
    } else if (client && client.s.options.readPreference) {
      readPreference = client.s.options.readPreference;
...
function decorateWithCollation(command, target, options) {
  ...
  if (options.collation && typeof options.collation === 'object') {
    if (capabilities && capabilities.commandsTakeCollation) {
      ...
      // NOTE: Old topologies randomly store the connected server name as `name`, the new unified
      //       topology will not do this because it is incorrect info. This will have to be
      //       corrected during the command refactor, where we have info about the selected server
      //       during command execution.
      const serverName = topology.s.coreTopology
        ? `server ${topology.s.coreTopology.name}`
        : 'server';

      throw new MongoError(`${serverName} does not support collation`);
    }
  }
}
...
function decorateWithReadConcern(command, coll, options) {
  ...
  const readConcern = Object.assign(
    {},
