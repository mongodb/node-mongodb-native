 ...
 * @api private
 */
function defaultMsgHandler(fName, option) {
  const optionAddition = option ? ' option [' + option + ']' : '';
  return fName + optionAddition + ' is deprecated and will be removed in a later version.';
}
...
function deprecateOptions(config, fn) {
  ...
  function deprecated() {
    ...
    config.deprecatedOptions.forEach(deprecatedOption => {
      if (options.hasOwnProperty(deprecatedOption) && !optionsWarned[deprecatedOption]) {
        optionsWarned[deprecatedOption] = true;
        const msg = config.msgHandler(config.name, deprecatedOption);
