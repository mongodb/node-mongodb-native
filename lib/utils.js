...
function messageHandler(fName, msg, param) {
  if (msg) {
  ...
  function deprecated() {
    ...
    config.deprecatedParams.forEach(deprecatedParam => {
      if (options.hasOwnProperty(deprecatedParam) && !paramsWarned[deprecatedParam]) {
        ...
        emitDeprecationWarning(messageHandler(config.fName, config.msg, deprecatedParam));
        //TODO: log warnings
      }
    });
...
function deprecateFunction(fn, config) {
  return require('util').deprecate(fn, messageHandler(config.fName, config.msg));
}
...
function deprecate(config) {
  ...
  if (config.deprecateFunction) {
    return deprecateFunction(paramsDeprecated, config);
  }
  return paramsDeprecated;
}
