...
/**
 * Do something with a topology once it connects, if it hasn't already
 *
 * @param {object|Function} topologyOrFn either a connected topology, or a function that passes a callback a topology once connected
 * @param {Function} callback executes once topology is connected and state has been updated
 * @param {Function|Array} [updateState] set the topology property on these objects. can either be an array or a function that returns an array
 * @param {boolean} [onlyCallbackOnFn] skip the callback if the topology is already connected
 * @returns {boolean} whether the topology was already connected
 */
function withTopology(topologyOrFn, callback, updateState = [], onlyCallbackOnFn) {
  if (typeof topologyOrFn === 'function') {
    topologyOrFn((err, topology) => {
      if (err) {
        callback(err);
      } else {
        (typeof updateState === 'function' ? updateState() : updateState).forEach(s => {
          s.topology = topology;
        });
    ...
    if (!onlyCallbackOnFn) callback(null, topologyOrFn);
    return true;
  }
}
