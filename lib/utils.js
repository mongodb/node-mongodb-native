...
/** @typedef {InstanceType<import('./cursor')['Cursor']>} Cursor */
/** @typedef {InstanceType<import('./sdam/topology')['Topology']>} Topology */
...
/**
 * Determines if arg is an object with string tag check
 *
 * @param {any} arg
 ...
 * @param {Collection|Cursor} coll the parent collection of the operation calling this method
...
function deprecateOptions(config, fn) {
  // TODO:(ts) Dear god why is this property missing from the node types
  let process = /** @type {any} */ (global.process);
 ...
 *
 * @private
 * @param {Topology|Server | any} topologyOrServer TODO:(ts) better typing
 */
function maxWireVersion(topologyOrServer) {
  if (topologyOrServer) {
    ...
    if (topologyOrServer.lastIsMaster && typeof topologyOrServer.lastIsMaster === 'function') {
...
const DRIVER_VERSION = JSON.parse(
