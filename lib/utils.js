 ...
 * @api private
 */
function defaultMsgHandler(fName, param) {
  const paramAddition = param ? ' parameter [' + param + ']' : '';
  return fName + paramAddition + ' is deprecated and will be removed in a later version.';
}

/**
 * Deprecates a given function's parameters.
 *
 * @param {object} [config] configuration for deprecation
 * @param {string} [config.fName] function name
 * @param {Array} [config.deprecatedParams] optional parameters to deprecate
 * @param {number} [config.optionsIndex] optional index of options object in function arguments array, required if config.deprecatedParams
 * @param {function} [config.msgHandler] optional custom message handler to generate warnings
 * @param {function} [config.fn] the target function of deprecation
 * @return {function} modified function that warns once per deprecated parameter, and executes original function
 * @ignore
 * @api private
 */
function deprecateParameters(config) {
  ...
  const paramsWarned = {};
  function deprecated() {
    const options = shallowClone(arguments[config.optionsIndex]);
    if (typeof options !== 'object') {
      throw new Error('Options index must point to options object argument of deprecation target');
    }
    ...
    config.deprecatedParams.forEach(deprecatedParam => {
      if (options.hasOwnProperty(deprecatedParam) && !paramsWarned[deprecatedParam]) {
        ...
        emitDeprecationWarning(config.msgHandler(config.fName, deprecatedParam));
        //TODO: log warnings
      }
    });
...
/**
 * Deprecates a given function.
 *
 * @param {function} [fn] function to deprecate
 * @param {object} [config] configuration containing information about the function to deprecate
 * @param {function} [config.msgHandler] message handler to generate warnings
 * @Aparam {string} [config.fName] function name
 * @return {function} modified function that warns once, and executes original function
 * @ignore
 * @api private
 */
function deprecateFunction(fn, config) {
  return require('util').deprecate(fn, config.msgHandler(config.fName));
}

/**
 * Deprecates a given function's parameters, the function itself, or both. Wraps the given function.
 * Warns only once.
 *
 * @param {object} [config] configuration for deprecation
 * @param {string} [config.fName] function name
 ...
 * @param {number} [config.optionsIndex] optional index of options object in function arguments array, required if config.deprecatedParams
 * @param {function} [config.msgHandler] optional custom message handler to generate warnings
 * @param {boolean} [config.deprecateFunction] true if deprecating the function, required if deprecatedParams not specified
 * @param {function} [config.fn] the target function of deprecation
 * @return {function} modified function that warns once per deprecation, and executes original function
 */
function deprecate(config) {
  ...
  // use user-provided message handler, or default if none supplied
  const msgHandler = config.msgHandler ? config.msgHandler : defaultMsgHandler;
  config.msgHandler = msgHandler;

  // deprecate the given parameters
  const paramsDeprecated = deprecateParameters(config);

  // additionally, deprecate function if instructed to do so
  const finalFunction = config.deprecateFunction
    ? deprecateFunction(paramsDeprecated, config)
    : paramsDeprecated;
