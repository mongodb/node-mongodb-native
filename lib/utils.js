...
const serverLacksFeature = function(server, feature) {
  const serverWireVersion = maxWireVersion(server);
  switch (feature) {
    case 'collation':
      if (serverWireVersion < 5)
        return `Server ${server.name}, which reports wire version ${serverWireVersion}, does not support collation`;
      break;
    case 'commitQuorum':
      if (serverWireVersion < 9)
        return '`commitQuorum` option for `createIndexes` not supported on servers < 4.4';
      break;
  }
  return false;
};
...
const keysToOmit = new Set(commandOptions.concat('name', 'key'));

const prepareCreateIndex = function(fieldOrSpec, options) {
  ...
  const opts = {};
  commandOptions.forEach(option => (opts[option] = options[option]));
  return {
    indexSpec: [indexes],
    options: opts
  };
