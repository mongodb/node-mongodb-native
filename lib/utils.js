 ...
 * @param {boolean} isRetryable=false indicates if the operation is supported for retryable writes
 * @returns {Object} the (now) decorated target
 */
function applyWriteConcern(target, sources, options, isRetryable) {
  options = options || {};
  const db = sources.db;
  const coll = sources.collection;
  if (isRetryable == null) isRetryable = false;

  // NOTE: there is probably a much better place for this
  if (isRetryable && db && db.s.options.retryWrites) {
