...
var createChangeNotificationCursor = function (serverTopology, namespace, pipeline, options) {
  ...
  // Ensure the existence of the aggregationOutput capped collection
  ...
  if (!localDB.serverConfig.s.replset) {
  ...
  oplogStream.on('end', function () {
    console.log('oplogStream ended')
  })

  oplogStream.on('close', function () {
    console.log('oplogStream closed')
  })
