...
Object.defineProperty(Cursor.prototype, 'optionsValidationLevel', {
  enumerable: true,
  get: function() {
...
Cursor.prototype.count = function(applySkipLimit, opts, callback) {
  ...
  validate(countSchema, opts, { optionsValidationLevel: this.optionsValidationLevel });

  opts = applyDefaults(
    countSchema,
    opts,
    { maxTimeMS: this.s.cmd ? this.s.cmd.maxTimeMS : undefined },
 ...
 * @method
 * @param {object} [options] Optional settings.
 * @param {boolean} [options.skipKillCursors=false] Bypass calling killCursors when closing the cursor.
 * @param {Cursor~resultCallback} [callback] The result callback.
 * @return {Promise} returns Promise if no callback passed
 */
const closeSchema = {
  skipKillCursors: { type: 'boolean', default: false }
};
Cursor.prototype.close = function(options, callback) {
  if (typeof options === 'function') (callback = options), (options = {});

  validate(closeSchema, options, { optionsValidationLevel: this.optionsValidationLevel });

  options = applyDefaults(closeSchema, options, {});
...
Cursor.prototype.stream = function(options) {
  validate(streamSchema, options, { optionsValidationLevel: this.optionsValidationLevel });

  this.s.streamOptions = applyDefaults(streamSchema, options, {});
 ...
 * @return {stream}
 */
const transformStreamSchema = {
  transform: { type: 'function' }
};
Cursor.prototype.transformStream = function(options) {
  validate(transformStreamSchema, options, { optionsValidationLevel: this.optionsValidationLevel });

  const streamOptions = applyDefaults(transformStreamSchema, options, {});
