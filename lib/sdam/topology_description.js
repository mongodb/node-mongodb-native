...
class TopologyDescription {
  ...
  update(serverDescription) {
    ...
    if (topologyType === TopologyType.Unknown) {
      if (serverType === ServerType.Standalone && options.directConnection !== false) {
        serverDescriptions.delete(address);
      } else {
        topologyType = topologyTypeForServerType(serverType, options);
      }
    }
...
function topologyTypeForServerType(serverType, options) {
  if (serverType === ServerType.Standalone) return TopologyType.Single;
  if (serverType === ServerType.Unknown && options.directConnection === false) {
    return TopologyType.Unknown;
  }
  if (serverType === ServerType.RSGhost && options.directConnection === false) {
    return TopologyType.Unknown;
  }
