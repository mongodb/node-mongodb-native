...
class Topology extends EventEmitter {
  ...
  selectServer(selector, options, callback) {
    const parsedSelector = (typeof selector === 'function' && selector) || null;
    const selectorIsReadPref = (selector instanceof ReadPreference && selector) || null;

    const parsedOptions =
      (typeof selector === 'object' && !parsedSelector && selector) ||
    ...
    const readPreference =
      selectorIsReadPref || (options && options.readPreference) || ReadPreference.primary;

    const serverSelector = selector || readPreferenceServerSelector(readPreference);
