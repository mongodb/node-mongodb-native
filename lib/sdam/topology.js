...
/** @typedef {InstanceType<import('../utils')['MongoDBNamespace']>} MongoDBNamespace */
/** @typedef {import('../read_preference')} ReadPreference */
/** @typedef {InstanceType<import('../cmap/auth/mongo_credentials')['MongoCredentials']>} MongoCredentials */
...
class Topology extends EventEmitter {
   ...
   * @param {MongoCredentials} [options.credentials] Authentication credentials.
   * @param {any} [options.cursorFactory] a Cursor constructor class.
   * @param {any} [options.emitError] TODO:(ts) types and documentation
   * @param {any} [options.logger] Logger class.
   * @param {any} [options.loggerLevel] Logger filter level.
   * @param {any} [options.maxPoolSize] The maximum number of connections that may be associated with a pool at a given time. This includes in use and available connections. If specified, MUST be an integer >= 0. A value of 0 means there is no limit. Defaults to 100.
   * @param {any} [options.metadata] Metadata to associate with this topology.
   * @param {any} [options.minHeartbeatFrequencyMS] The smallest frequency of heartbeat messages.
   * @param {any} [options.minPoolSize] The minimum number of connections that MUST exist at any moment in a single connection pool. If specified, MUST be an integer >= 0. If maxPoolSize is > 0 then minPoolSize must be <= maxPoolSize Defaults to 0.
   * @param {any} [options.monitorCommands] Enable command monitoring.
   * @param {any} [options.promiseLibrary] A substitute Promise API. Defaults to native Promise.
   * @param {any} [options.reconnect] If false will not attempt to reconnect.
   * @param {any} [options.replicaSet] Replica set name.
   * @param {any} [options.srvHost] The host from an srv url.
   * @param {any} [options.srvPoller] A replacement SRV Poller for the default implementation.
   */
  constructor(seedlist, options) {
    super();
    if (typeof options === 'undefined' && typeof seedlist !== 'string') {
      options = /** @type {any} */ (seedlist);
      seedlist = [];

      // this is for legacy single server constructor support
      if (/** @type {any} */ (options).host) {
        seedlist.push({
          host: /** @type {any} */ (options).host,
          port: /** @type {any} */ (options).port
        });
      }
    }
  ...
  /**
   * @returns {TopologyDescription} the description of this topology
   */
  get description() {
   ...
   * @param {any} [options.readPreference] TODO:(ts) types and documentation
   ...
   *
   * @param {any} [selector] An optional selector to select servers by, defaults to a random selection within a latency window
   ...
   * @param {ReadPreference} [options.readPreference] the read preference
   * @param {any} [options.session] TODO:(ts) documentation
  ...
  selectServer(selector, options, callback) {
    if (typeof options === 'function') {
      ...
      if (typeof selector !== 'function') {
        ...
        let readPreference;
        if (/** @type {any} */ (selector) instanceof ReadPreference) {
   ...
   *
   * @param {string | MongoDBNamespace} ns The full qualified namespace for this operation
   ...
   *
   * @param {string | MongoDBNamespace} ns The fully qualified namespace for this operation
   ...
   *
   * @param {string | MongoDBNamespace} ns The MongoDB fully qualified namespace (ex: db1.collection1)
   ...
   * @param {boolean} [options.ignoreUndefined=false] Specify if the BSON serializer should ignore undefined fields.
   * @param {ClientSession} [options.session=null] Session to use for the operation
   * @param {boolean} [options.noResponse] Mark the command to require no server response.
   * @param {number} [options.serverSelectionTimeoutMS] The number of milliseconds before timing out server selection.
   * @param {boolean} [options.retrying] If this command is being retried.
   * @param {any} [options.retryWrites] If retry writes is enabled.
   * @param {(error?: Error, result?: any) => void} callback A callback function
   */
   ...
   * @param {{new(topology: any, ns: string | MongoDBNamespace, cmd: object, options: object)}} [options.cursorFactory] A Cursor constructor
   * @returns {Cursor}
   */
