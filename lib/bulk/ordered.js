...
const WriteConcernError = require('./common').WriteConcernError;
  ...
  var resultHandler = function(err, result) {
    ...
    const mergeResult =
      err instanceof MongoWriteConcernError
        ? mergeBatchResults(true, batch, self.s.bulkResult, null, err.result)
        : mergeBatchResults(true, batch, self.s.bulkResult, err, result);
    ...
    if (err instanceof MongoWriteConcernError) {
      ...
      return handleCallback(
        callback,
        new BulkWriteError(toError(wrappedWriteConcernError), writeResult),
        null
      );
    }
