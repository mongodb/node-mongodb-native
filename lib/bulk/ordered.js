...
function OrderedBulkOperation(topology, collection, options) {
  ...
  this.raw = function(op) {
    if(op.update) return addToOperationsList(self, common.UPDATE, op.update);
    if(op.remove) return addToOperationsList(self, common.REMOVE, op.remove);
    return addToOperationsList(self, common.INSERT, op.insert);
  }
    ...
    var resultHandler = function(err, result) {
      ...
      if(bulkResult.writeErrors.length > 0) {
        return callback(bulkResult.writeErrors[0], new BatchWriteResult(bulkResult));
      }
