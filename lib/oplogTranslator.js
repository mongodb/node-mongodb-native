var createChangeNotificationCursor = function (serverTopology, namespace) {
  var DB = require('./db')
  ...
  return oplogCollection.find(query, {
    tailable: true,
    awaitdata: true
  }).map(convertFromOplogToChangeNotification)

}

var processOplogDocument = function (oplogDocument) {
  pipelineSteps = [convertFromOplogToChangeNotification]
}
...
var convertFromOplogToChangeNotification = function (oplogMessage) {
  ...
  switch (oplogMessage.op) {
    ...
    case 'c':
      ...
      break;
    default:
      throw new Error('Unknown operation type', oplogMessage.op)
  }
