...
var createChangeNotificationCursor = function (serverTopology, databaseName, collectionName, pipeline) {
  ...
  if (typeof databaseName !== 'string') throw new Error('A database name must be provided to createChangeNotificationCursor')
  ...
  if (collectionName) {
    query.$or.push({
      ns: new RegExp('^' + databaseName + '\.'),
      "o.drop": collectionName
    })
  }
  ...
  var oplogCursor = oplogCollection.find(query, {
    ...
    return convertFromOplogToChangeNotification(oplogDocument)
  })
  ...
  co(function*() {
    while (true) {
      if (yield oplogCursor.hasNext()) {
        var oplogDocument = yield oplogCursor.next()
        aggregationProcessorCollection.insert(oplogDocument, function (err, result) {
          if (err) throw err

          // Construct an aggregation pipeline that matches oplogDocument and then performs the user-provided pipeline stages
          var composedAggregationPipeline = [ { $match: { _id: oplogDocument._id } } ].concat(pipeline)

          aggregationProcessorCollection.aggregate(composedAggregationPipeline).toArray(function(error, docs) {
            aggregationOutputCollection.insert(docs, function (err, result) {
              if (err) throw err
            })
          })
        })
