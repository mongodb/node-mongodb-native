...
var createChangeNotificationCursor = function (serverTopology, databaseName, collectionName, pipeline) {
  ...
  var oplogCursor = oplogCollection.find(query, {
    ...
    return translateFromOplogToChangeNotification(oplogDocument)
  })
  ...
  co(function*() {
    while (true) {
      if (yield oplogCursor.hasNext()) {
        var oplogDocument
        oplogCursor.next().then(function (documentFromCursor) {
          oplogDocument = documentFromCursor
          return aggregationProcessorCollection.insert(oplogDocument)
        }).then(function () {
