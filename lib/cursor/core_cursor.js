...
class CoreCursor extends Readable {
   ...
   * @param {any} [options.disconnectHandler] handler for disconnection
   * @param {number} [options.skip] skip count
   * @param {number} [options.limit] limit count
   * @param {any} [options.raw] Return document results as raw BSON buffers.
   * @param {boolean} [options.promoteLongs] Promotes Long values to number if they fit inside the 53 bits resolution.
   * @param {boolean} [options.promoteValues] Promotes BSON values to native types where possible, set to false to only receive wrapper types.
   * @param {boolean} [options.promoteBuffers] Promotes Binary BSON values to native Node Buffers.
   * @param {object} [options.session] TODO:(ts) documentation
   ...
   * @param {ReadPreference} [options.readPreference] The preferred read preference.
   * @param {boolean} [options.tailable] TODO:(ts) documentation
   * @param {any} [options.hint] An optional hint for query optimization. See the {@link https://docs.mongodb.com/manual/reference/command/update/#update-command-hint|update command} reference for more information.
   * @param {any} [options.startAfter] Will allow you to start after an invalidated event. See {@link https://docs.mongodb.com/master/changeStreams/#startafter-for-change-streams|ChangeStream documentation}.
   ...
   */
  close(_options, _callback) {
    throw new Error('CoreCursor.prototype.close() is abstract.');
  }
   ...
   */
  next(_callback) {
    throw new Error('CoreCursor.prototype.next() is abstract.');
  }
  ...
  async *[Symbol.asyncIterator]() {
    // NOTE: Normally we avoid async await in driver code, this is an exception
    while (true) {
      const value = await this.next();
      // @ts-ignore TODO:(ts) returns Promise<void> | void error
      if (!value) {
        await this.close();
        return;
      }
