  ...
  parseFn(url, options, (err, _object) => {
    ...
    if (_finalOptions.monitor) {
      ...
      const out = _finalOptions.monitorout
        ? getCustomOut(_finalOptions.monitorout, callback)
        : createOutFileStream(defaultFile, callback);

      // attach the event listeners
      monitors.forEach(group => {
        attachMonitorListenerGroup(mongoClient, group, out);
      });
    }
...
function getMonitors(monitor) {
  const allMonitors = ['command', 'sdam'];
  // split on comma if multiple groups specified
  let monitors = monitor.indexOf(',') > -1 ? monitor.split(',') : monitor;
  // enable all monitors if monitor = all
  monitors = monitors === 'all' ? allMonitors : monitors;
  // ensure `monitors` is an array
  monitors = Array.isArray(monitors) ? monitors : [monitors];
  return monitors;
}

function getCustomOut(monitorOut, cb) {
  let out;
  if (monitorOut === 'stderr' || monitorOut === 'stdout') {
    out = process[monitorOut];
  } else {
    const file = path.resolve(monitorOut);
    out = createOutFileStream(file, cb);
  }

  return out;
}

function createOutFileStream(file, cb) {
  // if file already exists, rename it to avoid overwrite
  if (fs.existsSync(file)) {
    const timestamp = new Date().toISOString();
    fs.renameSync(file, file + '-' + timestamp);
  }

  const stream = fs.createWriteStream(file);
  stream.on('error', e => cb(new Error(e)));
  return stream;
}

function attachMonitorListenerGroup(client, group, stream) {
  apmLoggingEvents[group].forEach(eventName => {
    client.on(eventName, e => {
      const eventObj = EJSON.stringify(e, { relaxed: true });
      const timestamp = new Date().toISOString();
      const logLine = `${timestamp} ${group.toUpperCase()} ${eventName} ${eventObj}\n`;
