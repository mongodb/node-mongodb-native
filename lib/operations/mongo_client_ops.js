...
// APM logging helper methods
const apmReadableStream = require('../apm').apmReadableStream;
const apmLoggingEventHandler = require('../apm').apmLoggingEventHandler;
const getMonitoringCategories = require('../apm').getMonitoringCategories;
const resolveMonitoringOutputFile = require('../apm').resolveMonitoringOutputFile;
const createMonitorWriteStream = require('../apm').createMonitorWriteStream;
  ...
  parseFn(url, options, (err, _object) => {
    ...
    if (_finalOptions.monitor) {
      ...
      try {
        ...
        const monitorWriteStream = createMonitorWriteStream(
          resolveMonitoringOutputFile(_finalOptions.monitorout)
        );

        // pipe readable monitor stream to output stream to avoid back pressure
        const monitorReadStream = new apmReadableStream(mongoClient, monitoringCategories);
        monitorReadStream.pipe(monitorWriteStream);

        // error handling
        monitorWriteStream.on('error', apmLoggingEventHandler(mongoClient));
        monitorReadStream.on('error', apmLoggingEventHandler(mongoClient));
      } catch (e) {
        return callback(e);
      }
    }
