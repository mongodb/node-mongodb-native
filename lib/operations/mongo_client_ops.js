...
function connect(mongoClient, url, options, callback) {
  ...
  parseFn(url, options, (err, _object) => {
    ...
    if (_finalOptions.monitor) {
      ...
      out.on('error', e => apmEventualErrorHandler(e, mongoClient));

      // attach each group of event listeners to the new client
      monitors.forEach(type => {
        attachMonitorListenerType(mongoClient, type, out);
      });
    }
...
function apmEventualErrorHandler(e, client) {
  if (e) {
    if (!client.topology) throw e;
    client.close(true, () => {
      throw e;
    });
  }
...
function getMonitors(monitor) {
  ...
  let monitors = monitor === 'all' ? allMonitors : monitor;
  // ensure `monitors` is an array
  monitors = Array.isArray(monitors) ? monitors : [monitors];
  return monitors;
}

function getCustomOut(monitorOut) {
  let out;
...
function createOutFileStream(file) {
...
function attachMonitorListenerType(client, type, stream) {
  apmLoggingEvents[type].forEach(eventName => {
    client.on(eventName, e => {
      ...
      stream.write(logLine, e => apmEventualErrorHandler(e, client));
    });
  });
}
