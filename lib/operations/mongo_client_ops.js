...
function createMongos(mongoClient, options, callback) {
  ...
  // Create the topology
  const topology = new Mongos(servers, options);
...
function createReplicaset(mongoClient, options, callback) {
  // Pass in the promise library
  options.promiseLibrary = mongoClient.s.promiseLibrary;
  ...
  // Create the topology
  const topology = new ReplSet(servers, options);
...
function createServer(mongoClient, options, callback) {
  ...
  servers[0].connect(options, (err, topology) => {
    ...
    if (ismaster && ismaster.msg === 'isdbgrid') {
      ...
      return createMongos(mongoClient, options, callback);
    }
 ...
 * @param {object} [options=null] Optional settings. See MongoClient.prototype.logout for a list of options.
