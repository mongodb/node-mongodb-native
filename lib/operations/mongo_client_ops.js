...
function connect(mongoClient, url, options, callback) {
  ...
  parseFn(url, options, (err, _object) => {
    ...
    if (_finalOptions.monitor) {
      ...
      // get writable output stream
      let monitoringStream;
      try {
        monitoringStream = createMonitoringStream(
      ...
      monitoringCategories.forEach(category => {
        attachMonitorListenerCategory(mongoClient, category, monitoringStream);
      });
    }
...
function getMonitoringCategories(monitor) {
...
function createMonitoringStream(fileName) {
 ...
 * be written to the given writable stream.
 *
 ...
 * @param {WritableStream} stream stream to which log output will be written
 */
function attachMonitorListenerCategory(client, category, stream) {
  apmLoggingEvents[category].forEach(eventName => {
    client.on(eventName, e => {
      ...
      stream.write(logLine, apmLoggingEventHandler(client));
    });
  });
}
