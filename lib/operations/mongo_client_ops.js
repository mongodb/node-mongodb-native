...
function createServer(mongoClient, options, callback) {
  ...
  const servers = createServerInstances(finalOptions);
  ...
  // Connect to topology
  server.connect(finalOptions, (err, topology) => {
    if (err) {
      server.close(true);
      return callback(err);
    }
    ...
    if (ismaster && ismaster.msg === 'isdbgrid') {
      ...
      return createTopology(mongoClient, 'mongos', finalOptions, callback);
    }
...
function createTopology(mongoClient, topologyType, options, callback) {
  ...
  if (topologyType === 'mongos') {
    topology = new Mongos(servers, finalOptions);
  } else if (topologyType === 'replicaset') {
    topology = new ReplSet(servers, finalOptions);
  }
  ...
  // Open the connection
  topology.connect(finalOptions, (err, topology) => {
    if (err) {
      topology.close(true);
      return callback(err);
    }
...
function translateOptions(options) {
  ...
  if (typeof options.readPreference === 'string' || typeof options.read_preference === 'string') {
    translatedOptions.readPreference = new ReadPreference(
  ...
  if (options.readPreference && (options.readPreferenceTags || options.read_preference_tags)) {
    translatedOptions.readPreference.tags =
      options.readPreferenceTags || options.read_preference_tags;
  }

  // Do we have maxStalenessSeconds
  if (options.maxStalenessSeconds) {
    translatedOptions.readPreference.maxStalenessSeconds = options.maxStalenessSeconds;
  }

  // Set the socket and connection timeouts
  if (options.socketTimeoutMS == null) translatedOptions.socketTimeoutMS = 360000;
  if (options.connectTimeoutMS == null) translatedOptions.connectTimeoutMS = 30000;

  return translatedOptions;
}

function createServerInstances(options) {
  // Create server instances
