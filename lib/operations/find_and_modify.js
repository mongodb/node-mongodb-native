...
class FindAndModifyOperation extends CommandOperationV2 {
   ...
   * @param {boolean} [options.projection]
   * @param {*} [options.fields]
   * @param {*} [options.arrayFilters]
   * @param {*} [options.maxTimeMS]
   * @param {*} [options.serializeFunctions]
   * @param {*} [options.bypassDocumentValidation]
   * @param {*} [options.hint]
   */
  ...
  execute(server, callback) {
    ...
    if (this.writeConcern && !inTransaction) {
      Object.assign(queryObject, { writeConcern: this.writeConcern });
    }
