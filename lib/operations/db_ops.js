 ...
 * @param {object} [options=null] Optional settings. See Db.prototype.addUser for a list of options.
...
function addUser(db, username, password, options, callback) {
  ...
  _executeAuthCreateUserCommand(db, username, password, options, (err, r) => {
    // We need to perform the backward compatible insert operation
    if (err && err.code === -5000) {
      const finalOptions = applyWriteConcern(Object.assign({}, options), { db: db }, options);
 ...
 * @param {object} [options=null] Optional settings. See Db.prototype.collections for a list of options.
 ...
 * @param {object} [options=null] Optional settings. See Db.prototype.createCollection for a list of options.
 ...
 * @param {object} [options=null] Optional settings. See Db.prototype.createIndex for a list of options.
 * @param {Db~resultCallback} [callback] The command result callback
 */
function createIndex(db, name, fieldOrSpec, options, callback) {
  ...
  _createIndexUsingCreateIndexes(db, name, fieldOrSpec, options, (err, result) => {
    ...
    // Create command
    const doc = _createCreateIndexCommand(db, name, fieldOrSpec, options);
    // Set no key checking
 ...
 * @param {(string|object)} fieldOrSpec Defines the index.
 * @param {object} [options=null] Optional settings. See Db.prototype.ensureIndex for a list of options.
 * @param {Db~resultCallback} [callback] The command result callback
 */
function ensureIndex(db, name, fieldOrSpec, options, callback) {
  ...
  // Create command
  const selector = _createCreateIndexCommand(db, name, fieldOrSpec, options);
 ...
 * @param {object} [options=null] Optional settings. See Db.prototype.eval for a list of options.
 * @param {Db~resultCallback} [callback] The results callback
 ...
 * @param {object} [options=null] Optional settings. See Db.prototype.command for a list of options.
 * @param {Db~resultCallback} [callback] The command result callback
 */
 ...
 * @param {Db} db The Db instance on which to execute the command.
 * @param {object} command The command hash
 * @param {object} [options=null] Optional settings. See Db.prototype.executeDbAdminCommand for a list of options.
 * @param {Db~resultCallback} [callback] The command result callback
 */
 ...
 * @param {object} [options=null] Optional settings. See Db.prototype.indexInformation for a list of options.
 * @param {Db~resultCallback} [callback] The command result callback
 */
 ...
 * @param {string} username The username.
 * @param {object} [options=null] Optional settings. See Db.prototype.removeUser for a list of options.
 * @param {Db~resultCallback} [callback] The command result callback
 */
function removeUser(db, username, options, callback) {
  // Attempt to execute command
  _executeAuthRemoveUserCommand(db, username, options, (err, result) => {
    if (err && err.code === -5000) {
      const finalOptions = applyWriteConcern(Object.assign({}, options), { db: db }, options);
      // If we have another db set
      const db = options.dbName ? new Db(options.dbName, db.s.topology, db.s.options) : db;
...
function setProfilingLevel(db, level, options, callback) {
  ...
  db.command(command, options, (err, doc) => {
 ...
 */
function _createCreateIndexCommand(db, name, fieldOrSpec, options) {
 ...
 * @param {Db~resultCallback} [callback] The command result callback.
 */
function _createIndexUsingCreateIndexes(db, name, fieldOrSpec, options, callback) {
  // Build the index
  const indexParameters = parseIndexOptions(fieldOrSpec);
  // Generate the index name
  const indexName = typeof options.name === 'string' ? options.name : indexParameters.name;
 ...
 * @param {Db~resultCallback} [callback] The command result callback
 */
function _executeAuthCreateUserCommand(db, username, password, options, callback) {
 ...
 * @param {Db~resultCallback} [callback] The command result callback
 */
function _executeAuthRemoveUserCommand(db, username, options, callback) {
