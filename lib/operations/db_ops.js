...
/** @typedef {import('../db').DbResultCallback} DbResultCallback */
...
function createIndex(db, name, fieldOrSpec, options, callback) {
  ...
  createIndexUsingCreateIndexes(db, name, fieldOrSpec, finalOptions, (err, result) => {
     ...
     */
    if (
      /** @type {Error & {code?:number}} */ (err).code === 67 ||
      /** @type {Error & {code?:number}} */ (err).code === 11000 ||
      /** @type {Error & {code?:number}} */ (err).code === 85 ||
      /** @type {Error & {code?:number}} */ (err).code === 86 ||
      /** @type {Error & {code?:number}} */ (err).code === 11600 ||
      /** @type {Error & {code?:number}} */ (err).code === 197
    ) {
 ...
 * @param {InstanceType<Code> | string | Function} code JavaScript to execute on server.
...
function removeUser(db, username, options, callback) {
  ...
  executeAuthRemoveUserCommand(db, username, options, (err, result) => {
    if (err && err.code === -5000) {
      const finalOptions = applyWriteConcern(Object.assign({}, options), { db }, options);
      // If we have another db set
      const anotherDb = options.dbName ? new Db(options.dbName, db.s.topology, db.s.options) : db;

      // Fetch a user collection
      const collection = anotherDb.collection(CONSTANTS.SYSTEM_USER_COLLECTION);
...
function executeAuthCreateUserCommand(db, username, password, options, callback) {
  ...
  if (roles.length === 0) {
    console.error('Creating a user without roles is deprecated in MongoDB >= 2.6');
  }
