...
const { shouldRetryOperation } = require('../sdam/topology');
...
function executeWithServerSelection(topology, operation, callback) {
  ...
  function callbackWithRetry(err, result) {
    ...
    // `ListIndexes succeeds after InterruptedDueToReplStateChange`
    // is specific retryable error via code

    // `FindOneAndDelete succeeds with RetryableWriteError from server`
    // is retryable via label

    // `FindOneAndDelete fails if server does not return RetryableWriteError`
    // is not retryable because there’s no label

    const isFindAndModify = operation instanceof FindAndModifyOperation;
    if (isFindAndModify) {
