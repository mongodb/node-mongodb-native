...
const { maxWireVersion, applyWriteConcern } = require('../utils');
const ReadPreference = require('../read_preference');
const { commandSupportsReadConcern } = require('../sessions');
const { MongoError } = require('../error');

const SUPPORTS_WRITE_CONCERN_AND_COLLATION = 5;
const SUPPORTS_HINT = 8;

class CommandOperationV2 extends OperationBase {
  /**
   * Creates an instance of CommandOperationV2.
   *
   * @param {*} parent
   * @param {*} options
   * @param {object} [operationOptions]
   * @param {boolean} [operationOptions.fullResponse]
   * @memberof CommandOperationV2
   */
  constructor(parent, options, operationOptions) {
    ...
    options = applyWriteConcern(options, { db: parent.s.db, collection: parent }, options);
  ...
  executeCommand(server, cmd, callback) {
    ...
    if (options.hint) {
      ...
      if (unacknowledgedWrite || serverWireVersion < SUPPORTS_HINT) {
        return callback(
          new MongoError('The current topology does not support a hint on findAndModify commands')
        );
      }
      Object.assign(cmd, { hint: options.hint });
    }
