...
class CommandOperationV2 extends OperationBase {
  constructor(parent, options, operationOptions) {
    ...
    const noInherit = this.hasAspect(Aspect.NO_INHERIT_OPTIONS);
    const inherit = noInherit ? {} : parent;

    this.ns = parent.s.namespace.withCollection('$cmd');
    const readPreference = ReadPreference.resolveNoThrow(inherit, this.options);
    if (readPreference) this.readPreference = readPreference;
    this.readConcern = resolveReadConcern(inherit, this.options);
    this.writeConcern = resolveWriteConcern(inherit, this.options);
    this.explain = false;
