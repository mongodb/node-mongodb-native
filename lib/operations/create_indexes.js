...
class CreateIndexesOperation extends CommandOperationV2 {
  ...
  execute(server, callback) {
    // by setting singular to true, ensure createIndex returns the name
    // of the created index instead of the full result from server
    // TODO: remove in v4 (breaking change)
    let singular = false;
    if (!Array.isArray(this.indexes)) {
      singular = true;
      const options = this.options;
      const indexParameters = parseIndexOptions(this.indexes);
    ...
    super.executeCommand(server, cmd, (err, result) => {
      ...
      callback(null, singular ? indexes[0].name : result);
    });
  }
}
