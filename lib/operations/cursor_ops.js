...
const createCountCommand = require('./collection_ops').createCountCommand;
...
function count(cursor, applySkipLimit, opts, callback) {
  ...
  if (cursor.s.cmd.hint) {
    opts.hint = cursor.s.cmd.hint;
  } else if (cursor.s.options.hint) {
    opts.hint = cursor.s.options.hint;
  }
  if (
  ...
  opts.collectionName = cursor.s.ns.substr(delimiter + 1);

  const command = createCountCommand(cursor, cursor.s.cmd.query, opts);
