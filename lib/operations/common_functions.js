...
/** @typedef {import('../collection').CollectionFindAndModifyCallback} CollectionFindAndModifyCallback */
...
function buildCountCommand(collectionOrCursor, query, options) {
  ...
  if (collectionOrCursor.s.numberOfRetries) {
    const cursor = /** @type {Cursor} */ (collectionOrCursor);
    if (cursor.options.hint) {
      hint = cursor.options.hint;
    } else if (cursor.cmd.hint) {
      hint = cursor.cmd.hint;
    }
    decorateWithCollation(cmd, collectionOrCursor, cursor.cmd);
  } else {
    const collection = /** @type {Collection} */ (collectionOrCursor);
