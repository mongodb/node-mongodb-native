...
const AUTHENTICATORS = require('../core/auth').AUTHENTICATORS;
...
function generateCredentials(client, username, password, options) {
  ...
  // authMechanism
  const mechanism = (options.authMechanism || 'DEFAULT').toUpperCase();
  const mechanismProperties = options.authMechanismProperties;

  if (!AUTHENTICATORS.has(mechanism)) {
    throw new MongoError(
      `authentication mechanism ${mechanism} not supported', options.authMechanism`
    );
  }
