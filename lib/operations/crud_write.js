...
class CrudWriteOperation extends OperationBase {
  ...
  execute(server, callback) {
    return this.model.execute(server, callback);
  }
}
...
class CrudModel {
  ...
  execute(server, callback) {
    ...
    server[this.type](this.namespace.toString(), this.operations, this.options, callback);
  }
}
...
function crudModelCallback(err, result, callback) {
...
function makeCrudModelCallback(callback) {
  return (err, result) => crudModelCallback(err, result, callback);
}
...
class InsertOneModel extends BulkInsertModel {
  ...
  execute(server, callback) {
    const cb = makeCrudModelCallback((err, r) =>
      insertOneCallback(err, r, this.operations, callback)
    );
    return super.execute(server, cb);
  }
}
...
class UpdateModel extends BulkUpdateModel {
  ...
  execute(server, callback) {
    const cb = makeCrudModelCallback((err, r) => updateCallback(err, r, callback));
    return super.execute(server, cb);
  }
}
...
class ReplaceModel extends UpdateModel {
  execute(server, callback) {
    return super.execute(server, (err, r) => {
...
class RemoveModel extends BulkRemoveModel {
  ...
  execute(server, callback) {
    const cb = makeCrudModelCallback((err, r) => deleteCallback(err, r, callback));
    return super.execute(server, cb);
  }
}
