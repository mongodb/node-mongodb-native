...
/**
 * Execute the command using executeDbAdminCommand
 *
 * @param {Admin} a collection instance.
 * @param {Object} command The command document
 * @param {Object} [options] Optional settings. See Admin.prototype.serverStatus for a list of options.
 * @param {Admin~resultCallback} [callback] The command result callback
 */
function constructCommand(admin, command, options, callback) {
  executeDbAdminCommand(admin.s.db, command, options, (err, doc) => {
...
function replSetGetStatus(admin, options, callback) {
  constructCommand(admin.s.db, { replSetGetStatus: 1 }, options, callback);
}
...
function serverStatus(admin, options, callback) {
  constructCommand(admin, { serverStatus: 1 }, options, callback);
}
