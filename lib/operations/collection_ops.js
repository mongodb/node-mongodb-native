...
function count(coll, query, options, callback) {
  ...
  const cmd = createCountCommand(coll, query, options);
...
function createCountCommand(context, query, options) {
  const skip = options.skip;
  const limit = options.limit;
  ...
  decorateWithReadConcern(cmd, context);

  // Apply a collation if set
  if (context.s.cmd) {
    decorateWithCollation(cmd, context, context.s.cmd);
  } else {
    decorateWithCollation(cmd, context, options);
  }
