...
function group(coll, keys, condition, initial, reduce, finalize, command, options, callback) {
  ...
  if (command) {
    const reduceFunction = reduce && reduce._bsontype === 'Code' ? reduce : new BSON.Code(reduce);
    ...
    if ('function' === typeof keys || (keys && keys._bsontype === 'Code')) {
      selector.group.$keyf = keys && keys._bsontype === 'Code' ? keys : new BSON.Code(keys);
    } else {
      const hash = {};
      keys.forEach(key => {
        hash[key] = 1;
    ...
    evaluate(coll.s.db, new BSON.Code(groupfn, scope), null, options, (err, results) => {
      if (err) return handleCallback(callback, err, null);
...
function parallelCollectionScan(coll, options, callback) {
  ...
  executeCommand(coll.s.db, commandObject, options, (err, result) => {
    ...
    for (let i = 0; i < result.cursors.length; i++) {
      ...
      const cursorId = typeof rawId === 'number' ? BSON.Long.fromNumber(rawId) : rawId;
