...
var Store = function(topology, storeOptions) {
  ...
  this.execute = function() {
    while(storedOps.length > 0) {
      ...
      if(op.t == 'insert' || op.t == 'update' || op.t == 'remove') {
        topology[op.t](op.n, op.o, op.op, op.c);
      } else {
        try {
          // Attempt to create a cursor
          var cursor = topology.cursor(op.n, op.o, op.op);
          // Attempt to execute the cursor operation
          cursor[op.t](op.c);
        } catch(err) {
          // Failed to create cursor, error out the operation
