 ...
 * @param {object} overrideOptions Optional. The values for options that need to check inheritance, like readPreference.
 * @param {object} validationOptions Optional. Options for the validation itself.
...
function validationFunction(optionsSchema, providedOptions, overrideOptions, validationOptions) {
  ...
  Object.keys(optionsSchema).forEach(optionName => {
    ...
    if (overrideOptions && overrideOptions[optionName] != null) {
      if (!verifiedOptions[optionName]) {
        verifiedOptions[optionName] = overrideOptions[optionName];
      }
      if (optionsSchema[optionName] && optionsSchema[optionName].default) {
        console.warn(`
