 ...
 *  5) deprecation notices are emitted for user-provided options with deprecated = true in optionsSchema
 *     or with deprecated = 'newOption' where newOption is the option that should be used instead of the deprecated option
 *
 * If the validation level is 'warn' or 'error',
 * this function will check that the types of user-provided options match their specified types in optionsSchema.
 *
...
function validate(optionsSchema, providedOptions, validationOptions) {
  ...
  if (validationOptions) {
    ...
    if (validationOptions.hasOwnProperty('unknownOptionsWarningLevel')) {
      ...
      Object.keys(providedOptions).forEach(providedOption => {
        if (optionSchemaNames.indexOf(providedOption) === -1) {
          const unknownOptionsMessage = `option ${providedOption} is not supported`;
          invalidate(validationOptions.unknownOptionsWarningLevel, unknownOptionsMessage, logger);
        }
      });
    }
  }
  ...
  Object.keys(optionsSchema).forEach(optionName => {
    ...
    if (providedOptions == null) {
      if (optionFromSchema.required) {
        throw new Error(`required option [${optionName}] was not found.`);
      }
    } else {
      if (
        optionFromSchema.hasOwnProperty('deprecated') &&
        providedOptions.hasOwnProperty(optionName)
      ) {
        let deprecationMessage;
        if (typeof optionFromSchema.deprecated === 'string') {
          deprecationMessage = `option [${optionName}] is deprecated. Use [${
            optionFromSchema.deprecated
          }] instead.`;
        } else if (optionFromSchema.deprecated === true) {
          deprecationMessage = `option [${optionName}] is deprecated and will be removed in a later version.`;
        }
        emitDeprecationWarning(deprecationMessage);
      }

      if (optionFromSchema.required && !providedOptions.hasOwnProperty(optionName)) {
        throw new Error(`required option [${optionName}] was not found.`);
      }

      if (optionsValidationLevel !== VALIDATION_LEVEL_NONE) {
        if (providedOptions.hasOwnProperty(optionName) && optionFromSchema.type != null) {
          if (!checkType(optionFromSchema.type, providedOptions[optionName])) {
            const invalidateMessageType = `option [${optionName}] should be of type ${
              optionFromSchema.type
            }, but is of type ${typeof providedOptions[optionName]}.`;
...
function checkType(requiredType, providedValue) {
  if (typeof requiredType === 'string') {
    if (requiredType === 'object') {
      ...
      return Array.isArray(providedValue);
    } else {
      return typeof providedValue === requiredType;
    }
  } else if (Array.isArray(requiredType)) {
    return requiredType.some(reqType => {
      return checkType(reqType, providedValue);
    });
  } else if (providedValue instanceof requiredType) {
    return true;
  }
  return false;
}
 ...
 *
 * @method
 * @param {string} validationLevel The level at which to validate the providedOptions or handle unknown options ('warn' or 'error').
...
module.exports = { arityZero, arityOne, arityTwo, assertArity, validate, applyDefaults };
