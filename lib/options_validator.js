...
function assertArity(args, requiredArity) {
  ...
  // requiredArity + 2 is the maximum number of arguments allowed, since this accounts for options and callback
  // requiredArity is the minimum number of arguments allowed
  if (args.length > requiredArity + 2 || args.length < requiredArity) {
  ...
  // a callback does not count towards the number of arguments
  if (typeof args[args.length - 1] === 'function') {
    numberOfArguments = numberOfArguments - 1;
  }

  // we are no longer considering the callback so requiredArity + 1 is the maximum number of arguments allowed
  if (numberOfArguments > requiredArity + 1) {
    const invalidateMessageArity = `This operation has a required arity of ${requiredArity}, but ${numberOfArguments} arguments were provided.`;
    invalidate(VALIDATION_LEVEL_ERROR, invalidateMessageArity);
  }
}
