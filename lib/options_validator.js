...
function validate(
  ...
  Object.keys(optionsSchema).forEach(optionName => {
    ...
    if (optionFromSchema.hasOwnProperty('default') && !verifiedOptions.hasOwnProperty(optionName)) {
...
function checkType(requiredType, providedValue) {
  if (typeof requiredType === 'string') {
    if (
      (requiredType === 'object' && isObject(providedValue)) ||
      (requiredType === 'array' && Array.isArray(providedValue)) ||
      (requiredType !== 'array' &&
        requiredType !== 'object' &&
        typeof providedValue === requiredType)
    ) {
      return true;
    }
