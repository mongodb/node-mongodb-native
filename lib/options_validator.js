...
function validate(optionsSchema, providedOptions, overrideOptions, validationOptions) {
  ...
  Object.keys(optionsSchema).forEach(optionName => {
    ...
    if (
      optionFromSchema.hasOwnProperty('deprecatedAliases') &&
      !verifiedOptions.hasOwnProperty(optionName)
    ) {
      const verifiedOptionNames = Object.keys(verifiedOptions);
      optionFromSchema.deprecatedAliases.forEach(deprecatedAlias => {
        if (verifiedOptionNames.indexOf(deprecatedAlias) > -1) {
          console.warn(`[${deprecatedAlias}] is deprecated. Use [${optionName}] instead.`);
          verifiedOptions[optionName] = verifiedOptions[deprecatedAlias];
          delete verifiedOptions[deprecatedAlias];
        }
      });
    }
