...
function validationFunction(optionsSchema, providedOptions, overrideOptions, validationOptions) {
  ...
  if (overrideOptions) {
    Object.keys(overrideOptions).forEach(optionName => {
      const overrideValue = overrideOptions[optionName];

      if (!Array.isArray(overrideValue)) {
        verifiedOptions[optionName] = overrideValue;
      } else {
        for (let i = 0; i < overrideValue.length; i++) {
          if (overrideValue[i] != null) {
            verifiedOptions[optionName] = overrideValue[i];
            break;
          }
        }
      }
