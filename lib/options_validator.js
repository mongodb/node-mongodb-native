...
class OperationBuilder {
  ...
  /**
   * Set an OperationBuilder's optionalArguments.
   *
   * @method
   * @param {object} acceptedArguments The optional arguments with their types and indexes.
   * @return {OperationBuilder} An operation builder that will validate options.
   */
  optionalArguments(acceptedArguments) {
    this.optionalArguments = acceptedArguments;
    return this;
  }
  ...
  build(operationToBuild) {
    ...
    function operation() {
      ...
      if (
        operationBuilder.optionalArguments &&
        args.length <
          (callback === undefined
            ? operationBuilder.requiredArity + 1
            : operationBuilder.requiredArity + 2)
      ) {
        Object.keys(operationBuilder.optionalArguments).forEach(optionalArg => {
          // insert new object
          args.splice(operationBuilder.optionalArguments[optionalArg].position, 0, {});
        });
        optionsIndex++;
      }

      assertArity(args, operationBuilder.requiredArity);

      let validationOptions = { validationLevel: VALIDATION_LEVEL_ERROR };
