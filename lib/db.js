...
var listCollectionsTranforms = {
  query: function(result) {
    if(result.documents.length == 1 && result.documents[0].collections) {
      return result.documents[0].collections;          
    }
...
Db.prototype.listCollections = function(filter, options) {
  ...
  if(this.serverConfig.capabilities().hasListCollectionsCommand) {
    ...
    options.transforms = listCollectionsTranforms;
    // Execute the cursor
    return this.s.topology.cursor(f('%s.$cmd', this.s.databaseName), command, options);
  }

  // If we have legacy mode and have not provided a full db name filter it
  if(typeof filter.name == 'string' && filter.name.indexOf('.') == -1) {
