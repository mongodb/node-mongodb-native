...
Db.prototype.aggregate = function(pipeline, options, callback) {
  ...
  // Build the command
  const command = { aggregate: 1, pipeline: pipeline };
  ...
  if (
    ...
    applyWriteConcern(command, { db: this }, options);
  }
  ...
  const namespace = `${this.s.databaseName}.1`;
  ...
  if (typeof callback !== 'function') {
    ...
    // Execute the cursor
    return this.s.topology.cursor(namespace, command, options);
  }

  return handleCallback(callback, null, this.s.topology.cursor(namespace, command, options));
};
