...
var EventEmitter = require('events').EventEmitter
  ...
  , oplogTranslator = require('./oplogTranslator')
...
Db.prototype.changes = function(pipeline, options) {
  ...
  if (serverSupportsChangeNotifications) {
    ...
    return this.aggregate([{$changeNotifications:{}}].concat(pipeline))
  } else {
    return oplogTranslator.createChangeNotificationCursor(this.s.topology, this.databaseName, pipeline)
  }
}
  ...
  _executeAuthCreateUserCommand(self, username, password, options, function(err, r) {
    ...
    if(err && err.code == -5000) {
      ...
      var db = options.dbName
        ? new Db(options.dbName, self.s.topology, self.s.options)
        : self;
...
var setProfilingLevel = function(self, level, callback) {
  ...
  self.command(command, function(err, doc) {
...
var profilingInfo = function(self, callback) {
  try {
    self.collection('system.profile').find({}).toArray(callback);
  } catch (err) {
...
var profilingLevel = function(self, callback) {
  self.command({profile:-1}, function(err, doc) {
    if(err == null && doc.ok === 1) {
      var was = doc.was;
