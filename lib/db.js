...
/** @typedef {InstanceType<import('./sdam/server')['Server']>} Server */
/** @typedef {import('./admin')} Admin */
/** @typedef {import('./mongo_client').ReadConcernLevel} ReadConcernLevel */
/** @typedef {import('./read_preference')} ReadPreference */
/** @typedef {import('bson').Code} Code */
/** @typedef {import('bson').Timestamp} Timestamp */

/**
 * @file The **Db** class is a class that represents a MongoDB Database.
 *
 * @example
 ...
 * @param {Server} topology The server topology for the database.
 ...
 * @param {boolean} [options.noListener] Do not make the db an event listener to the original connection.
...
class Db {
  ...
  get bufferMaxEntries() {
    return this.s.bufferMaxEntries;
  }
  get databaseName() {
   ...
   * @param {DbResultCallback} [callback] The command result callback
   ...
   * @param {string | {level: string}} [options.readConcern] Specify a read concern for the collection. (only MongoDB 3.2 or higher supported)
   * @param {boolean} [options.ignoreUndefined] Specify if the BSON serializer should ignore undefined fields.
   * @param {Promise} [options.promiseLibrary] Promise Library to use
   * @param {DbCollectionResultCallback} [callback] The collection result callback
  ...
  collection(name, options, callback) {
    ...
    if ('object' === typeof options.readConcern) {
      options.readConcern = new ReadConcern(options.readConcern.level);
    } else if ('object' === typeof options.readConcern) {
   ...
   * @param {DbResultCallback} [callback] The collection result callback
   * @returns {Promise} returns Promise if no callback passed
   */
   ...
   * @param {ClientSession} [options.session] optional session to use for this operation
   * @param {DbCollectionResultCallback} [callback] The results callback
   * @returns {Promise} returns Promise if no callback passed
   */
   ...
   * @param {ClientSession} [options.session] optional session to use for this operation
   * @param {DbResultCallback} [callback] The results callback
   * @returns {Promise} returns Promise if no callback passed
   */
 ...
 *
 * @callback DbCollectionResultCallback
 ...
 *
 * @callback DbAggregationCallback
 * @param {MongoError} [error] An error instance representing the error during the execution.
 * @param {AggregationCursor} [cursor] The cursor if the aggregation command was executed successfully.
 */

/** @typedef {(error?: Error | MongoError, result?: any) => void} DbResultCallback */
