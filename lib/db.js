...
var Db = function(databaseName, topology, options) {
  ...
  if(typeof options.native_parser == 'boolean') {
  ...
  this.open = function(callback) {
    topology.connect(self, options, function(err, topology) {
      ...
      if(err) return internalCallback(err);
  ...
  this.command = function(selector, options, callback) {
    ...
    var dbName = options.dbName || databaseName;
    // Clone the options
    ...
    // Execute command
    topology.command(f('%s.$cmd', dbName), selector, options, function(err, result) {
  ...
  this.admin = function(callback) {
    if(callback == null) return new Admin(this, topology);
    handleCallback(callback, null, new Admin(this, topology));
  };
  ...
  this.createCollection = function(name, options, callback) {
    ...
    this.collectionNames(name, function(err, collections) {
      // console.log("-------------------------------- createCollection 0")
      if(err != null) return handleCallback(callback, err, null);
      var found = false;
      ...
      // console.log("-------------------------------- createCollection 2")
        return handleCallback(callback, new Error(f("Collection %s already exists. Currently in strict mode.", name)), null);
      } else if(found){
        try {
      ...
      self.command(cmd, finalOptions, function(err, result) {
        if(err) return handleCallback(callback, err);
        // console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ 1")
      // console.log("-------------------------------- createCollection 5")
  ...
  this.collectionsInfo = function(name, callback) {
    ...
    if(callback) handleCallback(callback, null, self.collection(Db.SYSTEM_NAMESPACE_COLLECTION).find(selector));
  ...
  this.dropDatabase = function(options, callback) {
    if(typeof options == 'function') callback = options, options = {};
    options = options || {};
  ...
  this.cursorInfo = function(options, callback) {
    if(typeof options == 'function') callback = options, options = {};
    options = options || {};
  ...
  this.dropIndex = function(collectionName, indexName, options, callback) {  
    var args = Array.prototype.slice.call(arguments, 2);
    callback = args.pop();
    options = args.length ? args.shift() || {} : {};
  ...
  this.reIndex = function(collectionName, options, callback) {
    if(typeof options == 'function') callback = options, options = {};
    options = options || {};
  ...
  var createIndexUsingCreateIndexes = function(self, name, fieldOrSpec, options, callback) {
    ...
    // // Verify if we have a write concern and merge into command
    // var writeConcern = {};

    // // Remove any write concern operations
    // var writeConcernKeys = ['w', 'wtimeout', 'j', 'fsync'];
    // for(var i = 0; i < writeConcernKeys.length; i++) {
    //   if(options[writeConcernKeys[i]] != null) {
    //     writeConcern[writeConcernKeys[i]] = options[writeConcernKeys[i]];
    //   }
    // }    

    // // Do we have any write concerns, add it to the command
    // if(Object.keys(writeConcernKeys).length > 0) {
