...
var Db = function(databaseName, topology, options) {
  ...
  this.open = function(callback) {
    var self = this;
    self.s.topology.connect(self, options, function(err, topology) {
  ...
  this.collections = function(callback) {
    ...
    this.listCollections(function(err, documents) {
      ...
      handleCallback(callback, null, documents.map(function(d) {
        return new Collection(self, self.s.topology, self.s.databaseName, d.name.replace(self.s.databaseName + ".", ''), self.s.pkFactory, options);
      }));
    });
  };  
  ...
  topology.once('all', createListener('all', self));
}
