...
var createChangeStreamCursor = function (self) {
  ...
  if (self.cachedResumeToken) {
    self.options.resumeAfter = self.cachedResumeToken;
  }
  ...
  if (serverSupportsChangeNotifications) {
    ...
    self.options.cursorNoConflict = self.options.cursorNoConflict || crypto.randomFillSync(new Buffer(16)).toString('hex');
...
var processNewChange = function (self, err, change, callback) {
  ...
  if (err) {
    ...
    if (!self.options.disableResume && err.name === 'MongoNetworkError') {
