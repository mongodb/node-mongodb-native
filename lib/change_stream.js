...
class ChangeStreamCursor extends Cursor {
  ...
  get resumeOptions() {
    ...
    // if the change stream was started with a startAfter and hasn't received results
    // it needs to continue to use startAfter, otherwise it should use resumeAfter
...
function createChangeStreamCursor(self, options) {
  ...
  // if the change stream was started with startAfter but has received results,
  // it should use resumeAfter when recovering from a resumable error
  if (changeStreamStageOptions.startAfter && self.hasReceived) {
    changeStreamStageOptions.resumeAfter = changeStreamStageOptions.startAfter;
    delete changeStreamStageOptions.startAfter;
  }
...
function processNewChange(args) {
  ...
  // mark the changeStream as having received changes to ensure it
  // uses resumeAfter even if it was initially called with startAfter
  if (change.operationType !== 'invalidate') {
    changeStream.hasReceived = true;
  }
