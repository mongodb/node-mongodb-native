...
function processNewChange(args) {
  ...
  if (error) {
    if (isResumableError(error)) {
      // set the cursor to null so getCursor returns a fresh cursor
      ...
      getCursor(
        ...
        // provide the old cursor so the options can be persisted
        cursor
      );
      return;
    }
 ...
 */
function getCursor(changeStream, callback, oldCursor) {
  // if a cursor exists and it is open, return it
  if (changeStream.cursor) {
    if (changeStream.isClosed()) {
      callback(new MongoError('Change Stream is not open.'));
      return;
    }
  ...
  if (
    changeStream.waitingForTopology || // don't reconnect if already reconnecting
    !oldCursor // only reconnect if the previous cursor was provided
  ) {
    return;
  }
  ...
  waitForTopologyConnected(
    ...
    { readPreference: oldCursor.options.readPreference },
    err => {
      ...
      changeStream.cursor = createChangeStreamCursor(changeStream, oldCursor.resumeOptions);
      processResumeQueue(changeStream, undefined, changeStream.cursor);
    }
  );
}
 ...
 * @param {?ChangeStreamCursor} cursor the new cursor
 */
function processResumeQueue(changeStream, err, cursor) {
