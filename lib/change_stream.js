...
ChangeStream.RESUMABLE_ERROR_CODES = RESUMABLE_ERROR_CODES;
ChangeStream.GET_MORE_NON_RESUMABLE_CODES = GET_MORE_NON_RESUMABLE_CODES;

function isGetMoreResumableError(error) {
  if (!error[MongoError.mongoContext]) {
    return false;
  }

  if (!error[MongoError.mongoContext][MongoError.mongoContextFlags.isGetMore]) {
    return false;
  }

  if (GET_MORE_NON_RESUMABLE_CODES.has(error.code)) {
    return false;
  }
