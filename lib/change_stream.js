...
function processNewChange({ changeStream, error, change, callback, eventEmitter }) {
  ...
  if (error) {
    if (isResumableError(error, maxWireVersion(cursor.server))) {
      ...
      return resumeFromError({
        changeStream,
        cursor, // the old cursor so its options can be preserved
        eventEmitter,
        callback: (resumeError, result) =>
          maybePromise(callback, cb => {
            if (eventEmitter && resumeError) {
              changeStream.emit('error', resumeError);
              changeStream.emit('close');
            }
            cb(resumeError, result);
          })
      });
    }
