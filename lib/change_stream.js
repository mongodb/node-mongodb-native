...
/** @typedef {import('./mongo_client')} MongoClient */
...
/**
 * @typedef ResumeToken
 * @property {string|Binary} _data type depends on the MongoDB versions
...
/**
 * @typedef {import('bson').Timestamp} OperationTime
 * @description Represents a specific point in time on a server. Can be retrieved by using {@link Db#command}
 * @see https://docs.mongodb.com/manual/reference/method/db.runCommand/#response
 */

/**
 * @typedef ChangeStreamOptions Options that can be passed to a ChangeStream. Note that startAfter, resumeAfter, and startAtOperationTime are all mutually exclusive, and the server will error if more than one is specified.
...
class ChangeStream extends EventEmitter {
   ...
   *
   * @param {ChangeStreamResultCallback} [callback] The result callback.
   * @throws {MongoError}
  ...
  next(callback) {
    ...
    return (
      this.cursor
        .next()
        // @ts-ignore TODO:(ts) We need better typing for these maybePromise functions
  ...
  close(callback) {
    ...
    if (callback) {
      //@ts-ignore TODO:(ts) function overloading needed or better params
 ...
 *
 * @callback ChangeStreamResultCallback
 * @param {Error | MongoError} [error] An error instance representing the error during the execution.
