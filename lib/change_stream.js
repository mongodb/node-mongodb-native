...
const { relayEvents, maxWireVersion, maybePromise } = require('./utils');
...
class ChangeStream extends EventEmitter {
  constructor(parent, pipeline, options) {
    ...
    createChangeStreamCursor(this, options);
...
// Wait for topology to re-connect, then create a new cursor.
// If in eventEmitter mode, returns null and doesn't iterate the change stream.
function resumeFromError({ changeStream, callback, eventEmitter }) {
  const { cursor } = changeStream;
  const {
  ...
  // attempt resume
  const resumeAttempt = maybePromise(callback, cb => {
    waitForTopologyConnected(changeStream.topology, { readPreference }, err => {
      if (err) {
        if (eventEmitter) {
      ...
      createChangeStreamCursor(changeStream, resumeOptions);

      if (eventEmitter) {
        cb();
      } else {
        changeStream.next(cb);
      }
    });
  });
...
function processNewChange({ changeStream, error, change, callback, eventEmitter }) {
  const cursor = changeStream.cursor;

  // If the cursor is null, then change stream is attempting to resume from an error and shouldn't process a change.
  if (!cursor) {
    return;
  }

  // If there's an error and it is resumable, attempt to resume. Otherwise, raise an error.
  if (error) {
    if (isResumableError(error, maxWireVersion(cursor.server))) {
      return resumeFromError({ changeStream, eventEmitter, callback });
    }

    if (eventEmitter) return changeStream.emit('error', error);
    return maybePromise(callback, cb => cb(error));
  }
  ...
  if (change && !change._id) {
    ...
    return maybePromise(callback, cb => cb(noResumeTokenError));
  }
  ...
  return maybePromise(callback, cb => cb(error, change));
}
