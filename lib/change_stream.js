  ...
  constructor(changeDomain, pipeline, options) {
    ...
    this.on('newListener', eventName => {
      if (eventName === 'change' && this.cursor && this.cursor.listenerCount('change') === 0) {
        this.cursor.on('data', change => processNewChange(this, null, change, null, true));
      }
    });
  ...
  next(callback) {
    ...
    return this.cursor
      ...
      .then(function(change) {
        return processNewChange(self, null, change, callback, false);
      })
      .catch(function(err) {
        return processNewChange(self, err, null, callback, false);
      });
  }
...
var createChangeStreamCursor = function(self) {
  ...
  if (self.listenerCount('change') > 0) {
    changeStreamCursor.on('data', function(change) {
      processNewChange(self, null, change, null, true);
    });
  }
   ...
   * @type {Error}
   */
  changeStreamCursor.once('error', function(error) {
    processNewChange(self, error, null, null, true);
  });
...
var processNewChange = function(self, err, change, callback, isStream) {
  // Handle errors
  if (err) {
    ...
    if (isResumableError(err)) {
      ...
      if (isStream) {
        ...
        return;
        // nothing, verify that 'data' event is attached to and that cursor has sent another command
      }

      if (callback) {
        return restartCursor(self, callback);
      }

      // not a stream or using a callback
      return restartCursor(self);
    }
  }
  ...
  if (!change || !change._id) {
    ...
    if (isStream) return self.emit('error', noResumeTokenError);
...
function restartCursor(self, callback) {
  if (callback) {
