...
var ChangeStream = function(changeDomain, pipeline, options) {
  ...
  self.namespace = {};
  var changeDb;
  if (self.changeDomain instanceof Collection) {
    self.namespace.collection = self.changeDomain.collectionName;
    changeDb = self.changeDomain.s.db;
  } else if (self.changeDomain instanceof Db) {
    changeDb = self.changeDomain;
  } else {
  ...
  self.readPreference = changeDb.readPreference;
...
var createChangeStreamCursor = function (self) {
  ...
  if (serverSupportsChangeNotifications) {
    changeStreamCursor = buildChangeStreamAggregationCommand(self.serverConfig, self.namespace, self.readPreference, self.pipeline, self.resumeToken, self.options);
  } else {
...
var buildChangeStreamAggregationCommand = function (serverConfig, namespace, readPreference, pipeline, resumeToken, options) {
    ...
    if (options.maxAwaitTimeMS) {
      aggregationOptions.maxAwaitTimeMS = options.maxAwaitTimeMS;
    }
    if (options.batchSize) {
      aggregationOptions.batchSize = options.batchSize;
    }
    if (options.collation) {
      aggregationOptions.collation = options.collation;
    }
    if (readPreference) {
      aggregationOptions.readPreference = readPreference;
    }
    ...
    // Execute the cursor
    return serverConfig.cursor(namespace.database + '.' + namespace.collection, command, aggregationOptions);
};
