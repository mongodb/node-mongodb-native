...
class ChangeStreamCursor extends Cursor {
  constructor(topology, operation, options, resumeKey) {
  ...
  get resumeOptions() {
    ...
    if (this.resumeToken || this.startAtOperationTime) {
      ...
      if (this.resumeToken) {
        result[this.resumeKey] = this.resumeToken;
...
function createChangeStreamCursor(self, options) {
  ...
  const changeStreamCursor = new ChangeStreamCursor(
    ...
    cursorOptions,
    options.startAfter && !self.hasReceived ? 'startAfter' : 'resumeAfter'
  );
...
function processNewChange(args) {
  ...
  if (error) {
    if (isResumableError(error, wireVersion) && !changeStream.attemptingResume) {
      ...
      changeStream.emit('attemptingResume');
