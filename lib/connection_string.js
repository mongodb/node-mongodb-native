...
function assertRepelOptions(options, optionKeyA, optionKeyB) {
  if (options.hasOwnProperty(optionKeyA) && options.hasOwnProperty(optionKeyB)) {
...
/**
 * Checks options that are invalid if MONGO+SRV is the protocol
 *
 * @param {object} options Optional user provided connection string options
 * @throws {MongoParseError}
 */
function checkSSLOptions(options) {
  if (!options) return;
  // I didn't expect to have lowercase versions of directconnection here, aparently tests pass caseTranslate: false
  // which internally switches all options from the passed in directConnection to directconnection
  if (options.ssl && (options.directconnection || options.directConnection)) {
...
function parseConnectionString(uri, options, callback) {
  ...
  if (hosts.length !== 1 && parsedOptions.directConnection) {
    // If the option is set to true, the driver MUST validate that there is exactly one host given in the host list in the URI, and fail client creation otherwise.
