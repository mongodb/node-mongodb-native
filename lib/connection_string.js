...
function parseSrvConnectionString(uri, options, callback) {
  ...
  dns.resolveSrv(`_mongodb._tcp.${lookupAddress}`, (err, addresses) => {
    ...
    if (
      ...
      result.query.ssl = /** @type {any} */ (true); // TODO:(ts) result.query.ssl: string | string[]
    }
    ...
    dns.resolveTxt(lookupAddress, (err, record) => {
      ...
      if (record) {
        ...
        record = /** @type {any} */ (qs.parse(record[0].join('')));
 ...
 *
 * @param {ParsedUrlQuery} queryString The query string to check
...
function assertTlsOptionsAreEqual(optionName, queryString, queryStringKeys) {
  ...
  if (queryStringHasTLSOption) {
    const opVal = queryString[optionName];
    if (Array.isArray(opVal)) {
      const firstValue = opVal[0];
      opVal.forEach(tlsValue => {
