...
function emitTopologyDescriptionChanged(self) {
  ...
  // Merge in all the servers into description
  if(self.isPrimaryConnected()) {
    description.servers.push(self.primary.getDescription());
  }

  // Merge all other servers into description
  ...
  if(self.replSet.listeners('topologyDescriptionChanged').length > 0) {
    self.replSet.emit('topologyDescriptionChanged', {
      topologyId: self.id,
 ...
 */
State.prototype.remove = function(server, noDescriptionEmit) {
  ...
  // Emit new sdam topology description
  if(!noDescriptionEmit) emitTopologyDescriptionChanged(this);
...
State.prototype.promotePrimary = function(server) {
  ...
  this.remove(currentServer, true);
  // Set as primary
