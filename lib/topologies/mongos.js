...
var State = function(readPreferenceStrategies) {
  ...
  //
  // Pick a server
  this.pickServer = function(readPreference, callback) {
    options = options || {};
    ...
    if(readPreferenceStrategies != null && readPreferenceStrategies[readPreference] != null) {
      return readPreferenceStrategies[readPreference].pickServer(connectedServers, readPreference, callback);
    }

    // No valid connections
    if(connectedServers.length == 0) return callback(new MongoError("no mongos proxy available"), null);
    // Pick first one
    return callback(null, connectedServers[0]);
  }
}
