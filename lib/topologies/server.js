...
var executeSingleOperation = function(self, ns, cmd, queryOptions, options, onAll, callback) {
    ...
    for(var i = 0; i < connections.length; i++) {
      ...
      var commandCallback = function(err, result) {
        ...
        if(total == 0) {
          ...
          result.hashedName = connection.hashedName;

          // Execute callback, catch and rethrow if needed
          try { callback(null, new CommandResult(options.fullResult ? result : result.documents[0], connections)); }
      ...
      self.s.callbacks.register(query.requestId, callback);
    }
  ...
  var commandCallback = function(err, result) {
    ...
    if(result.documents[0]['$err']
      ...
      // Execute callback, catch and rethrow if needed
      try { callback(null, new CommandResult(options.fullResult ? result : result.documents[0], connection)); }
      catch(err) { process.nextTick(function() { throw err}); }
  };
...
Server.prototype.command = function(ns, cmd, options, callback) {
  ...
  for(var i = 0; i < queries.length; i++) {
    ...
    var commandCallback = function(err, result) {
      ...
      // Push results
      results.push(options.fullResult ? result : result.documents[0]);
    ...
    // Do we want raw BSON returned
    if(raw) commandResults.raw = true;
