...
Callbacks.prototype.flushConnection = function(err, connection) {
  // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FLUSH CONNECTIONS 0 -- " + connection.id)
  for(var id in this.callbacks) {
    if(!isNaN(parseInt(id, 10))) {
      var callback = this.callbacks[id];
      // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FLUSH CONNECTIONS 1 :: " + (this.callbacks[id] != null) + " :: " + (callback.connection === connection) + " :: ")
      // console.dir(callback.connection)

      // Validate if the operation ran on the connection
      if(callback.connection === connection) {
        // console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! FLUSH CONNECTIONS 2")
        delete this.callbacks[id];
        callback(err, null);
      }
    }
...
var timeoutHandler = function(self, state) {
  return function(err, connection) {
    ...
    // console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$ SERVER timeout :: " + self.name)
...
var inquireServerState = function(self) {
  // console.log("!!!!!!!!!!!!!!!!!!!!! ADDED inquireServerState :: " + self.name)
  return function() {
    // console.log("============================ SERVER 0 :: " + self.name + " :: " + self.s.state)
    if(self.s.state == DESTROYED) return;
    // console.log("============================ SERVER 1 :: " + self.name)
    // Attempt to execute ismaster command
    self.command('admin.$cmd', { ismaster:true },  { monitoring:true }, function(err, r) {
      // console.log("============================ SERVER 2 :: " + self.name)
      if(!err) {
