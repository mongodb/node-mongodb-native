...
Callbacks.prototype.flushConnection = function(err, connection) {
  for(var id in this.callbacks) {
    if(!isNaN(parseInt(id, 10))) {
      ...
      //   console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! flushConnection = " + (callback.connection == null))
      //   console.log(callback.toString())
      //   // console.log(callback.connection.id + " = " + connection.id)
      // }
...
var closeHandler = function(self, state) {
  return function(err, connection) {
    // console.log("---------------- errorHandler :: " + connection.port)
    if(state.state == DISCONNECTED || state.state == DESTROYED) return;
  ...
  return function() {
    ...
    applyAuthentications(function() {
      ...
      self.command('admin.$cmd', {ismaster:true}, function(err, r) {
        ...
        // console.log("##### server connect even firing 0 " + self.name)
        ...
        // console.log("##### server connect even firing 1 " + self.name)
        ...
        if(!supportsServer(state) && state.wireProtocolHandler == null) {
          // console.log("##### server connect even firing 1:1 " + self.name)
          state.state = DISCONNECTED
          return self.emit('error', new MongoError("non supported server version"), self);
        }

        // console.log("##### server connect even firing 2 " + self.name)
        // console.dir(state.serverDetails)

        try {
        // Set the details
        if(state.ismaster && state.ismaster.me) {
          // console.log("##### server connect even firing 2:1 " + self.name)
          // console.log(state.serverDetails.name)
          // console.dir(state.ismaster)
          state.serverDetails.name = state.ismaster.me;
          // console.log("##### server connect even firing 2:2 " + self.name)
        }
      }catch(e) {
        console.log(e.stack)
      }

        // console.log("##### server connect even firing 3 " + self.name)
...
Server.prototype.connect = function(_options) {
  ...
  // var events = ['timeout', 'close', 'error', 'connect', 'parseError', 'connection'];
  // for(var i = 0; i < events.lenght; i++) {
  //   self.s.pool.removeAllListeners(events[i]);
  // }
...
var executeSingleOperation = function(self, ns, cmd, queryOptions, options, onAll, callback) {
  ...
  try {
    // if(monitoring)
    //   console.log("=============== command monitoring")
    // console.log("======= execute")
    // console.dir(options)
