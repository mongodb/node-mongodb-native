...
var ReplSet = function(seedlist, options) {
  ...
  var addToListIfNotExist = function(list, server) {
    ...
    for(var i = 0; i < list.length; i++) {
      console.log("existing " + list[i].lastIsMaster().me + " " + list[i].equal(server))
  ...
  var replicasetInquirer = function() {    
    ...
    if(logger.isInfo()) logger.info(f('replicasetInquirer process running %s', JSON.stringify(replState)));    

    disconnectedServers.forEach(function(m) {
      console.log(m.lastIsMaster())
    })
    ...
    while(disconnectedServers.length > 0) {
      ...
      if(logger.isInfo()) logger.info(f('replicasetInquirer attempting to connect to %s', server.lastIsMaster() ? server.lastIsMaster().me : 'N/A'));
  ...
  var connectHandler = function(server) {
    ...
    server.command('system.$cmd', {ismaster:true}, function(err, r) {
      ...
      console.log(f("=================== %s server.isConnected() " + server.isConnected(), server.lastIsMaster().me))
      ...
      for(var i = 0; i < ismaster.hosts.length; i++) {
        ...
        if(replState.primary && replState.primary.equal(ismaster.hosts[i])) {
          found = true;
        }

        // Check if we have a secondary
        for(var j = 0; j < replState.secondaries.length; j++) {
          if(replState.secondaries[j].equal(ismaster.hosts[i])) {
            found = true;
          }
        }
        ...
        if(!found) {
          if(connectingServers[ismaster.hosts[i]] == null) {
            if(logger.isInfo()) logger.info(f('schedule server %s for connection', ismaster.hosts[i]));
            // Make sure we know what is trying to connect            
            connectingServers[ismaster.hosts[i]] = ismaster.hosts[i];            
            // Connect the server
            connectToServer(ismaster.hosts[i].split(':')[0], parseInt(ismaster.hosts[i].split(':')[1], 10));
          }
        }
      }
    });
