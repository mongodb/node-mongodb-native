...
var Server = function(host, port, options) {
  ...
  if(options.socketOptions && Object.keys(options.socketOptions).length > 0) {
    socketOptions = options.socketOptions;
  }

  if(socketOptions.connectTimeoutMS) {
    this.connectTimeoutMS = socketOptions.connectTimeoutMS;
    clonedOptions.connectionTimeout = socketOptions.connectTimeoutMS;
  }

  if(socketOptions.socketTimeoutMS) {
    clonedOptions.socketTimeout = socketOptions.socketTimeoutMS;
  }

  // Map keep alive setting
  if(typeof socketOptions.keepAlive == 'number') {

    clonedOptions.keepAlive = socketOptions.keepAlive > 0;

    if(typeof socketOptions.keepAlive == 'number') {
      clonedOptions.keepAliveInitialDelay = socketOptions.keepAlive;
    }
  }

  // Set no delay if specified
  if(typeof socketOptions.noDelay == 'boolean') {
    clonedOptions.noDelay = socketOptions.noDelay;
  }
