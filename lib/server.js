...
var Server = function(host, port, options) {
  ...
  // console.dir(storeOptions)
    ...
    var errorHandler = function(event) {
      return function(err) {
        // console.log("------------------------------------- err 1 :: " + event)
        // console.log(self.listeners(event).length)
        if(event != 'error') {
          self.emit(event, err);
        }
      }
    ...
    // // All the events to remove
    // var events = ['timeout', 'error', 'close', 'connect', 'reconnect'];
    // events.forEach(function(e) {
