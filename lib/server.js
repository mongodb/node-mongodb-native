...
var Server = function(host, port, options) {
  ...
  this.insert = function(ns, ops, options, callback) {
    if(!server.isConnected()) return store.add('insert', ns, ops, options, force, bufferMaxEntries, clonedOptions, callback);
    server.insert(ns, ops, options, callback);
  }

  // Update
  this.update = function(ns, ops, options, callback) {
    if(!server.isConnected()) return store.add('update', ns, ops, options, force, bufferMaxEntries, clonedOptions, callback);
    server.update(ns, ops, options, callback);
  }

  // Remove
  this.remove = function(ns, ops, options, callback) {
    if(!server.isConnected()) return store.add('remove', ns, ops, options, force, bufferMaxEntries, clonedOptions, callback);
