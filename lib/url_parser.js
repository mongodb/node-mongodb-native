...
module.exports = function(url, options, callback) {
  // First do the async stuff checking for srv and txt records
  ...
  if (result.protocol === 'mongodb+srv:') {
    ...
    dns.resolveSrv(srvAddress, function(err, addresses) {
      ...
      dns.resolveTxt(result.host, function(err, records) {
        ...
        // If nodata, continue with records null
        if (err && err.code === 'ENODATA') records = null;
        // Compose a connection string based on each of the addresses
        // Priority and Weight fields are ignored
        ...
        if (records) {
          records.forEach(function(record, i) {
            // Check each record
            // TODO Cannot have & at end of record
            // TODO Throw parse exception if anything is incorrectly formatted
            // TODO Do not support any authentification creditials in txt records
            // TODO Warn if two txt records have the same options
            // TODO options in connection string API override txt records
            if (i === 0) connectionString += `/?`;
            if (i === records.length - 1) return (connectionString += record);
            else return (connectionString += `${record}&`);
          });
        }
        callback(null, parseConnectionString(connectionString));
        // Now continue parsing as normal
      });
    });
  } else {
    callback(null, parseConnectionString(url, options));
  }
};
