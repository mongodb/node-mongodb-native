...
function parseConnectionString(url, options) {
  ...
  // Go through each option, get the key and value
  // Try each in switch statements, if they throw exception
  // set no values
  urlOptions.forEach(function(option) {
    if (!option) return;
    var splitOpt = option.split('='),
      name = splitOpt[0],
      value = splitOpt[1];

    try {
      optionCases(name, value);
    } catch (err) {
      if (err.message === 'Value invalid') return;
      else throw err;
    }
  });

  function optionCases(name, value) {
    ...
    switch (name) {
      ...
      case 'maxPoolSize':
      case 'poolSize':
        serverOptions.poolSize = validateInteger(value);
        replSetServersOptions.poolSize = validateInteger(value);
        break;
      case 'appname':
      ...
      case 'rs_name':
        ...
        break;
      case 'reconnectWait':
        replSetServersOptions.reconnectWait = validateInteger(value);
        break;
      case 'retries':
        replSetServersOptions.retries = validateInteger(value);
        break;
      case 'readSecondary':
      case 'read_secondary':
      ...
      case 'connectTimeoutMS':
        serverOptions.socketOptions.connectTimeoutMS = validateInteger(value);
        replSetServersOptions.socketOptions.connectTimeoutMS = validateInteger(value);
        mongosOptions.socketOptions.connectTimeoutMS = validateInteger(value);
        break;
      case 'socketTimeoutMS':
        serverOptions.socketOptions.socketTimeoutMS = validateInteger(value);
        replSetServersOptions.socketOptions.socketTimeoutMS = validateInteger(value);
        mongosOptions.socketOptions.socketTimeoutMS = validateInteger(value);
        break;
      case 'w':
        dbOptions.w = validateInteger(value);
        // TODO What about this, it won't get here
      ...
      case 'authMechanismProperties':
        ...
        break;
      case 'wtimeoutMS':
        dbOptions.wtimeout = validateInteger(value);
        break;
      case 'readPreference':
      ...
      case 'maxStalenessSeconds':
        dbOptions.maxStalenessSeconds = validateInteger(value);
        break;
      ...
      case 'zlibCompressionLevel':
        compression = serverOptions.compression || {};
        var zlibCompressionLevel = validateInteger(value);
        ...
        break;
      default:
        // TODO this does not make it through the try/catch in the forEach
...
function validateInteger(value) {
  ...
  if (isNaN(result)) {
    Logger('Url Parser').warn('Expected an integer but got: ' + value);
