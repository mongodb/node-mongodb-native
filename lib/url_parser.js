...
module.exports = function(url, options) {
  ...
  } else if(dbOptions.read_preference_tags.length > 0) {
    var tags = {};

    for(var i = 0; i < dbOptions.read_preference_tags.length; i++) {
      for(var name in dbOptions.read_preference_tags[i]) {
        tags[name] = dbOptions.read_preference_tags[i][name];
      }
    }

    dbOptions.read_preference_tags = tags;
  }
